...
class StaticInfoTablesDiv extends \tx_staticinfotables_div {
	 ...
	 *
	 * @param
	 *        	array		itemsProcFunc data array
	 * @return void
	 */
	function selectItemsTCA($params) {
		/*
		 * $params['items'] = &$items; $params['config'] = $config; $params['TSconfig'] = $iArray; $params['table'] = $table; $params['row'] = $row; $params['field'] = $field;
		 */
		$table = $params ['config'] ['itemsProcFunc_config'] ['table'];
		$tcaWhere = $params ['config'] ['itemsProcFunc_config'] ['where'];
		$where = StaticInfoTablesDiv::replaceMarkersInSQL ($tcaWhere, $params ['table'], $params ['row']);
		
		if ($table) {
			$indexField = $params ['config'] ['itemsProcFunc_config'] ['indexField'];
			$indexField = $indexField ? $indexField : 'uid';
			
			$lang = strtolower (\tx_staticinfotables_div::getCurrentLanguage ());
			...
			if ($params ['config'] ['itemsProcFunc_config'] ['prependHotlist']) {
				...
				$res = $GLOBALS ['TYPO3_DB']->exec_SELECT_mm_query ($fields, $table, 'tx_staticinfotables_hotlist', '', 				// $foreign_table
				'AND tx_staticinfotables_hotlist.application=' . $GLOBALS ['TYPO3_DB']->fullQuoteStr ($app, 'tx_staticinfotables_hotlist'), '', 'tx_staticinfotables_hotlist.sorting DESC', 				// $orderBy
				$limit);
