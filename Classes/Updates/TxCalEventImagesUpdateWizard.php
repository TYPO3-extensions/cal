 ...
 */
class TxCalEventImagesUpdateWizard extends \TYPO3\CMS\Install\Updates\AbstractUpdate {
	...
	public function checkForUpdate(&$description) {
		...
		if ($notMigratedRowsCount > 0) {
			$description = 'There are Content Elements of type "image" which are referencing files that are not using ' . ' the File Abstraction Layer. This wizard will move the files to fileadmin/' . self::FOLDER_ContentUploads . ' and index them.';
			$updateNeeded = TRUE;
		}
		return $updateNeeded;
	}
	...
	public function performUpdate(array &$dbQueries, &$customMessages) {
		$this->init();
		$records = $this->getRecordsFromTable('tx_cal_event');
