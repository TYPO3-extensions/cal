 ...
 */
class TxCalLocationImagesUpdateWizard extends \TYPO3\CMS\Install\Updates\AbstractUpdate {
	...
	public function checkForUpdate(&$description) {
		...
		if ($notMigratedRowsCount > 0) {
			$description = 'There are calendar locations of type "image" which are referencing files that are not using ' . ' the File Abstraction Layer. This wizard will move the files to fileadmin/' . self::FOLDER_ContentUploads . ' and index them.';
			$updateNeeded = TRUE;
		}
		return $updateNeeded;
	}
	...
	public function performUpdate(array &$dbQueries, &$customMessages) {
		$this->init();
		$records = $this->getRecordsFromTable('tx_cal_location');
...
', $record['imagetitletext']);
	...
	protected function cleanRecord(array $record, $fileCount, array $collectionUids) {
		$GLOBALS['TYPO3_DB']->exec_UPDATEquery('tx_cal_location', 'uid = ' . $record['uid'], array(
			'image' => $fileCount,
			'imagecaption' => '',
			'imagetitletext' => ''
		));
	...
	protected function getTableColumnMapping() {
		...
		if ($GLOBALS['TYPO3_DB'] instanceof \TYPO3\CMS\Dbal\Database\DatabaseConnection) {
			if (!empty($GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['dbal']['mapping']['tx_cal_location'])) {
				$mapping = array_merge_recursive($mapping, $GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['dbal']['mapping']['tx_cal_location']);
			}
		}
