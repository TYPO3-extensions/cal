...
class RecurrenceGenerator
{
    ...
    public function __construct($pageIDForPlugin = null, $starttime = null, $endtime = null)
    {
        ...
        if ($starttime == null) {
            $starttime = $this->getTimeParsed($this->extConf ['recurrenceStart'])
                ->format('%Y%m%d');
        }
        $this->starttime = $starttime;
        if ($endtime == null) {
            $endtime = $this->getTimeParsed($this->extConf ['recurrenceEnd'])
                ->format('%Y%m%d');
        }
    ...
    public function cleanIndexTable($pageId)
    {
        $this->getDatabaseConnection()
            ->exec_DELETEquery(self::INDEX_TABLE,
    ...
    {
        $this->getDatabaseConnection()
            ->exec_DELETEquery(self::INDEX_TABLE, 'event_uid = ' . $uid . ' AND tablename = "' . $table . '"');
    }
    ...
    function cleanIndexTableOfCalendarUid($uid)
    {
        ...
        $uids[] = 0;
        $databaseConnection->exec_DELETEquery(self::INDEX_TABLE,
    ...
    function cleanIndexTableOfExceptionGroupUid($uid)
    {
        ...
        $results = $databaseConnection->exec_SELECT_mm_query('tx_cal_exception_event_group.*', 'tx_cal_exception_event',
            'tx_cal_exception_event_mm', 'tx_cal_exception_event_group', $where);
        if ($results) {
    ...
    function generateIndex($eventPage = 0)
    {
        ...
        $this->info .= '</ul>';
        $this->info .= 'Done.';
        $this->info .= '<br/><br/><a href="javascript:history.back();">' . LocalizationUtility::translate('l_back',
                'cal') . '</a>';
    }
    ...
    function getEventService()
    {
        ...
        if (!is_object($eventService)) {
            $this->info = CalIndexer::getMessage('Could not fetch the event service! Please make sure the page id is correct!',
                FlashMessage::ERROR);
        }
        return $eventService;
    }
