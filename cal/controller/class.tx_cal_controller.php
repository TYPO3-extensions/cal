...
class tx_cal_controller extends tslib_pibase {
	...
	function saveEvent() {
		...
		$modelObj = &tx_cal_registry::Registry('basic','modelcontroller');
		$ok = $modelObj->saveEvent($this->conf['uid'], $this->conf['type'], $pid);
	...
	function saveLocation() {
		...
		$modelObj = &tx_cal_registry::Registry('basic','modelcontroller');
		$ok = $modelObj->saveLocation($this->conf['uid'], $this->conf['type'], $pid);
	...
	function saveOrganizer() {
		...
		$modelObj = &tx_cal_registry::Registry('basic','modelcontroller');
		$ok = $modelObj->saveOrganizer($this->conf['uid'], $this->conf['type'], $pid);
	...
	function saveCalendar() {
		...
		$modelObj = &tx_cal_registry::Registry('basic','modelcontroller');
		$ok = $modelObj->saveCalendar($this->conf['uid'], $this->conf['type'], $pid);
	...
	function saveCategory() {
		...
		$modelObj = &tx_cal_registry::Registry('basic','modelcontroller');
		$ok = $modelObj->saveCategory($this->conf['uid'], $this->conf['type'], $pid);
	...
	function searchEvent() {
		...
		$start_day = $this->conf['start_day'];
		$end_day = $this->conf['end_day'];
		$searchword = strip_tags($this->conf['query']);
		...
		*/
		...
		switch(trim($params['view'])) {
			...
			default:
				...
				if($params[$this->getPointerName] || $params['category']) {
					$useLastView = false;
				}
				break;
		}
