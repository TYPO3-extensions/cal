...
class tx_model_iCalendar {
    ...
    /**
     * Gets the params of an parameter as an array.  Multiple values
     * are possible due to:
     *
     *  a) multiplce occurences of 'name'
     *  b) (unsecapd) comma seperated lists.
     ...
     *
     * @return boolean|tx_model_iCalendar_*  False if no matching subcomponent of
        ...
        if (preg_match_all('/(.*):([^\r\n]*)[\r\n]+/', $vCal, $matches)) {
            foreach ($matches[0] as $attribute) {
                ...
                if ((isset($params['ENCODING'])
					...
					$value = $GLOBALS['TSFE']->csConv($value,$csConvObj->parse_charset[$params['CHARSET']] ? $csConvObj->parse_charset[$params['CHARSET']] : 'utf-8');
                    
                } elseif (isset($params['CHARSET'])) {
                    $value = $GLOBALS['TSFE']->csConv($value,$csConvObj->parse_charset[$params['CHARSET']]);
                } else {
                    // As per RFC 2279, assume UTF8 if we don't have an
                    // explicit charset parameter.
                    $value = $GLOBALS['TSFE']->csConv($value,'utf-8');
                }
