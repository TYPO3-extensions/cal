			...
			if (is_a($component,'tx_iCalendar_vevent')){
				...
				if($component->getAttribute('DTSTART')){
					...
					if(is_array($startdate)){
						...
						$insertFields['start_time'] = mktime(date('H',$startdate),date('i',$startdate), 0, 1, 1, 1970);
					}
				}else{
					continue;
				}
				if($component->getAttribute('DTEND')){
					...
					if(is_array($enddate)){
						...
						$insertFields['end_time'] = mktime(date('H',$enddate),date('i',$enddate), 0, 1, 1, 1970);
					}				
				}
				...
				if($component->organizerName()){
					$insertFields['organizer'] = $component->organizerName();
				}
