...
class tx_cal_location_service extends tx_cal_base_service {
	...
	function getLocationFromTable($pidList='', $additionalWhere=''){
		...
		while($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($result)){
			...
			if($row['shared_user_cnt']>0 && !$lastLocation->isSharedUser($feUserUid, $feGroupsArray)){
				continue;
			}
			/* @fixme	Including events breaks the query for organizers if they're not already in an event */
			$this->_addEventLinkToLocation($lastLocation, $row['event_uid']);
	...
	function updateLocation($uid){
		...
		if($this->rightsObj->isAllowedTo('edit','location','shared')){
			...
			$userIdArray = t3lib_div::intExplode(',',$this->conf['rights.']['edit.']['location.']['fields.']['shared.']['defaultUser']);
			if($this->conf['rights.']['edit.']['location.']['addFeUserToShared']){
				$userIdArray[] = $this->rightsObj->getUserId();
			}
			
			$groupIdArray = t3lib_div::intExplode(',',$this->conf['rights.']['edit.']['location.']['fields.']['shared.']['defaultGroup']);
			if($this->conf['rights.']['edit.']['location.']['addFeGroupToShared']){
				$groupIdArray = $this->rightsObj->getUserGroups();
				$ignore = t3lib_div::intExplode(',',$this->conf['rights.']['edit.']['location.']['addFeGroupToShared.']['ignore']);
	...
	function _saveLocation(&$insertFields){
		...
		if($this->rightsObj->isAllowedTo('create','location','shared')){
			...
			$ignore = t3lib_div::intExplode(',',$this->conf['rights.']['create.']['location.']['addFeGroupToShared.']['ignore']);
			...
			if($this->conf['rights.']['create.']['location.']['fields.']['shared.']['defaultGroup']!=''){
				$groupArray = t3lib_div::intExplode(',',$this->conf['rights.']['create.']['location.']['fields.']['shared.']['defaultGroup']);
