...
class tx_cal_organizer_service extends tx_cal_base_service {
	...
	function getOrganizerFromTable($pidList='', $additionalWhere=''){
		...
		if(!$this->conf['view.'][$this->conf['view'].'.']['organizer.']['includeEventsInResult']){
			...
			$where = 'tx_cal_organizer.uid = tx_cal_event.organizer_id AND tx_cal_event.l18n_parent = 0'.$this->cObj->enableFields('tx_cal_event');
		}
		...
		while($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($result)){
			...
			if($row['shared_user_cnt']>0 && !$lastOrganizer->isSharedUser($feUserUid, $feGroupsArray)){
				continue;
			}
	...
	function updateOrganizer($uid){
		...
		if($this->rightsObj->isAllowedTo('edit','organizer','shared')){
			...
			$userIdArray = t3lib_div::intExplode(',',$this->conf['rights.']['edit.']['organizer.']['fields.']['shared.']['defaultUser']);
			if($this->conf['rights.']['edit.']['organizer.']['addFeUserToShared']){
				$userIdArray[] = $this->rightsObj->getUserId();
			}
			
			$groupIdArray = t3lib_div::intExplode(',',$this->conf['rights.']['edit.']['organizer.']['fields.']['shared.']['defaultGroup']);
			if($this->conf['rights.']['edit.']['organizer.']['addFeGroupToShared']){
				$groupIdArray = $this->rightsObj->getUserGroups();
				$ignore = t3lib_div::intExplode(',',$this->conf['rights.']['edit.']['organizer.']['addFeGroupToShared.']['ignore']);
	...
	function _saveOrganizer(&$insertFields){
		...
		if($this->rightsObj->isAllowedTo('create','organizer','shared')){
			...
			$ignore = t3lib_div::intExplode(',',$this->conf['rights.']['create.']['organizer.']['addFeGroupToShared.']['ignore']);
			...
			if($this->conf['rights.']['create.']['organizer.']['fields.']['shared.']['defaultGroup']!=''){
				$groupArray = t3lib_div::intExplode(',',$this->conf['rights.']['create.']['organizer.']['fields.']['shared.']['defaultGroup']);
