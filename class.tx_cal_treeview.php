	...
	function displayCategoryTree($PA, $fobj)    {
		...
		if ($maxitems<=1 AND !$config['treeView'])	{
			...
			if ($row['sys_language_uid'] && $row['l18n_parent'] ) { // the current record is a translation of another record
				...
				$catres = $GLOBALS['TYPO3_DB']->exec_SELECT_query('tx_cal_category.uid, tx_cal_caegory.title', 'tx_cal_category');
				$categories = array();
				$NACats = array();
				$na = false;
				...
				if($config['treeView'] AND $config['foreign_table']) {
					...
					$treeViewObj->parentField = 'calendar_id';
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/cal/class.tx_cal_treeview.php'])    {
    include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/cal/class.tx_cal_treeview.php']);
}
?>
