...
class tx_cal_calendar {
	...
	function calculateStartWeekTime($dateObject='', $weekStartDay='monday'){
		if(strtolower($weekStartDay) == 'monday') {
			$dayOffset = 1;
		} else {
			$dayOffset = 0;
		}
		...
		$timeObj->setTZbyId('UTC');
		if($dayOffset==0){
			$timeObj->subtractSeconds(86400);
		}
		return $timeObj;
	}
	
	function calculateEndWeekTime($dateObject='', $weekStartDay='monday'){
		$timeObj = tx_cal_calendar::calculateStartWeekTime($dateObject,$weekStartDay);
