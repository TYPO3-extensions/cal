...
class tx_cal_calendar {
	...
	function dateOfWeek($date, $day, $weekStartDay='Sunday') {
		...
		$num = gmdate('w', strtotime($weekStartDay)	+ strtotimeOffset());
		$start_day_time = strtotime(((gmdate('w',$date)==$num)==1 ? $weekStartDay : 'last '.$weekStartDay), $date)	+ strtotimeOffset();
		$ret_unixtime = strtotime($day,$start_day_time)	+ strtotimeOffset();
		// Fix for 992744
	...
	function calculateStartWeekTime($timestamp=''){
		return gmmktime(0, 0, 0, gmdate('m',$timestamp), gmdate('d',$timestamp)-gmdate('w',$timestamp)+1, gmdate('Y',$timestamp));
	}
	
	function calculateEndWeekTime($timestamp=''){
		return gmmktime(0, 0, 0, gmdate('m',$timestamp), gmdate('d',$timestamp)-gmdate('w',$timestamp)+7, gmdate('Y',$timestamp));
	}
