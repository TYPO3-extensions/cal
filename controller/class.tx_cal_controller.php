...
//debug("Start:".microtime());
		...
		if(!$this->link_vars['view'] && !$this->link_vars['getdate']){
			...
			$this->conf['cache']=0;
			$this->conf['clear_anyway'] = 0;
		}else{
			unset($this->piVars['no_cache']);
			$this->conf['cache']=1;
			$this->conf['clear_anyway'] = 0;
		}
		...
		if ($this->piVars['getdate'] == '') {
			...
			$this->conf['getdate'] = $this->piVars['getdate'];
		}
		$this->conf['view'] = $this->piVars['view'];
		$this->conf['lastview'] = $this->piVars['lastview'];
		$this->conf['uid'] = $this->piVars['uid'];
		$this->conf['type'] = $this->piVars['type'];
		$this->conf['category'] = $category;
		$this->conf['calendar'] = $calendar;
		$this->conf['monitor'] = $this->piVars['monitor'];
		$this->conf['gettime'] = $this->piVars['gettime'];
		$this->conf['preview'] = $this->piVars['preview'];
		$this->conf['page_id'] = $this->piVars['page_id'];
		$this->conf['option'] = $this->piVars['option'];
		$this->conf['switch_calendar'] = $this->piVars['switch_calendar'];
...
//debug($view);
		...
		// catch create_organizer view type and check all conditions
			...
			$this->conf['view'] = $this->conf["view."]['defaultView'][0];
			if ($this->conf['view'] == '') {
		...
		if(count($this->conf["view."]['defaultView'])==1){
			$this->conf['view'] = $this->conf["view."]['defaultView'][0];
			if($this->conf["view."]['defaultView']!="event"){
				$this->conf['uid'] = "";
				$this->piVars['uid']=null;
				$this->conf['type'] = "";
				$this->piVars['type']=null;
			}else if($this->conf["view."]['defaultView'][0]=="event" && (($this->piVars['view']=='location' && !in_array('location', $this->conf["view."]['defaultView'])) || ($this->piVars['view']=='organizer' && !in_array('organizer', $this->conf["view."]['defaultView'])))){
				return;
			}
		}else if(!($this->conf['view'] == 'admin'&& $this->rightsObj->isCalAdmin()) && !in_array($this->conf['view'], $this->conf["view."]['defaultView'])){
			$this->conf['view'] = $this->conf["view."]['defaultView'][0];
		}
		$return = "";
		$count = 0;
...
//		}
	...
	function updateConfWithFlexform(&$piFlexForm){
		...
		$this->updateIfNotEmpty($this->conf['view.']['defaultView'] , array_unique(split(',',$this->pi_getFFvalue($piFlexForm, 'viewmode'))));
