...
class tx_cal_controller extends tslib_pibase {
	...
	function getContent($notEmpty = true){
		...
		if($this->conf['view.'][$this->conf['view'].'.']['doNotWrapInBaseClass']){
			return $return;
		}
	...
	function initConfigs(){
		...
		$this->getDateTimeObject = new tx_cal_date($this->conf['getdate']);
	...
	//new Mode - category can be configurred
		...
		foreach((Array)$calendarArray as $calendar){
			$allCalendars[]=$calendar->getUid();
		}
	...
	function day() {
		...
		$timeObj = new tx_cal_date($this->conf['getdate']);
	...
	function getListViewTime($timeString, $timeObj='') {
		...
		switch($timeString) {
			case 'cal:yearstart':
				$newTime = tx_cal_calendar::calculateStartYearTime($timeObj);
				break;
			case 'cal:monthstart':
				$newTime = tx_cal_calendar::calculateStartMonthTime($timeObj);
				break;
			case 'cal:weekstart':
				$newTime = tx_cal_calendar::calculateStartWeekTime($timeObj, $weekStartDay);
				break;
			case 'cal:yesterday':
				$timeObj->subtractSeconds(24 * 60 * 60);
				$newTime = tx_cal_calendar::calculateStartDayTime($timeObj);
				break;
			case 'cal:today':
				$newTime = tx_cal_calendar::calculateStartDayTime($timeObj);
				break;
			case 'cal:tomorrow':
				$newTime = tx_cal_calendar::calculateEndDayTime($timeObj);
				break;
			case 'cal:weekend':
				$newTime = tx_cal_calendar::calculateEndWeekTime($timeObj, $weekStartDay);
				break;
			case 'cal:monthend':
				$newTime = tx_cal_calendar::calculateEndMonthTime($timeObj);
				break;
			case 'cal:yearend':
				$newTime = tx_cal_calendar::calculateEndYearTime($timeObj);
				break;
			default:
				require_once(t3lib_extMgm::extPath('cal').'controller/class.tx_cal_dateParser.php');
				$dp = new tx_cal_dateParser();
	...
	function searchEvent() {
		...
		if(!$start_day){
			...
			$start_day = new tx_cal_date(getYmdFromDateString($this->conf, $start_day));
			$start_day->setHour(0);
		...
		if(!$end_day){
			...
			$end_day = new tx_cal_date(getYmdFromDateString($this->conf, $end_day));
			$end_day->setHour(23);
		...
		$minStarttime=new tx_cal_date($this->conf['view.']['search.']['startRange']);
		$maxEndtime = new tx_cal_date($this->conf['view.']['search.']['endRange']);
	...
	function searchAll() {
		...
		$minStarttime = new Date(intval($this->conf['view.']['search.']['startRange']), 'Ymd');
		$maxEndtime = new Date(intval($this->conf['view.']['search.']['endRange']), 'Ymd');
	...
	function updateConfWithFlexform(&$piFlexForm){
		...
		$tsparser->parse($this->pi_getFFvalue($piFlexForm, 'myTS','s_TS_View'));
		$this->conf = $this->array_merge_recursive_unique($this->conf, $tsparser->setup);
	...
	// http://de.php.net/manual/de/function.array-merge-recursive.php#73843
