...
class tx_cal_controller extends tslib_pibase {
	...
	//new Mode - category can be configurred
		...
		foreach((Array)$calendarArray['tx_cal_calendar'] as $calendar){
	...
	function getListViewTime($timeString, $timeObj='') {
		...
		switch($timeString) {
			...
			case 'cal:monthstart':
				...
				break;
			case 'cal:weekstart':
				$newTime = tx_cal_calendar::calculateStartWeekTime('', $weekStartDay);
				break;
			case 'cal:yesterday':
			...
			case 'cal:tomorrow':
				...
				break;
			case 'cal:weekend':
				$newTime = tx_cal_calendar::calculateEndWeekTime('', $weekStartDay);
				break;
			case 'cal:monthend':
	...
	function updateConfWithFlexform(&$piFlexForm){
		...
		$this->conf = t3lib_div::array_merge_recursive_overrule($this->conf, $tsparser->setup,0,true);
