...
class tx_cal_controller extends tslib_pibase {
	...
	function initConfigs(){
		...
		if($this->conf['displayCurrentRecord']) {
			...
			$this->conf['view.']['allowedViews'] = array('event');
	...
	function searchAll() {
		...
		$modelObj = &tx_cal_registry::Registry('basic','modelcontroller');
		$list = array();
		if ($this->piVars['query'] && $this->conf['view.']['search.']['startSearchAfterSubmit']) {
	...
	function replace_tags($tags = array(), $page)
	{
		if (sizeof($tags) > 0)
		{
			...
			foreach ($tags as $tag => $data)
			{
				...
				$sims['###' . strtoupper($tag) . '###'] = substituteMarkerArrayNotCached($data,'###' . strtoupper($tag) . '###', array(),array());
			}
	...
	*/
	function finish(&$content) {
		...
		$hookObjectsArr = $controller->getHookObjectsArray('finishViewRendering');
