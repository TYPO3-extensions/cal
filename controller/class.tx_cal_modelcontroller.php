...
class tx_cal_modelcontroller extends tx_cal_base_controller {
	...
	function findEventsForList(&$startDateObject, &$endDateObject, $type='', $pidList='', $eventType='0,1,2,3') {
		return $this->findAllWithin('cal_event_model', $startDateObject, $endDateObject, $type, 'event', $pidList, $eventType);
	}
	 ...
	 */
	function findAllWithin($serviceName, &$startDateObject, &$endDateObject, $type='', $subtype='', $pidList='', $eventType='') {
//t3lib_div::debug('findallWithin Start:'.microtime());
		...
		if ($type == '') {		
			...
			while (is_object($service = t3lib_div::makeInstanceService($serviceName, $subtype, $serviceChain))) {
				...
				$eventsFromService = $service->findAllWithin($startDateObject, $endDateObject, $pidList, $eventType);
...
//debug('findallWithin end:'.microtime());
		...
		/* Operate on the provided key only */
			...
			if(!is_object($service)){
				return $this->findAllWithin($service, $startDateObject, $endDateObject, '', $subtype, $pidList, $eventType);
			}
			/* Get all events from the model as an array */
			$events = $service->findAllWithin($startDateObject, $endDateObject, $pidList, $eventType);
			ksort($events);
			$return = array();
			foreach($events as $key => $obj){
				ksort($obj);
