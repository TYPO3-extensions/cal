...
class tx_cal_modelcontroller extends tx_cal_base_controller {
	...
	function findEvent($uid, $type='', $pidList='', $showHiddenEvents=false, $showDeletedEvents=false, $getAllInstances=false) {
		if($uid==''){
			return;
		}
		$event = $this->find('cal_event_model', $uid, $type, 'event', $pidList, $showHiddenEvents, $showDeletedEvents, $getAllInstances);
		return $event;
	}
...
//debug('findallWithin end:'.microtime());
	 ...
	 */
	function find($serviceName, $uid, $type, $subtype, $pidList='', $showHiddenEvents=false, $showDeletedEvents=false, $getAllInstances=false) {
		/* Gets the model for the provided service key */
		$service = $this->getServiceObjByKey($serviceName, $subtype, $type);		
		/* Look up an event with a specific ID inside the model */
		$event = $service->find($uid, $pidList, $showHiddenEvents, $showDeletedEvents, $getAllInstances);
		return $event;
	}
