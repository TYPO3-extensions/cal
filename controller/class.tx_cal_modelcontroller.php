...
class tx_cal_modelcontroller extends tx_cal_base_controller {
	...
	function findEvent($uid, $type='', $pidList='', $showHiddenEvents=false, $showDeletedEvents=false, $getAllInstances=false, $disableCalendarSearchString=false) {
		if($uid==''){
			return;
		}
		$event = $this->find('cal_event_model', $uid, $type, 'event', $pidList, $showHiddenEvents, $showDeletedEvents, $getAllInstances, $disableCalendarSearchString);
		return $event;
	}
	...
	function findAllEventInstances($uid, $type='', $pidList='', $showHiddenEvents=false, $showDeletedEvents=false, $getAllInstances=false) {
		if($uid==''){
			return;
		}
		$event_s = $this->find('cal_event_model', $uid, $type, 'event', $pidList, $showHiddenEvents, $showDeletedEvents, $getAllInstances);
		return $event_s;
	}
	...
	function findEventsForDay(&$dateObject, $type='', $pidList='') {
	...
	function findEventsForWeek(&$dateObject, $type='', $pidList='') {
		$starttime = tx_cal_calendar::calculateStartWeekTime($dateObject,$this->conf['view.']['weekStartDay']);
		$endtime = tx_cal_calendar::calculateEndWeekTime($dateObject);
		return $this->findAllWithin('cal_event_model', $starttime, $endtime, $type, 'event', $pidList);
	}
	
	function findEventsForMonth(&$dateObject, $type='', $pidList='') {
		$starttime = tx_cal_calendar::calculateStartMonthTime($dateObject);
		$endtime = tx_cal_calendar::calculateEndMonthTime($dateObject);
		return $this->findAllWithin('cal_event_model', $starttime, $endtime, $type, 'event', $pidList);
	}
	
	function findEventsForYear(&$dateObject, $type='', $pidList='') {
		$starttime = tx_cal_calendar::calculateStartYearTime($dateObject);
		$endtime = tx_cal_calendar::calculateEndYearTime($dateObject);
		return $this->findAllWithin('cal_event_model', $starttime, $endtime, $type, 'event', $pidList);
	}
	
	function findEventsForList(&$startDateObject, &$endDateObject, $type='', $pidList='') {
		return $this->findAllWithin('cal_event_model', $startDateObject, $endDateObject, $type, 'event', $pidList);
	}
	...
	function searchEvents( $type, $pidList, &$startDateObject, &$endDateObject, $searchword, $locationIds, $organizerIds) {
		return $this->_searchEvents('cal_event_model', $type, $pidList, $startDateObject, $endDateObject, $searchword, $locationIds, $organizerIds);
	}
	 ...
	 */
	function findAllWithin($serviceName, &$startDateObject, &$endDateObject, $type='', $subtype='', $pidList='') {
//t3lib_div::debug('findallWithin Start:'.microtime());
		...
		if ($type == '') {		
			...
			while (is_object($service = t3lib_div::makeInstanceService($serviceName, $subtype, $serviceChain))) {
				...
				$eventsFromService = $service->findAllWithin($startDateObject, $endDateObject, $pidList);				
...
//debug('findallWithin end:'.microtime());
		...
		/* Operate on the provided key only */
			...
			if(!is_object($service)){
				return $this->findAllWithin($service, $startDateObject, $endDateObject, '', $subtype, $pidList);
			}
			/* Get all events from the model as an array */
			$events = $service->findAllWithin($startDateObject, $endDateObject, $pidList);
			ksort($events);
			$return = array();
			foreach($events as $key => $obj){
				ksort($obj);
	...
	function _searchEvents($serviceName,$type, $pidList, &$startDateObject, &$endDateObject, $searchword, $locationIds='', $organizerIds='') {
		...
		if ($type == '') {
			...
			while (is_object($service = t3lib_div::makeInstanceService($serviceName, 'event', $serviceChain))) {
				$serviceChain.=','.$service->getServiceKey();
				/* Gets all events from the current model as an array*/
				$eventsFromService = $service->search($pidList, $startDateObject, $endDateObject, $searchword, $locationIds, $organizerIds);
				if(!empty($eventsFromService)){
					if(empty($events)){
						$events = $eventsFromService;
					}else{
		...
		/* Operate on the provided key only */
			...
			$events = $service->search($pidList, $startDateObject, $endDateObject, $searchword, $locationIds, $organizerIds);
	 ...
	 */
	function find($serviceName, $uid, $type, $subtype, $pidList='', $showHiddenEvents=false, $showDeletedEvents=false, $getAllInstances=false, $disableCalendarSearchString=false) {
		/* Gets the model for the provided service key */
		$service = $this->getServiceObjByKey($serviceName, $subtype, $type);		
		/* Look up an event with a specific ID inside the model */
		$event = $service->find($uid, $pidList, $showHiddenEvents, $showDeletedEvents, $getAllInstances, $disableCalendarSearchString);
		return $event;
	}
