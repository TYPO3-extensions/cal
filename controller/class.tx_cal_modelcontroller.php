...
class tx_cal_modelcontroller extends tx_cal_base_controller {
	...
	function findEventsForList($starttime, $endtime, $type='', $pidList='') {
		return $this->findAllWithin('cal_event_model', $starttime, $endtime, $type, 'event', $pidList);
	}
	...
	function findEventsForRss( $starttime, $endtime, $type='', $pidList) {
		return $this->findAllWithin('cal_event_model', $starttime, $endtime, $type, 'event', $pidList);
	}
	
	function searchEvents( $type='', $pidList='', $starttime, $endtime, $searchword, $locationIds) {
		return $this->_searchEvents('cal_event_model', $type, $pidList, $starttime, $endtime, $searchword, $locationIds);
	}
	...
	function findAllCategories($serviceName, $type, $pidList) {
		...
		if ($type == '') {
			...
			while (is_object($service = t3lib_div::makeInstanceService($serviceName, 'category', $serviceChain))) {
				...
				$categories[$service->getServiceKey()] = $service->findAll($pidList, $categoryArrayToBeFilled);
				$serviceChain.=','.$service->getServiceKey();
			}
		}else{
			/* Gets the model for the provided service key */
		...
		return $categoryArrayToBeFilled;
	}
	
	function _searchEvents($serviceName='',$type='', $pidList='', $starttime, $endtime, $searchword, $locationIds) {
		...
		if ($type == '') {
			...
			while (is_object($service = t3lib_div::makeInstanceService($serviceName, 'event', $serviceChain))) {
				$serviceChain.=','.$service->getServiceKey();
				/* Gets all events from the current model as an array*/
				$eventsFromService = $service->search($pidList, $starttime, $endtime, $searchword, $locationIds);
				if(count($eventsFromService)>0){
					if(count($events)==0){
						$events = $eventsFromService;
					}else{
		...
		/* Operate on the provided key only */
			...
			$events = $service->search($pidList, $starttime, $endtime, $searchword, $locationIds);
