...
class tx_cal_reminder_cron extends tx_gabriel_event {
	...
	function execute() {
		...
		$calAPI = $tx_cal_api->tx_cal_api_without($event['pid']);
		
		$eventObject = $calAPI->findEvent($event['uid'], 'tx_cal_phpicalendar', $event['pid']);
		$eventObject->conf['view'] = 'event';
		$pageTSConf = t3lib_befunc::getPagesTSconfig($event['pid']);
		
		$reminderService = &getReminderService();
		$reminderService->remind($eventObject , $pageTSConf['options.']['tx_cal_controller.']['view.']['event.']['remind.']);
	}
