	...
	function processCmdmap_postProcess(&$command, &$table, &$id, &$value, &$this) {
		if ($table == 'tx_cal_event') {
			...
			while ($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($result)) {
				...
				if($command=='delete'){
					$notificationService->notifyOfChanges($row, array('deleted' => $value), $pageTSConf['options.']['tx_cal_controller.']['event.']['notify.']);	
				}else{
					$notificationService->notifyOfChanges($row, array($command => $value), $pageTSConf['options.']['tx_cal_controller.']['event.']['notify.']);
				}
			}
