...
class tx_cal_tcemain_processdatamap {
	function processDatamap_postProcessFieldArray($status, $table, $id, &$fieldArray, &$this) {
		if ($table == 'tx_cal_event' && count($fieldArray)>1) {
			...
			notifyOfChanges($row, $fieldArray);
   		} 
	}
	
	function processDatamap_preProcessFieldArray(&$incommingFieldArray, $table, $id, &$this) {
		if ($table == 'tx_cal_event' && $incommingFieldArray['relation_cnt'] == "") {
			$ffConf = tx_cal_tcemain_processdatamap::getFFConf($GLOBALS['HTTP_POST_VARS']['popViewId']);
			$anonymousUserUid = $ffConf['data']['sDEF']['lDEF']['anonymousUserId']['vDEF'];
			if(!is_numeric($anonymousUserUid)){
				$pageTSConf = t3lib_befunc::getPagesTSconfig($GLOBALS['HTTP_POST_VARS']['popViewId']);
				if(is_array($pageTSConf)){
					$anonymousUseUid = $pageTSConf['options.']['anonymousUserUid.']['tx_cal_controller'];
				}	
			}
			$incommingFieldArray['relation_cnt'] = "fe_users_".$anonymousUserUid;
		}  else if($table == 'tx_cal_category' && array_key_exists("type",$incommingFieldArray)){
   			$row = t3lib_BEfunc::getRecord ("tx_cal_category", $id);
   			// Here we have to check if the category belongs to the type
   			...
   			switch ($incommingFieldArray['type']){
   				...
   				case 0:
   					...
   					$where = " category_id=".$row['uid']." AND isTemp=1 AND pid=".$row['pid'];
    ...
    */
    function getFFConf($pid) {
		...
		$tables = 'tt_content';
		$where = 'tt_content.list_type="cal_controller" AND tt_content.deleted=0 AND tt_content.pages='.$pid;
		...
		if (!$row) {
			...
			$where = 'tt_content.list_type="cal_controller" AND tt_content.deleted=0 AND pages.storage_pid='.$pid;
			$result = $GLOBALS['TYPO3_DB']->exec_SELECTquery($fields,$tables,$where);
			$row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($result);
		}
	 ...
	 */
	function insertCalEventsIntoDB($iCalendarComponentArray=array(), $catId, $pid="", $cruserId=""){
		...
		foreach($iCalendarComponentArray as $component){
			...
			if (is_a($component,"tx_iCalendar_vevent")){
				...
				$insertFields['category_id'] = $catId;
...
//								$byhour = split (',', $byhour);
							...
							case 'BYDAY':
								$byday = $val;
								$byday = split (',', $byday);
								$insertFields['byday'] = $byday;
								break;
							case 'BYMONTHDAY':
								$bymonthday = $val;
								$bymonthday = split (',', $bymonthday);
								$insertFields['bymonthday'] = $bymonthday;
								break;					
							case 'BYYEARDAY':
//								$byyearday = $val;
