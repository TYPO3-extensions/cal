...
class tx_cal_cache {
	...
	function set($hash, $content, $ident, $lifetime=0) {
		...
		if ($this->cachingEngine=='cachingFramework' ) {
			$this->tx_cal_cache->set($hash, $content, array('ident_' . $ident),$lifetime);
			...
			$table = 'tx_cal_cache';
			$fields_values = array('identifier' => $hash, 'content' => $content, 'crdate' => $GLOBALS['EXEC_TIME'], 'lifetime' => $lifetime);
