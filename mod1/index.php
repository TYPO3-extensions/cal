		...
		switch(intval($this->MOD_SETTINGS['function']))	{
			case 2:
				...
				if(!empty($pageIds) && is_int($starttime) && is_int($endtime)){
					...
					$results = $GLOBALS['TYPO3_DB']->exec_SELECTquery('pid','tx_cal_event',$where,'pid');
					if($results) {
					...
					$results = $GLOBALS['TYPO3_DB']->exec_SELECTquery('pid','tx_cal_exception_event',$where,'pid');
					if($results) {
						while ($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($results)){
							$pids[] = $row['pid'];
						}
					...
					$pageTitle = Array();
					$results = $GLOBALS['TYPO3_DB']->exec_SELECTquery('uid,title','pages','uid in ('.implode(',',$pids).')');
					if($results) {
						while ($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($results)){
							$pageTitle[$row['uid']] = $row['title'];
						}
						$GLOBALS['TYPO3_DB']->sql_free_result($results);
					}
					$selectFieldIds = Array();
					$content.='<table><thead><tr><th>'.$GLOBALS['LANG']->getLL('tableHeader1').'</th><th>'.$GLOBALS['LANG']->getLL('tableHeader2').'</th></tr></thead><tbody>';
					foreach($pageTitle as $pid => $title){
						$content.='<tr><td>';
