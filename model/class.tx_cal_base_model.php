...
class tx_cal_base_model {
	...
	function getImageMarker(& $template, & $sims, & $rems, & $wrapped, $view){
		...
		if($this->conf['view.'][$view.'.'][$this->objectType.'.']['image.']['imageCount'] < count($this->getImage())){
...
/**
	...
	function getDescriptionImageMarker(&$template, &$sims, &$rems, & $wrapped, $view){
		unset($this->conf['view.'][$view.'.'][$this->objectType.'.']['descriptionImage.']['imgList.']);
		if(!$this->getImage()){
			$sims['###DESCRIPTION_IMAGE###'] = $this->getDescriptionMarker($template, $sims, $rems, $wrapped, $view);
			return;
		}else{
			$this->conf['view.'][$view.'.'][$this->objectType.'.']['descriptionImage.']['imgList'] = implode(',',$this->getImage());
		}
		t3lib_div::loadTCA('tx_cal_'.$this->objectType);
		$this->conf['view.'][$view.'.'][$this->objectType.'.']['descriptionImage.']['imgPath'] = $GLOBALS['TCA']['tx_cal_'.$this->objectType]['columns']['image']['config']['uploadfolder'].'/';
		if($this->conf['view.'][$view.'.'][$this->objectType.'.']['descriptionImage.']['overridePath']){
			$this->conf['view.'][$view.'.'][$this->objectType.'.']['descriptionImage.']['imgPath'] = $this->conf['view.'][$view.'.'][$this->objectType.'.']['image.']['overridePath'];
		}
		$cObj = &tx_cal_registry::Registry('basic','cobj');
		$remLayout = $cObj->data['layout'];
		$cObj->data['layout'] = $this->conf['view.'][$view.'.'][$this->objectType.'.']['descriptionImage.']['layout'];
		$remCaption = $cObj->data['imagecaption'];
		if($this->getImageCaption()){
			$cObj->data['imagecaption'] = implode(chr(10),$this->getImageCaption());
		}
		...
		if($this->getImageAltText()){
			$this->conf['view.'][$view.'.'][$this->objectType.'.']['descriptionImage.']['1.']['altText'] = implode(chr(10),$this->getImageAltText());
		}
		if($this->getImageTitleText()){
			$this->conf['view.'][$view.'.'][$this->objectType.'.']['descriptionImage.']['1.']['titleText'] = implode(chr(10),$this->getImageTitleText());
		}
		$this->conf['view.'][$view.'.'][$this->objectType.'.']['descriptionImage.']['text.']['20.']['value'] = $this->getRTEParsedContent($this->getDescription());
		$sims['###DESCRIPTION_IMAGE###'] = $cObj->cObjGetSingle($this->conf['view.'][$view.'.'][$this->objectType.'.']['descriptionImage'], $this->conf['view.'][$view.'.'][$this->objectType.'.']['descriptionImage.']);
	}
