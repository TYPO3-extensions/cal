	...
	function getValuesAsArray() {
		...
		if(!is_array($this->cachedValueArray) || (is_array($this->cachedValueArray) && !count($this->cachedValueArray))) {
			...
			if ($cachedValues != '') {
				...
				$this->controller->cache->set($storeKey, serialize(Array($this->classMethodVars,$autoFetchTextFields,$autoFetchTextSplitValue)), __FUNCTION__);
			}
					...
					if($markerArray[0]=='') {
						...
						foreach($moduleMarker as $key=>$val) {
							if ($this->conf[$base.'.'][$view.'.'][$this->getObjectType().'.']['module__'.strtolower($themodule).'___'.strtolower($key)]) {
								...
								$sims['###MODULE__'.$themodule.'___'.strtoupper($key).'###'] = 
								$this->local_cObj->cObjGetSingle($this->conf[$base.'.'][$view.'.'][$this->getObjectType().'.']['module__'.strtolower($themodule).'___'.strtolower($key)],$this->conf[$base.'.'][$view.'.'][$this->getObjectType().'.']['module__'.strtolower($themodule).'___'.strtolower($key).'.']);
							} else {
