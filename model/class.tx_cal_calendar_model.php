...
class tx_cal_calendar_model extends tx_cal_base_model {
	...
	function getEditLink(& $template, & $sims, & $rems, $view){
		...
		if ($this->isUserAllowedToEdit()) {
			...
			$this->local_cObj->setCurrentVal($controller->pi_getLL('l_edit_calendar'));
		...
		if ($this->isUserAllowedToDelete()) {
			$this->initLocalCObject($this->getValuesAsArray());
			$this->local_cObj->setCurrentVal($controller->pi_getLL('l_delete_calendar'));
			$controller = &tx_cal_registry::Registry('basic','controller');
	...
	function getValuesAsArray() {
		...
		$values['activate_fnb'] = $this->getActivateFreeAndBusy();
		$values['type'] = $this->getCalendarType();
		$values['ext_url'] = $this->getExtUrl();
		$values['ics_file'] = $this->getIcsFile();
		$values['refresh'] = $this->getRefresh();
		$values['md5'] = $this->getMD5();
		return $values;
	}
