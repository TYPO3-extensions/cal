...
class tx_cal_category_model extends tx_cal_base_model {
	...
	function getEditLink(& $template, & $sims, & $rems, $view){
		...
		if ($this->isUserAllowedToEdit()) {
			...
			$this->local_cObj->setCurrentVal($controller->pi_getLL('l_edit_category'));
			$controller = &tx_cal_registry::Registry('basic','controller');
			#$GLOBALS['TSFE']->ATagParams = 'title="' . $controller->pi_getLL('l_edit_category') . '" alt="' . $controller->pi_getLL('l_edit_category') . '"';
			$this->local_cObj->data['link'] = $controller->pi_linkTP_keepPIvars_url(array (
				'view' => 'edit_category',
			'type' => $this->getType(), 'uid' => $this->getUid(),'lastview' => $controller->extendLastView()), $this->conf['cache'], $this->conf['clear_anyway'], $this->conf['view.']['calendar.']['editCategoryViewPid']);
		...
		if ($this->isUserAllowedToDelete()) {
			$this->initLocalCObject($this->getValuesAsArray());
			$this->local_cObj->setCurrentVal($controller->pi_getLL('l_delete_category'));
			$controller = &tx_cal_registry::Registry('basic','controller');
			#$GLOBALS['TSFE']->ATagParams = 'title="' . $controller->pi_getLL('l_delete_category') . '" alt="' . $controller->pi_getLL('l_delete_category') . '"';
			$this->local_cObj->data['link'] = $controller->pi_linkTP_keepPIvars_url(array (
				'view' => 'delete_category',
			'type' => $this->getType(), 'uid' => $this->getUid(), 'lastview' => $controller->extendLastView()), $this->conf['cache'], $this->conf['clear_anyway'], $this->conf['view.']['category.']['deleteCategoryViewPid']);
