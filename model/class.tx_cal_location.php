...
class tx_cal_location extends tx_cal_location_model {
	...
	function tx_cal_location($row, $pidList){
		...
		$this->tx_cal_location_model($controller, $this->getType());
	...
	function getEditLink(& $template, & $sims, & $rems, $view){
		...
		if ($this->isUserAllowedToEdit()) {
			...
			$this->local_cObj->setCurrentVal($controller->pi_getLL('l_edit_location'));
			$controller = &tx_cal_registry::Registry('basic','controller');
			#$GLOBALS['TSFE']->ATagParams = 'title="' . $controller->pi_getLL('l_edit_location') . '" alt="' . $controller->pi_getLL('l_edit_location') . '"';
			$this->local_cObj->data['link'] = $controller->pi_linkTP_keepPIvars_url(array (
				'view' => 'edit_location',
			'type' => $this->getType(), 'uid' => $this->getUid(),'lastview' => $controller->extendLastView()), $this->conf['cache'], $this->conf['clear_anyway'], $this->conf['view.']['calendar.']['editLocationViewPid']);
		...
		if ($this->isUserAllowedToDelete()) {
			$this->initLocalCObject($this->getValuesAsArray());
			$this->local_cObj->setCurrentVal($controller->pi_getLL('l_delete_location'));
			$controller = &tx_cal_registry::Registry('basic','controller');
			#$GLOBALS['TSFE']->ATagParams = 'title="' . $controller->pi_getLL('l_delete_location') . '" alt="' . $controller->pi_getLL('l_delete_location') . '"';
			$this->local_cObj->data['link'] = $controller->pi_linkTP_keepPIvars_url(array (
				'view' => 'delete_location',
			'type' => $this->getType(), 'uid' => $this->getUid(), 'lastview' => $controller->extendLastView()), $this->conf['cache'], $this->conf['clear_anyway'], $this->conf['view.']['location.']['deleteLocationViewPid']);
