 ...
 * @subpackage cal
 */
class tx_cal_location extends t3lib_svbase {
	 ...
	 function createLocation($row){
	 	$location = t3lib_div::makeInstance(get_class($this));
	 	$location->setUid($row['uid']);
		$location->setName($row['name']);
		$location->setDescription($row['title']);
		$location->setStreet($row['address']);
		$location->setZip($row['zip']);
		$location->setCity($row['city']);
		$location->setPhone($row['phone']);
		$location->setEmail($row['email']);
		$location->setImage($row['image']);
		$location->setLink($row['www']);
		return $location;
	 }
	 
	 function renderLocation($cObj){
	 	$this->cObj = $cObj; // cObj
	...
	function retrievePostData(&$insertFields){
		...
		if($this->rightsObj->isAllowedToEditLocationStreet() || $this->rightsObj->isAllowedToCreateLocationStreet()){
			$insertFields['street'] = $GLOBALS['HTTP_POST_VARS']['street'];
		}
