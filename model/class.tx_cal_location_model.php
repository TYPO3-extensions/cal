...
class tx_cal_location_model extends tx_cal_base_model{
	...
	function getCountryMarker(& $template, & $sims, & $rems, $view){
		...
		if(t3lib_extMgm::isLoaded('static_info_tables')) {
			...
			$sims['###COUNTRY###'] = $this->local_cObj->cObjGetSingle($this->conf['view.'][$this->conf['view'].'.'][$this->objectType.'.']['countryStaticInfo'], $this->conf['view.'][$this->conf['view'].'.'][$this->objectType.'.']['countryStaticInfo.']);
		} else {
			$current = $this->getCountry();
			$this->local_cObj->setCurrentVal($current);
			$sims['###COUNTRY###'] = $this->local_cObj->cObjGetSingle($this->conf['view.'][$this->conf['view'].'.'][$this->objectType.'.']['country'], $this->conf['view.'][$this->conf['view'].'.'][$this->objectType.'.']['country.']);
		}
	}
	...
	function getCountryZoneMarker(& $template, & $sims, & $rems, $view){
		...
		if(t3lib_extMgm::isLoaded('static_info_tables')) {
			...
			$staticInfo = t3lib_div::makeInstance('tx_staticinfotables_pi1');
			$staticInfo->init();
			$current = $staticInfo->getStaticInfoName('SUBDIVISIONS', $this->getCountryzone());
			$this->local_cObj->setCurrentVal($current);
			$sims['###COUNTRYZONE###'] = $this->local_cObj->cObjGetSingle($this->conf['view.'][$this->conf['view'].'.'][$this->objectType.'.']['countryzoneStaticInfo'], $this->conf['view.'][$this->conf['view'].'.'][$this->objectType.'.']['countryzoneStaticInfo.']);
		} else {
			$current = $this->getCountryzone();
			$this->local_cObj->setCurrentVal($current);
			$sims['###COUNTRYZONE###'] = $this->local_cObj->cObjGetSingle($this->conf['view.'][$this->conf['view'].'.'][$this->objectType.'.']['countryzone'], $this->conf['view.'][$this->conf['view'].'.'][$this->objectType.'.']['countryzone.']);
		}
	}
	...
	function getEditLinkMarker(& $template, & $sims, & $rems, $view){
		...
		if ($this->isUserAllowedToEdit()) {
			...
			$this->local_cObj->setCurrentVal($this->conf['view.'][$this->conf['view'].'.'][$this->objectType.'.']['editIcon']);#$controller->pi_getLL('l_edit_'.$this->objectType)
			$sims['###EDIT_LINK###'] = $this->local_cObj->cObjGetSingle($this->conf['view.'][$this->conf['view'].'.'][$this->objectType.'.']['editLink'],$this->conf['view.'][$this->conf['view'].'.'][$this->objectType.'.']['editLink.']);
		}
		if ($this->isUserAllowedToDelete()) {
			...
			$this->initLocalCObject($linkConf);
			$this->local_cObj->setCurrentVal($this->conf['view.'][$this->conf['view'].'.'][$this->objectType.'.']['deleteIcon']);#$controller->pi_getLL('l_delete_'.$this->objectType)
			$sims['###EDIT_LINK###'] .= $this->local_cObj->cObjGetSingle($this->conf['view.'][$this->conf['view'].'.'][$this->objectType.'.']['deleteLink'],$this->conf['view.'][$this->conf['view'].'.'][$this->objectType.'.']['deleteLink.']);
		}
	}
	...
	function getEventLinkMarker(& $template, & $sims, & $rems, & $wrapped, $view){
		...
		if($this->conf['view.'][$view.'.'][$this->objectType.'.']['includeEventsInResult']){
			...
			$sims['###EVENT_LINK###'] = $this->local_cObj->cObjGetSingle($this->conf['view.'][$view.'.'][$this->objectType.'.']['eventLink'],$this->conf['view.'][$view.'.'][$this->objectType.'.']['eventLink.']);
		}
	}
