	...
	function setHidden($hidden) {
	 ...
	 function getCountryMarker(&$page, &$rems, &$sims){
	 	if($this->getCountry()!=''){
			...
			$countrysims['###COUNTRY###'] = $this->getCountry();
			$countrysims['###COUNTRY_LABEL###'] = $this->controller->pi_getLL('l_location_country');
			$rems['###COUNTRY_DATA_LOOP###'] = $this->cObj->substituteMarkerArrayCached($loop, $countrysims, array(), array ());	
		}else{
