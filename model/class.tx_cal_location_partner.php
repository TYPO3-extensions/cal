...
class tx_cal_location_partner extends tx_cal_location {
	...
	function renderLocation(){
		...
		$page = $cObj->fileResource($this->conf['view.']['location.']['locationModelTemplate4Partner']);
		if ($page=='') {
			return '<h3>calendar: no location model template file found:</h3>'.$this->conf['view.']['location.']['locationModelTemplate4Partner'];
		}
		$rems = array();
		$sims = array();
		$wrapped = array();
