...
class tx_cal_location_partner extends tx_cal_location {
	  ...
	  * @param	string		$pidList	The pid-list to search in
 	  */
 	 function tx_cal_location_partner($controller, $uid, $pidList){
		...
		$this->tx_cal_location($controller, $this->type);
	...
	function getCountryZone() {
		return $this->partner->data['admin_area'];
	}
	
	function setCountryZone($t) {
		$this->partner->data['admin_area'] = $t;
	}
	
	function getCountry() {
		return $this->partner->data['country'];
	}
	
	function setCountry($t) {
		$this->partner->data['country'] = $t;
	}
	...
	function setType($type) {
	 ...
	 function renderLocation(){
		...
		$page = $this->cObj->fileResource($this->conf['view.']['location.']['locationTemplate4partner']);
		if ($page=='') {
		...
		if($this->getCountryZone()!=''){
			$loop = $this->cObj->getSubpart($locationloop, '###COUNTRYZONE_DATA_LOOP###');
			$sims['###COUNTRYZONE###'] = $this->getCity();
			$sims['###COUNTRYZONE_LABEL###'] = $LANG->getLL('tx_partner_main.admin_area');				
		}
		if($this->getCountry()!=''){
			$loop = $this->cObj->getSubpart($locationloop, '###COUNTRY_DATA_LOOP###');
			$sims['###COUNTRY###'] = $this->getCity();
			$sims['###COUNTRY_LABEL###'] = $LANG->getLL('tx_partner_main.country');				
		}
