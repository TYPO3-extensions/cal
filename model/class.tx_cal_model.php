...
class tx_cal_model extends tx_cal_base_model {
	...
	function setStarttime($t) {
		if($t>0){
			$this->starthour = gmmktime(gmdate('H',$t),gmdate('i',$t),0,0,0,1) - gmmktime(0,0,0,0,0,1);
			$this->startdate = gmmktime(0,0,0,gmdate('m',$t),gmdate('d',$t),gmdate('Y',$t));
			$this->starttime = $t;
		}
	}
	...
	function setEndtime($time) {
		if($time>0){
			$this->endhour = gmmktime(gmdate('H',$time),gmdate('i',$time),0,0,0,1) - gmmktime(0,0,0,0,0,1);
			$this->enddate = gmmktime(0,0,0,gmdate('m',$time),gmdate('d',$time),gmdate('Y',$time));
			$this->endtime = $time;
		}
	}
	...
	function isSharedUser($userId){
	 ...
	 function getValuesAsArray(){
		return array(
			...
			'attachment'=>t3lib_div::csvValues($this->attachment),
		);
	}
	...
	function getCategoriesAsString($asLink = true){
		...
		if(!empty($this->categories)){
			foreach($this->getCategories() as $categoryArray){
				if(in_array($categoryArray['uid'], $rememberCats)){
					continue;
				}
				$rememberCats[] = $categoryArray['uid'];
