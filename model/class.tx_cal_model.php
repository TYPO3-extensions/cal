	...
	function getCategoriesAsString($asLink = true){
		...
		if(!empty($this->categories)){
			foreach($this->categories as $categoryObject){
				...
				$GLOBALS['TSFE']->ATagParams = 'title="'.$categoryObject->getTitle().'"'.($headerstyle!='' ? ' class="'.$headerstyle.'"':'');
				$parameter['category']=$categoryObject->getUid();
				
                $categoryTitle=$cObj->stdWrap($categoryObject->getTitle(),$this->conf['view.'][$this->conf['view'].'.']['categoryLink_stdWrap.']);
                
				if($asLink){
					$controller = &tx_cal_registry::Registry('basic','controller');
					$this->categoriesAsString[]= $controller->pi_linkTP_keepPIvars($categoryTitle, $parameter, $this->conf['cache'], $this->conf['clear_anyway']);
				}else{
		...
		return implode($cObj->cObjGetSingle($this->conf['view.'][$this->conf['view'].'.']['categoryLink_splitChar'],$this->conf['view.'][$this->conf['view'].'.']['categoryLink_splitChar.']),$this->categoriesAsString);
	}
