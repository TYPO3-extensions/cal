...
require_once(t3lib_extMgm::extPath('cal').'model/class.tx_cal_baseobject.php');
 ...
 * @subpackage cal
 */
class tx_cal_model extends tx_cal_baseobject {
	...
	function tx_cal_model(&$cObj, &$rightsObj, $serviceKey){
		$this->tx_cal_baseobject($cObj, $rightsObj, $serviceKey);
	}
...
//		$parameter = $this->shared->getLinkVarsForLink($this->prefixId);
		foreach($this->getCategories() as $categoryArray){
			...
			if($first){
				if($asLink){
					$this->categoriesAsString .= $this->controller->pi_linkTP_keepPIvars($categoryArray['title'], $parameter, $this->cObj->conf['cache'], $this->cObj->conf['clear_anyway']);
				}else{
	...
	function cloneEvent(){
		...
		$event = new $tx_cal_model ($this->cObj,$this->rightsObj, $this->getType());
	...
	function getImageMarkers(&$markerArray, $lConf, $isSingleView=false) {
		...
		if ($this->cObj->conf['FFimgH'] || $this->cObj->conf['FFimgW']) {
			$lConf['image.']['file.']['maxW'] = $this->cObj->conf['FFimgW'];
			$lConf['image.']['file.']['maxH'] = $this->cObj->conf['FFimgH'];
		}

		if ($this->cObj->conf['imageMarkerFunc']) {
			...
			if (count($imgs) > 1 && $this->cObj->conf['firstImageIsPreview'] && $isSingleView) {
				array_shift($imgs);
			...
			if ($this->piVars[$this->cObj->conf['singleViewPointerName']]) {
				$spage = $this->piVars[$this->cObj->conf['singleViewPointerName']];
	...
	function userProcess($mConfKey, $passVar) {
		if ($this->cObj->conf[$mConfKey]) {
			$funcConf = $this->cObj->conf[$mConfKey . '.'];
			$funcConf['parentObj'] = & $this;
			$passVar = $GLOBALS['TSFE']->cObj->callUserFunction($this->cObj->conf[$mConfKey], $funcConf, $passVar);
		}
		return $passVar;
	}
