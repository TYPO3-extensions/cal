...
class tx_cal_organizer extends tx_cal_location_model {
	  ...
	  * @param	string		$pidList	The pid-list to search in
 	  */
 	 function tx_cal_organizer($controller, $row, $pidList){	 	
	...
	function search($pidList, $searchword){
		...
		$organizer = array();
		if($searchword!=''){
			$result = $GLOBALS['TYPO3_DB']->exec_SELECTquery('*', 'tt_address', ' pid IN ('.$pidList.') '.$this->cObj->enableFields('tt_address').$this->searchWhere($searchword));
	...
	function isUserAllowedToEdit($feUserUid = '', $feGroupsArray = array ()) {
		...
		return $this->rightsObj->isAllowedToEditOrganizer();
	}
	...
	function isUserAllowedToDelete($feUserUid = '', $feGroupsArray = array ()) {
		...
		return $this->rightsObj->isAllowedToDeleteOrganizer();
	}
}
