...
class tx_cal_organizer extends tx_cal_location_model {
	...
	function getEditLink(& $template, & $sims, & $rems, $view){
		...
		if ($this->isUserAllowedToEdit()) {
			...
			$this->local_cObj->setCurrentVal($controller->pi_getLL('l_edit_organizer'));
			$controller = &tx_cal_registry::Registry('basic','controller');
			#$GLOBALS['TSFE']->ATagParams = 'title="' . $controller->pi_getLL('l_edit_organizer') . '" alt="' . $controller->pi_getLL('l_edit_organizer') . '"';
			$this->local_cObj->data['link'] = $controller->pi_linkTP_keepPIvars_url(array (
				'view' => 'edit_organizer',
			'type' => $this->getType(), 'uid' => $this->getUid(),'lastview' => $controller->extendLastView()), $this->conf['cache'], $this->conf['clear_anyway'], $this->conf['view.']['calendar.']['editOrganizerViewPid']);
		...
		if ($this->isUserAllowedToDelete()) {
			$this->initLocalCObject($this->getValuesAsArray());
			$this->local_cObj->setCurrentVal($controller->pi_getLL('l_delete_organizer'));
			$controller = &tx_cal_registry::Registry('basic','controller');
			#$GLOBALS['TSFE']->ATagParams = 'title="' . $controller->pi_getLL('l_delete_organizer') . '" alt="' . $controller->pi_getLL('l_delete_organizer') . '"';
			$this->local_cObj->data['link'] = $controller->pi_linkTP_keepPIvars_url(array (
				'view' => 'delete_organizer',
			'type' => $this->getType(), 'uid' => $this->getUid(), 'lastview' => $controller->extendLastView()), $this->conf['cache'], $this->conf['clear_anyway'], $this->conf['view.']['organizer.']['deleteOrganizerViewPid']);
