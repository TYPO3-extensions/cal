...
class tx_cal_organizer extends tx_cal_location_model {
	 ...
	 function renderOrganizer(){
		...
		if($email!=""){
			...
			$phonerems['###EMAIL_LOOP###'] = $this->cObj->parseFunc("<link ".$email.">".$email."</link>",$this->conf["parseFunc."]);
			$loop = $this->cObj->substituteMarkerArrayCached($loop, array (), $phonerems, array ());
		...
		if($link!=""){
			...
			$phonerems['###LINK_LOOP###'] = $this->cObj->parseFunc("<link ".$link.">".$link."</link>",$this->conf["parseFunc."]);
			$loop = $this->cObj->substituteMarkerArrayCached($loop, array (), $phonerems, array ());
