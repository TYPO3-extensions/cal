...
class tx_cal_organizer extends tx_cal_location_model {
	...
	function renderOrganizer(){
		...
		$page = $cObj->fileResource($this->conf['view.']['organizer.']['organizerModelTemplate']);
		if ($page=='') {
			return '<h3>calendar: no organizer model template file found:</h3>'.$this->conf['view.']['organizer.']['organizerModelTemplate'];
		}
		$rems = array();
		$sims = array();
		$wrapped = array();
