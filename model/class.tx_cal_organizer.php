...
class tx_cal_organizer extends tx_cal_location_model {
	  ...
	  * @param	string		$pidList	The pid-list to search in
 	  */
 	 function tx_cal_organizer(&$cObj, &$rightsObj, $row, $pidList){	 	
	 ...
	 function renderOrganizer(){
		...
		$page = $this->cObj->fileResource($this->cObj->conf["view."]["organizer."]["organizerTemplate"]);
		if ($page=="") {
			return "<h3>calendar: no organizer template file found:</h3>".$this->cObj->conf["view."]["organizer."]["organizerTemplate"];
		}
		$rems = array();
		$sims = array();
		...
		if($mobilephone!=""){
			...
			$sims['mobilephone_label'] = $this->shared->lang('l_organizer_mobilephone');	
		}else{
		...
		if($fax!=""){
			...
			$sims['fax_label'] = $this->shared->lang('l_organizer_fax');	
		}else{
			$rems['###FAX_DATA_LOOP###'] = "";
		}
		if($email!=""){
			$loop = $this->cObj->getSubpart($organizerloop, "###EMAIL_DATA_LOOP###");
			$phonerems['###EMAIL_LOOP###'] = $this->cObj->parseFunc("<link ".$email.">".$email."</link>",$this->cObj->conf["parseFunc."]);
			$loop = $this->cObj->substituteMarkerArrayCached($loop, array (), $phonerems, array ());
		...
		if($link!=""){
			...
			$phonerems['###LINK_LOOP###'] = $this->cObj->parseFunc("<link ".$link.">".$link."</link>",$this->cObj->conf["parseFunc."]);
			$loop = $this->cObj->substituteMarkerArrayCached($loop, array (), $phonerems, array ());
		...
		$sims["heading"] = $this->shared->lang('l_event_organizer');
...
//debug($sims["editlink"]);		
	...
	function searchWhere($sw) {
		$where = $this->cObj->searchWhere($sw, $this->cObj->conf["view."]["search."]["searchOrganizerFieldList"], 'tt_address');
		return $where;
	}
