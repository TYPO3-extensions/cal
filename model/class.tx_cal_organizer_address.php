...
class tx_cal_organizer_address extends tx_cal_location_model {
	  ...
	  * @param	string		$pidList	The pid-list to search in
 	  */
 	 function tx_cal_organizer_address(&$cObj, &$rightsObj, $row, $pidlist){
	 ...
	 function renderOrganizer(){
		...
		$page = $this->cObj->fileResource($this->cObj->conf["view."]["organizer."]["organizerTemplate4Address"]);
		if ($page=="") {
			return "<h3>calendar: no organizer template file found:</h3>".$this->cObj->conf["view."]["organizer."]["organizerTemplate4Address"];
		}
		$rems = array();
		$sims = array();
...
//debug($sims["editlink"]);		
	...
	function removeOrganizer(&$rightsObj, &$conf, $uid){
		$this->rightsObj = $rightsObj;		
		if($rightsObj->isAllowedToDeleteOrganizers()){
