...
class tx_cal_organizer_address extends tx_cal_location_model {
	 ...
	 function renderOrganizer(){
		...
		$page = $cObj->fileResource($this->conf['view.']['organizer.']['organizerModelTemplate4Address']);
		if ($page=='') {
			return '<h3>calendar: no organizer model template file found:</h3>'.$this->conf['view.']['organizer.']['organizerModelTemplate4Address'];
		}
		$rems = array();
		$sims = array();
		$wrapped = array();
