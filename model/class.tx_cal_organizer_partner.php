...
class tx_cal_organizer_partner extends tx_cal_organizer {
	  ...
	  * @param	string		$pidList	The pid-list to search in
 	  */
 	 function tx_cal_organizer_partner(&$cObj, &$rightsObj, $uid, $pidList){
		...
		$this->tx_cal_organizer_model($cObj, $rightsObj, $this->type);
		$partner = t3lib_div::makeInstanceClassName('tx_partner_main');
		$this->partner = new $partner();
	 	$this->partner->getPartner($uid);
	 	$this->partner->getContactInfo($this->cObj->conf['view.']['organizer.']['contactInfoType']);
 	 }
	...
	function getCalLegendDescription() { 
	 ...
	 function renderOrganizer(){
		...
		$page = $this->cObj->fileResource($this->cObj->conf["view."]["organizer."]["organizerTemplate4Partner"]);
		if ($page=="") {
			return "<h3>calendar: no organizer template file found:</h3>".$this->cObj->conf["view."]["organizer."]["organizerTemplate4Partner"];
		}
		$rems = array();
		$sims = array();
