...
class tx_cal_phpicalendar_model extends tx_cal_model {
	...
	function getIcsLinkMarker(& $template, & $sims, & $rems, & $wrapped, $view) {
		if ($this->conf['view.']['ics.']['showIcsLinks'] == 1) {
			$this->initLocalCObject($tempArray);
			$this->controller->getParametersForTyposcriptLink($this->local_cObj->data, array('type' => $this->getType(),'view' => 'single_ics','uid' => $this->getUid()),$this->conf['cache'],$this->conf['clearAnyway'],$GLOBALS['TSFE']->id);
	...
	function isUserAllowedToEdit($feUserUid = '', $feGroupsArray = array ()) {
		...
		return $isAllowedToEditEvent && ($isEventOwner || $isSharedUser) && ($eventHasntStartedYet || $rightsObj->isAllowedToEditEventInPast());
	}
	...
	function getLinkToEvent($linktext, $view, $date, $urlOnly = false) {
		...
		if($rightsObj->isViewEnabled('event') || $this->conf['view.']['event.']['eventViewPid']){
			...
			$link = $this->local_cObj->cObjGetSingle($this->conf['view.'][$view.'.']['event.']['eventLink'],$this->conf['view.'][$view.'.']['event.']['eventLink.']);

			if($urlOnly) {
				return $this->local_cObj->lastTypoLinkUrl;
			}
			return $link;
		}else{
			return $linktext;
