...
class tx_cal_phpicalendar_model extends tx_cal_model {
	...
	function tx_cal_phpicalendar_model(&$row, $isException, $serviceKey) {
		...
		$this->setType('tx_cal_phpicalendar');
	...
	*/
	...
	function fillTemplate($subpartMarker){
		...
		return $this->finish(substituteMarkerArrayNotCached($page, $sims, $rems, $wrapped));
	}
					...
					case 'start' :
						{
							if ($user_uid>0) {
								...
								if (($captchaStr && $controller->piVars['captcha'] === $captchaStr) ||
									...
									$htmlTemplate = substituteMarkerArrayNotCached($htmlTemplate, $local_switch, $local_rems, $local_wrapped);
									...
									$plainTemplate = substituteMarkerArrayNotCached($plainTemplate, $local_switch, $local_rems, $local_wrapped);
	...
	function getEventUrlMarker(& $template, & $sims, & $rems, & $wrapped, $view) {
		$eventStart = $this->getStart();
		$wrapped['###EVENT_URL###'] = htmlspecialchars($this->getLinkToEvent('|',$view, $eventStart->format('%Y%m%d'),true));
	}
					...
					foreach ($attendeeArray as $attendee) {
						 ...
						 if($rightsObj->getUserId() == $attendee->getFeUserId() || $isChairMan){
							...
							if($attendee->getAttendance() != 'CHAIR' && ($attendee->getStatus()=='ACCEPTED' || $attendee->getStatus()=='0' || $attendee->getStatus()=='NEEDS-ACTION')){
								$this->local_cObj->setCurrentVal(sprintf($this->conf['view.'][$view.'.']['event.']['attendeeIcon'],'DECLINE',$controller->pi_getLL('l_meeting_decline'),$controller->pi_getLL('l_meeting_decline')));
								$this->local_cObj->data['link'] = $controller->pi_linkTP_keepPIvars_url(array ('view' => 'meeting', 'lastview'=>$controller->extendLastView(), 'attendee' => $attendee->getUid(), 'uid' => $this->getUid(), 'status' => 'decline', 'sid' => md5($this->getUid().$attendee->getEmail().$attendee->row['crdate'])), $this->conf['cache'], $this->conf['clear_anyway'],	$this->conf['view.']['event.']['meeting.']['statusViewPid']).' ';
							...
							if($attendee->getAttendance() != 'CHAIR' && ($attendee->getStatus()=='DECLINE' || $attendee->getStatus()=='0' || $attendee->getStatus()=='NEEDS-ACTION')){
								$this->local_cObj->setCurrentVal(sprintf($this->conf['view.'][$view.'.']['event.']['attendeeIcon'],'ACCEPTED',$controller->pi_getLL('l_meeting_accept'),$controller->pi_getLL('l_meeting_accept')));
								$this->local_cObj->data['link'] = $controller->pi_linkTP_keepPIvars_url(array ('view' => 'meeting', 'lastview'=>$controller->extendLastView(), 'attendee' => $attendee->getUid(), 'uid' => $this->getUid(), 'status' => 'accept', 'sid' => md5($this->getUid().$attendee->getEmail().$attendee->row['crdate'])), $this->conf['cache'], $this->conf['clear_anyway'],  $this->conf['view.']['event.']['meeting.']['statusViewPid']);
	...
	function getLinkToEvent($linktext, $view, $date, $urlOnly = false) {
		...
		if($rightsObj->isViewEnabled('event') || $this->conf['view.']['event.']['eventViewPid']){
			...
			switch ($this->getEventType()) {
				...
				default:
					$linkParams = array ('page_id' => $GLOBALS['TSFE']->id, 'getdate' => $date, 'lastview' => $controller->extendLastView(), 'view' => $this->getObjectType(), 'type' => $this->getType(), 'uid' => $this->getUid());
					...
					$param = $controller->pi_linkTP_keepPIvars_url($linkParams, $this->conf['cache'], $this->conf['clear_anyway'],  $pid);
					break;
			}

			if($urlOnly){
				return $this->local_cObj->typolink('',array('parameter' => $param, 'returnLast' => 'url'));
			}
