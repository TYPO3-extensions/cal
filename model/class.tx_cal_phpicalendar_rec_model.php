...
class tx_cal_phpicalendar_rec_model extends tx_cal_model {
	...
	*/
	...
	function fillTemplate($subpartMarker){
		...
		$page = $cObj->fileResource($this->parentEvent->conf['view.']['event.']['phpicalendarEventTemplate']);
		if ($page == '') {
			return '<h3>calendar: no event template file found:</h3>' . $this->parentEvent->conf['view.']['event.']['phpicalendarEventTemplate'];
		}
		$page = $cObj->getSubpart($page,$subpartMarker);
		if(!$page){
			return 'could not find the >'.$subpartMarker.'< subpart-marker in '.$this->parentEvent->conf['view.']['event.']['phpicalendarEventTemplate'];
		}
		$rems = array ();
		$sims = array ();
		$wrapped = array();
