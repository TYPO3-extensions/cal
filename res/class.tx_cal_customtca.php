...
class tx_cal_customtca {
	...
	function rdate($PA, $fobj) {
		...
		$out[] = 'var dateFormated = document.getElementById("data_tx_cal_event_'.$this->uid.'_rdate"+i+"_hr").value;';
		$out[] = 'if(dateFormated!=""){';
		...
		if($this->rdateType == 'date_time') {
			$out[] = 'dateFormated += "T"+splittedTime[0]+splittedTime[1]+"00Z";';
		}else if($this->rdateType == 'period') {
			$out[] = 'dateFormated += "T"+splittedTime[0]+splittedTime[1]+"00Z/P";';
		...
		$out[] = '}';
		$out[] = '</script>';
		$calendarWizard = $TCA['tx_cal_event']['columns']['start_date']['config']['wizards']['calendar'];
		if($calendarWizard){
			$key = 0;
			foreach($this->rdateValues as $value){
				...
				$out[] = '<div style="float:left;"><input type="text" value="'.$formatedValue.'" name="rdate'.$key.'" id="data_tx_cal_event_'.$this->uid.'_rdate'.$key.'_hr" onchange="rdateChanged();"/><input type="hidden" value="'.$formatedValue.'" id="data_tx_cal_event_'.$this->uid.'_rdate'.$key.'" />';
				$params = Array();
