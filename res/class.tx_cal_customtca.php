...
class tx_cal_customtca {
	...
	function rdate($PA, $fobj) {
		...
		$out[] = '}';
		$out[] = 'if(jsDate=="%d-%m-%Y"){';
		$out[] = 'dateFormated = splittedDate[2]+splittedDate[1]+splittedDate[0];';
		$out[] = '} else {';
		$out[] = 'dateFormated = splittedDate[2]+splittedDate[0]+splittedDate[1];';
		$out[] = '}';
		if($this->rdateType == 'date_time') {
			$out[] = 'dateFormated += "T"+(parseInt(splittedTime[0])<10?"0"+parseInt(splittedTime[0]):parseInt(splittedTime[0]))+(parseInt(splittedTime[1])<10?"0"+parseInt(splittedTime[1]):parseInt(splittedTime[1]))+"00Z";';
		}else if($this->rdateType == 'period') {
			$out[] = 'dateFormated += "T"+(parseInt(splittedTime[0])<10?"0"+parseInt(splittedTime[0]):parseInt(splittedTime[0]))+(parseInt(splittedTime[1])<10?"0"+parseInt(splittedTime[1]):parseInt(splittedTime[1]))+"00Z/P";';
			$out[] = 'var rdateYear = parseInt(document.getElementById("rdateYear"+i).value);';
			$out[] = 'var rdateMonth = parseInt(document.getElementById("rdateMonth"+i).value);';
			$out[] = 'var rdateWeek = parseInt(document.getElementById("rdateWeek"+i).value);';
			$out[] = 'var rdateDay = parseInt(document.getElementById("rdateDay"+i).value);';
			$out[] = 'var rdateHour = parseInt(document.getElementById("rdateHour"+i).value);';
			$out[] = 'var rdateMinute = parseInt(document.getElementById("rdateMinute"+i).value);';
		...
		if($calendarWizard){
			...
			foreach($this->rdateValues as $value){
				...
				if (t3lib_div::int_from_ver(TYPO3_version) < 4003000){
					...
					if($this->rdateType == 'date_time' || $this->rdateType == 'period') {
						$out[] = '<div style="float:left;"><input type="text" value="'.$formatedValue.'" class="tceforms-datetimefield" name="rdate'.$key.'" id="tceforms-datetimefield-data_'.$this->table.'_'.$this->uid.'_rdate'.$key.'_hr" onchange="'.
								'rdateChanged();"/>'.
								'<input type="hidden" value="'.$formatedValue.'" id="data_'.$this->table.'_'.$this->uid.'_rdate'.$key.'" />'.
								'<img id="picker-tceforms-datetimefield-data_'.$this->table.'_'.$this->uid.'_rdate'.$key.'_hr" alt="" style="cursor: pointer; vertical-align: middle;" src="gfx/datepicker.gif"/>';
					} else {
						$out[] = '<div style="float:left;"><input type="text" value="'.$formatedValue.'" class="tceforms-datefield" name="rdate'.$key.'" id="tceforms-datefield-data_'.$this->table.'_'.$this->uid.'_rdate'.$key.'_hr" onchange="'.
								'rdateChanged();"/>'.
								'<input type="hidden" value="'.$formatedValue.'" id="data_'.$this->table.'_'.$this->uid.'_rdate'.$key.'" />'.
								'<img id="picker-tceforms-datefield-data_'.$this->table.'_'.$this->uid.'_rdate'.$key.'_hr" alt="" style="cursor: pointer; vertical-align: middle;" src="gfx/datepicker.gif"/>';
					}
				}
				$params = Array();
				...
				if($this->rdateType == 'period') {
					...
					$params['item'] .= '</div><div><span style="padding-left:10px;">'.$LANG->getLL('l_duration').':</span>'.
