...
class tx_cal_treeview {
	...
	function displayCalendar($PA, $fobj)    {
        ...
        if($isPluginFlexform){
            ...
            foreach($pagesEntryArray as $id){
            	ereg('[a-zA-Z]*_([0-9]*)|[a-zA-Z0-9]*', $id, $idArray);
            	$list = array();
            	$this->getTreeList($idArray[1],$recursiveEntry, $list);
            	if ($list)		$pidList[] = implode(',', $list);
	    ...
	    if($field = 'pi_flexform' && $GLOBALS['BE_USER']->user['tx_cal_enable_accesscontroll']){
		    ...
		    if($GLOBALS['BE_USER']->user['tx_cal_enable_accesscontroll']){
			    if($GLOBALS['BE_USER']->user['tx_cal_calendar']){
			    	$be_userCalendars = explode(',',$GLOBALS['BE_USER']->user['tx_cal_calendar']);
			    }
		    }
	...
	function displayCategoryTree($PA, $fobj)    {
		...
		if($row['calendar_id']==0){
			...
			$tempCalWhere = '0=0'.$enableFields.$GLOBALS['TCA']['tx_cal_event']['columns']['calendar_id']['config']['foreign_table_where'].' LIMIT 1';
			$tempCalRes = $GLOBALS['TYPO3_DB']->exec_SELECTquery('tx_cal_calendar.uid', 'tx_cal_calendar',$tempCalWhere);
		...
		if ($maxitems<=1 AND !$config['treeView'])	{
			...
			if ($row['sys_language_uid'] && $row['l18n_parent'] ) { // the current record is a translation of another record
				...
				$catres = $GLOBALS['TYPO3_DB']->exec_SELECTquery('tx_cal_category.uid, tx_cal_category.title', 'tx_cal_category');
				$categories = array();
				$NACats = array();
				$na = false;
...
//*************************************************
				if($config['treeView'] AND $config['foreign_table']) {
                    ...
                    if($table=='be_users' || $table=='be_groups') {
					    ...
					    $be_userCalendars = array();
					    if($row['calendar_id']>0){
							$catWhere = 'tx_cal_category.calendar_id IN(0,'.$row['calendar_id'].')';
						    $calWhere = 'tx_cal_calendar.uid IN(0,'.$row['calendar_id'].')';
	                        
							if($table!='tx_cal_event'){
						    	$notAllowedItems[] = $row['uid'];
							}
							if($GLOBALS['BE_USER']->user['tx_cal_enable_accesscontroll']){
							    $be_userCategories = explode(',',$GLOBALS['BE_USER']->user['tx_cal_category']);
							    $be_userCalendars = explode(',',$GLOBALS['BE_USER']->user['tx_cal_calendar']);
							}
						    if (is_array($BE_USER->userGroups)) {
								foreach ($BE_USER->userGroups as $gid => $group) {
					    ...
					    }else if($field = 'pi_flexform' && $GLOBALS['BE_USER']->user['tx_cal_enable_accesscontroll']){
						    $allowAllCalendars = false;
						    $allowAllCategories = false;
						    $be_userCategories = array(0);
						    ...
						    $catWhere = 'tx_cal_category.calendar_id = 0';
						    $calWhere = 'tx_cal_calendar.uid = 0';
					...
					$catWhere.= $catWhere=='' ? '0=0'.$enableFields:$enableFields;
					
					if($isPluginFlexform){
						$catWhere.= ' AND pid IN ('.$pidList.')';
					}

					$catres = $GLOBALS['TYPO3_DB']->exec_SELECTquery ('tx_cal_category.uid, tx_cal_category.title, tx_cal_category.calendar_id, tx_cal_category.parent_category', 'tx_cal_category',$catWhere);
					...
					foreach($categoryById as $catRow){
						$ids = $this->checkChildIds($catRow['uid'],$allCategoryByParentId);
					}
					foreach($ids as $id){
	...
	function addChildren($uid, &$childArray, &$categoryById, &$categoryByParentId, $subLevelID){
		...
		if($categoryByParentId[$uid]){
			foreach($categoryByParentId[$uid] as $category){
				$childArray2 = array('title'=>$category['title'],'uid'=>$category['uid']);
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/cal/res/class.tx_cal_treeview.php'])    {
    include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/cal/res/class.tx_cal_treeview.php']);
}
?>
