...
class tx_cal_treeview {
	...
	function displayCalendar($PA, $fobj)    {
		...
		if(TYPO3_MODE == 'BE') {
			$enableFields = t3lib_befunc::BEenableFields('tx_cal_calendar');
		} else {
	...
	function displayCategoryTree($PA, $fobj)    {
		...
		if ($maxitems<=1 AND !$config['treeView'])	{
			...
			if ($row['sys_language_uid'] && $row['l18n_parent'] ) { // the current record is a translation of another record
				...
				$catres = $GLOBALS['TYPO3_DB']->exec_SELECT_query('tx_cal_category.uid, tx_cal_category.title', 'tx_cal_category');
				$categories = array();
				$NACats = array();
				$na = false;
...
//*************************************************
				if($config['treeView'] AND $config['foreign_table']) {
                    ...
                    if($table=='be_users' || $table=='be_groups') {
					    ...
					    $be_userCategories = array();
					    $be_userCalendars = array();
					    if($row['calendar_id']>0 && $GLOBALS['BE_USER']->user['tx_cal_enable_accesscontroll']){
							$catWhere = 'tx_cal_category.calendar_id = '.($row['calendar_id']?$row['calendar_id']:0);
						    $calWhere = 'tx_cal_calendar.uid = '.($row['calendar_id']?$row['calendar_id']:0);
					...
					if(TYPO3_MODE == 'BE') {
						$enableFields = t3lib_befunc::BEenableFields('tx_cal_category');
					} else {
