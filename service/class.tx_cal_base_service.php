...
class tx_cal_base_service extends t3lib_svbase {
	...
	function _notify($uid, &$insertFields){
		...
		$this->conf['calendar'] = $this->controller->piVars['calendar_id'];
		$eventService = $this->getEventService();
		$event = $eventService->find($uid, $this->conf['pidList'], true, true);
		$this->conf['calendar'] = $config;
		$notificationService =& getNotificationService();
		$notificationService->notifyOfChanges($event->getValuesAsArray(), $insertFields, $this->conf['view.']['event.']['notify.']);
	}
