...
class tx_cal_base_service extends t3lib_svbase {
	...
	function insertIdsIntoTableWithMMRelation($mm_table,$idArray,$uid,$tablename,$additionalParams=array()){
		foreach($idArray as $key => $foreignid){
			if(is_numeric ($foreignid)){
				$insertFields = array_merge(array('uid_local'=>$uid, 'uid_foreign' => $foreignid, 'tablenames' =>$tablename, 'sorting' => $key+1),$additionalParams);
	...
	function getAdditionalWhereForLocalizationAndVersioning($table){
		...
		if ($this->conf['showRecordsWithoutDefaultTranslation']) {
				$selectConf['where'] = ' AND ('.$selectConf['where'].' OR ('.$table.'.sys_language_uid='.$GLOBALS['TSFE']->sys_language_content.' AND NOT '.$table.'.l18n_parent))';
		} else {
			$selectConf['where'] = ' AND '.$selectConf['where'];
		}
