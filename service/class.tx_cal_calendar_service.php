...
class tx_cal_calendar_service extends tx_cal_base_service {
	...
	function findAll($pidList){
		$return = array();
		if($pidList==''){
			$result = $GLOBALS['TYPO3_DB']->exec_SELECTquery('*', 'tx_cal_calendar', ' 0 = 0'.$this->cObj->enableFields('tx_cal_calendar'));
		}else{
			$result = $GLOBALS['TYPO3_DB']->exec_SELECTquery('*', 'tx_cal_calendar', ' pid IN ('.$pidList.') '.$this->cObj->enableFields('tx_cal_calendar'));
		}
		while($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($result)){
			$return[] = $row;
		}
...
//debug($calendarSearchString);	
	...
	function getIdsFromTable($list, $pidList, $includePublic, $includeData=false, $onlyPublic=false){
		...
		$result = $GLOBALS['TYPO3_DB']->exec_SELECTquery('uid_local', 'tx_cal_calendar, tx_cal_calendar_user_group_mm', 'tx_cal_calendar.uid=tx_cal_calendar_user_group_mm.uid_local '.$this->cObj->enableFields('tx_cal_calendar'));
		while($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($result)) {
			$ids[] = $row['uid_local'];
		}
