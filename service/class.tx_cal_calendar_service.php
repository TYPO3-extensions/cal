...
class tx_cal_calendar_service extends tx_cal_base_service {
	...
	var $fnbCalendarSearchStringCache = Array();
	var $calendarOwner;	
	var $calendarIds;
...
//		$idString = implode(',',$ids);
		if(!empty($idArray)){
			...
			$calendarSearchString = ' AND tx_cal_calendar.uid IN ('.implode(',',$idArray).')';
		}
...
//t3lib_div::debug('SELECT '.$select.' FROM '.$table.' WHERE '.$where.' GROUP BY '.$groupby);
		...
		$calendarIdsWantedAndAllowed = array_intersect($allowedFnbCalendars, explode(',',$calendarIds));
