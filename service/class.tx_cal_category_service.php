...
//		$categorySearchString = '(tx_cal_event_category_mm.uid_local=tx_cal_event.uid AND tx_cal_event_category_mm.uid_foreign = tx_cal_category.uid AND tx_cal_calendar.uid = tx_cal_event.calendar_id)';
	...
	function getCategoryArray($pidList, $showPublicCategories=true){
		...
		$select = 'tx_cal_category.*, tx_cal_event_category_mm.*';
		$table = 'tx_cal_category, tx_cal_event_category_mm';
		$groupby = '';//' tx_cal_category.uid';
		$orderby = 'tx_cal_event_category_mm.sorting DESC';

		$where = '(tx_cal_event_category_mm.uid_foreign = tx_cal_category.uid AND tx_cal_category.calendar_id=0)';
...
//t3lib_div::debug('SELECT '.$select.' FROM '.$table.' WHERE '.$where.' ORDER BY '.$orderby);
		...
		while ($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($result)) {
			...
			$this->addCalLegendDescription('', $row['headerstyle']?$row['headerstyle']:$this->getHeaderStyle(), $row);
		}
		...
		$where .= ' AND tx_cal_calendar.hidden = 0 AND tx_cal_calendar.deleted = 0 AND tx_cal_category.deleted = 0 AND tx_cal_category.hidden = 0';
