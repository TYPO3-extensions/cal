...
class tx_cal_category_service extends tx_cal_base_service {
	...
	function findAll($pidList){
		return $this->getCategoryArray(true);
	}
	...
	function retrievePostData(&$insertFields){
		...
		if($this->rightsObj->isAllowedToEditCategoryHeaderstyle() || $this->rightsObj->isAllowedToCreateCategoryHeaderstyle()){
			$insertFields['headerstyle'] = $this->controller->piVars['headerstyle'];
		}
		
		if($this->rightsObj->isAllowedToEditCategoryBodystyle() || $this->rightsObj->isAllowedToCreateCategoryBodystyle()){
			$insertFields['bodystyle'] = $this->controller->piVars['bodystyle'];
		}
	...
	function getCategorySearchString($pidList, $includePublic){
		if($this->cObj->conf['category']==""){
			...
			$categorySearchString = " AND tx_cal_category.uid IN (".$this->cObj->conf['category'].")";
		}
	 ...
	 */
	function getCategoryArray($showPublicCategories=true){
...
//		}
		...
		$additionalWhere = "";
		if($this->cObj->conf['category']){
			$additionalWhere = " AND tx_cal_category.uid IN (".$this->cObj->conf['category'].")";
		}
		$calendarService  = $this->getCalendarService();
		$calendarIds = $calendarService ->getIdsFromTable($this->cObj->conf['calendar'], $this->cObj->conf['pidList'], true);
		...
		if(!empty($calendarIds)){
			$where .= " AND tx_cal_calendar.uid in (".$this->arrayToCommaseparatedString($calendarIds).")";
		}
