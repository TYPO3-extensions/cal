...
//		}
	...
	function getCategoryArray($pidList, $showPublicCategories=true){
        ...
        if($this->conf['view.']['allowedCategory']!='' && $this->conf['view.']['allowedCategory']!=0) {
...
//t3lib_div::debug('SELECT '.$select.' FROM '.$table.' WHERE '.$where.' GROUP BY '.$groupby.' ORDER BY '.$orderby);
		...
		$calendarsWithoutCategory = array_diff(explode(',',$this->conf['view.']['calendar']),array_unique($calendarUids));
		...
		if(!empty($calendarsWithoutCategory)){
			...
			$where = 'tx_cal_calendar.uid IN ('.implode(',',$calendarsWithoutCategory).')'.$this->cObj->enableFields('tx_cal_calendar');
