	...
	function getCategoryArray($pidList, &$categoryArrayToBeFilled, $showPublicCategories=true){
		...
		switch($this->conf['view.']['categoryMode']) {
			...
			case 1: #show selected
				$allowedCategories = t3lib_div::trimExplode(',',$this->conf['view.']['category'],1);
			    if (!empty($allowedCategories)){
				    $filterWhere = ' AND tx_cal_category.uid IN ('.$this->conf['view.']['category'].')';
			    }
				break;
			case 2: #exclude selected
				$allowedCategories = t3lib_div::trimExplode(',',$this->conf['view.']['category'],1);
			    if (!empty($allowedCategories)){
				    $filterWhere = ' AND tx_cal_category.uid NOT IN ('.$this->conf['view.']['category'].')';
			    }
				break;
		}
...
//			$groupby = 'tx_cal_category.uid';
		...
		if($this->conf['view.']['allowedCategories']){
			$this->categoryArrayCached[md5($this->conf['view.']['allowedCategories'])] = array($this->categoryArrayByUid,$this->categoryArrayByEventUid,$this->categoryArrayByCalendarUid);
		}
