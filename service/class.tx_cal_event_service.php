...
//	t3lib_div::debug('SELECT '.$select.' FROM '.$table.' WHERE '.$where.' GROUP BY '.$groupby);
	...
	function recurringEvent($event){
		...
		if($event->getRdateType()!='none'){
			$this->getRecurringDate($master_array, $event);
		}
	...
	function getRecurringDate(&$master_array, &$event){
			...
			case 'date':
				foreach($event->getRdateValues() as $rdateValue){
					...
					if($new_event->isAllday()){
						$master_array[$start->format('%Y%m%d')]['-1'][$start->getUid()] = $new_event;
					}else{
