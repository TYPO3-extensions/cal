...
//		$additionalWhere = $categorySearchString.$calendarSearchString.' AND tx_cal_event.pid IN ('.$pidList.') AND tx_cal_event.hidden = 0 AND tx_cal_event.deleted = 0 AND (tx_cal_event.start_time>='.$start_date.' OR tx_cal_event.end_time<'.$end_date.' OR ((tx_cal_event.freq!='none' OR tx_cal_event.freq!='') AND tx_cal_event.until <= '.$this->conf['getdate'].'))';
		$additionalWhere = $calendarSearchString.' AND tx_cal_event.pid IN ('.$pidList.') AND tx_cal_event.hidden = 0 AND tx_cal_event.deleted = 0 AND (tx_cal_event.start_time>='.$start_date.' OR tx_cal_event.end_time<'.$end_date.' OR tx_cal_event.freq!="none" OR tx_cal_event.freq!="")';
...
//debug($row);
			...
			$offset = $GLOBALS['TYPO3_CONF_VARS']['SYS']['serverTimeZone']*3600*12;
			$row['start_date'] += $offset;
			if($row['end_date']>0){
				$row['end_date'] += $offset;
			}else{
...
//debug($categories);
	...
	function saveEvent($pid){
		...
		if($this->rightsObj->isAllowedToCreateEventNotify()){
			...
			$this->insertIdsIntoTableWithMMRelation('tx_cal_fe_user_event_monitor_mm',array($this->conf['rights.']['create.']['event.']['notifyUserOnPublicCreate']),$uid,'fe_users');
		}
		...
		if($this->rightsObj->isAllowedToCreateEventShared()){
			$idArray = split(',',strip_tags($this->controller->piVars['shared_user_ids']));
			...
			$idArray = split(',',$this->conf['rights.']['create.']['event.']['fields.']['allowedToCreateShared.']['uidDefault']);
			if($this->conf['rights.']['create.']['event.']['addFeUserToShared']){
				$idArray[] = $this->rightsObj->getUserId();
			}
			$this->insertIdsIntoTableWithMMRelation('tx_cal_event_shared_user_mm',$idArray,$uid,'fe_users');
		...
		$insertFields['category'] = $this->controller->piVars['category_display_ids'];
		require_once(t3lib_extMgm::extPath('cal').'controller/class.tx_cal_functions.php');
...
//		}
		...
		$this->_notify($event,$insertFields);
		$this->unsetPiVars();
		require_once(t3lib_extMgm::extPath('cal').'controller/class.tx_cal_functions.php');
		clearCache();
	}
	...
	function retrievePostData(&$insertFields){
		...
		if($this->rightsObj->isAllowedToEditEventDateTime() || $this->rightsObj->isAllowedToCreateEventDateTime()){
			$offset = $GLOBALS['TYPO3_CONF_VARS']['SYS']['serverTimeZone']*3600;
			if($this->controller->piVars['event_start_day']!=''){
				$time = array();
				preg_match ('/([0-9]{4})([0-9]{2})([0-9]{2})/', $this->controller->piVars['event_start_day'], $time);
				$insertFields['start_date'] = gmmktime(0,0,0,$time[2],$time[3],$time[1])-$offset;
			}else{
				return;
			}
			...
			if($this->controller->piVars['event_end_day']!=''){
				...
				$insertFields['end_date'] = gmmktime(0,0,0,$time[2],$time[3],$time[1])-$offset;
			}
	...
	function recurringEvent($event){
		...
		if ($uid_valid && isset($processed[$uid]) && isset($recurrence_id['date'])) {
			...
			if (isset($master_array[gmdate('Ymd',$start_date_tmp)][$old_start_time][$uid])) {
				unset($master_array[gmdate('Ymd',$start_date_tmp)][$old_start_time][$uid]);  // SJBO added $uid twice here
				if (sizeof($master_array[gmdate('Ymd',$start_date_tmp)][$old_start_time]) == 0) {
					unset($master_array[gmdate('Ymd',$start_date_tmp)][$old_start_time]);
				}
			}
		...
		if (isset($start_time) && isset($end_time)) {
			...
			if (($end_time - $start_time) > 2345 || $end_time==$start_time) {
				...
				$alldayEnd = ($start_date + 86400);
			}
		}
		if (isset($start_unixtime,$end_unixtime) && (gmdate('Ymd',$start_unixtime) == gmdate('Ymd',$end_unixtime))==true && $start_time == $end_time) {
			$event->setSpansDay(1);
			$bleed_check = (($start_unixtime - $end_unixtime) < (86400)) ? '-1' : '0';
		} else {
			$event->setSpansDay(0);
			$bleed_check = 0;
		}
		...
		if ((isset($alldayStart) && $alldayStart != '')) {
			...
			if (($start < $mArray_end) && ($start < $end)) {
				...
				while (($start != $end) && ($start < $mArray_end)) {
					$start_date2 = gmdate('Ymd', $start);
					$master_array[$start_date2][('-1')][$uid]= $event;
					$start += 86400;

				}
				if (!$write_processed) $master_array[gmdate('Ymd',$start_date)]['-1'][$uid]['exception'] = true;
			}
		}	
		...
		if ((isset($start_time) && $start_time != '') && (!isset($alldayStart) || $alldayStart == '')) {
			...
			if (($bleed_check == '-1')) {//($end_time >= $bleed_time) && 
				$start_tmp = gmmktime(0,0,0,gmdate('m',$start_unixtime),gmdate('d',$start_unixtime),gmdate('Y',$start_unixtime));
				$end_date_tmp = gmdate('Ymd',$end_unixtime);
				while ($start_tmp <= $end_unixtime) {
					$start_date_tmp = gmdate('Ymd',$start_tmp);
...
//						$minute = '';
				...
				if (!in_array($start_date, $except_dates)) {
					if($event->getEndHour()){
						$master_array[gmdate('Ymd',$start_date)][($hour.$minute)][$uid] = $event;
					}else{
						$master_array[gmdate('Ymd',$start_date)][('-1')][$uid] = $event;
					}
					if (!$write_processed) $master_array[gmdate('Ymd',$start_date)][($hour.$minute)][$uid]['exception'] = true;
				}
			}
		}
		...
		if (isset($rrule_array) && is_array($rrule_array)) {
			...
			$this_month_start_time = gmmktime(0,0,0,$this_month,1,$this_year);
	
			if ($current_view == 'year'){// || ($save_parsed_cals == 'yes' && !$is_webcal)) {
				$start_range_time = gmmktime(0,0,0,1,1,$this_year);//strtotime($this_year.'-01-01 -2 weeks');
				$end_range_time = gmmktime(0,0,0,12,31,$this_year);//strtotime($this_year.'-12-31 +2 weeks');
			} else if ($current_view == 'list') {
				$start_range_time = strtotime($this->conf['view.']['list.']['starttime']) - strtotimeOffset();
				$end_range_time = strtotime($this->conf['view.']['list.']['endtime']) - strtotimeOffset();
			} else {
				$start_range_time = strtotime('-1 month -2 day', $this_month_start_time) - strtotimeOffset();
				$end_range_time = strtotime('+2 month +2 day', $this_month_start_time) - strtotimeOffset();
			}
			if($event->getEndHour()){
				$recur = $master_array[gmdate('Ymd',$start_date)][($hour.$minute)][$uid]->getRecurringRule();
			}else{
				$recur = $master_array[gmdate('Ymd',$start_date)][('-1')][$uid]->getRecurringRule();
			}
			...
			if (!isset($abs_until)){
				$abs_until = gmdate('YmdHis', $end_range_time);
			}
			$end_date_time = $until;
...
//debug('end_range_time_tmp:'.gmdate('Ymd H:i',$end_range_time_tmp));
//debug('start_date_time:'.gmdate('Ymd H:i',$start_date_time));
//debug('start_range_time_tmp:'.gmdate('Ymd H:i',$start_range_time_tmp));
//debug('end_date_time:'.gmdate('Ymd H:i',$end_date_time));
...
//debug('hier');
//debug($freq_type);
//debug('next_range_time:'.gmdate('Ymd H:i',$next_range_time));
//debug('start_range_time_tmp:'.gmdate('Ymd H:i',$start_range_time_tmp));
//debug('next_range_time:'.gmdate('Ymd H:i',$next_range_time));
//debug('end_range_time_tmp:'.gmdate('Ymd H:i',$end_range_time_tmp));
							...
							switch ($rrule_array['FREQ']) {
								...
								case 'week':
									...
									if (empty($byday)) {
										...
										$byday[] = strtoupper(substr($daysofweek[gmdate('w', $start_date_time)], 0, 2));//
									}
									if (is_array($byday)) {
//debug($byday);
										foreach($byday as $day) {
											$day = tx_cal_calendar::two2threeCharDays(strtoupper($day));									
											$next_date_time = strtotime($day,$next_range_time) + strtotimeOffset();// + (12 * 60 * 60);
//debug('next_date_time:'.gmdate('Ymd H:i',$next_date_time));
											...
											if ($next_date_time > strtotime($this->conf['view.']['weekStartDay'], $next_range_time) - strtotimeOffset() && $interval > 1) {
												$next_date_time = strtotime('+'.($interval - 1).' '.$freq_type, $next_date_time) - strtotimeOffset();
											}
											$recur_data[] = $next_date_time;
										}
									}
								...
								case 'month':
									...
									$next_range_time = strtotime(gmdate('Y-m-01', $next_range_time)) - strtotimeOffset();
									$next_date_time = $next_range_time;
									if(empty($bymonthday) && empty($byday)){
										$bymonthday = array(gmdate('d', $event->getStarttime()));
									}

									if (isset($bymonthday) && !empty($bymonthday) && ((empty($byday)))) {
										foreach($bymonthday as $day) {
											if ($day < 0) $day = ((gmdate('t', $next_range_time)) + ($day)) + 1;
										...
										foreach($byday as $day) {
											...
											if ((isset($byday_arr[1])) && ($byday_arr[1] == '-')) {
												$last_day_tmp = gmdate('t',$next_range_time);
												...
												foreach($bymonthday as $day) {
													...
													if (checkdate($month,$day,$year)) {
														...
														$month = intval($month);
														$diff = $next_date_time - strtotime($testarray[$byday_arr[2]].' last '.$on_day,(gmmktime(0,0,0,$month,$last_day_tmp,$year))) - strtotimeOffset();
												...
												foreach($bymonthday as $day) {
													...
													if (checkdate($month,$day,$year)) {
														$next_date_time = gmmktime(0,0,0,$month,$day,$year);
														$daday = strtolower(gmstrftime('%a', $next_date_time));
												...
												$next_date_time = strtotime($on_day.' +'.$nth.' week', $next_range_time) - strtotimeOffset();
												$month = gmdate('m', $next_date_time);
												if (in_array($month, $bymonth)) {
													$recur_data[] = $next_date_time;
												}
											}
											$next_date = gmdate('Ymd', $next_date_time);
										}
									}
									break;
								case 'year':
									...
									foreach($bymonth as $month) {
										...
										if ($month < gmdate('m', $next_range_time)) {
											$year = gmdate('Y', strtotime('+1 years', $next_range_time) - strtotimeOffset());
										} else {
											$year = gmdate('Y', $next_range_time);
										}
										...
										if (!empty($byday)) {
											...
											foreach($byday as $day) {
												...
												if ($byday_arr[1] == '-') {
													$last_day_tmp = gmdate('t',$checkdate_time);
													$checkdate_time = strtotime(gmdate('Y-m-'.$last_day_tmp, $checkdate_time) - strtotimeOffset());
													$last_tmp = (gmdate('w',$checkdate_time) == $on_day_num) ? '' : 'last ';
													$next_date_time = strtotime($last_tmp.$on_day.' -'.$nth.' week', $checkdate_time) - strtotimeOffset();
												} else {															
													$next_date_time = strtotime($on_day.' +'.$nth.' week', $checkdate_time) - strtotimeOffset();
												}
											}
										} else {
											$day 	= gmdate('d', $start_date_time);
											$next_date_time = gmmktime(0,0,0,$month,$day,$year);
										}
										$recur_data[] = $next_date_time;
									}
										...
										$yearday = gmdate('Ymd',$event->getStartdate());
										ereg ('([-\+]{0,1})?([0-9]{1,3})', $yearday, $byyearday_arr);
										if ($byyearday_arr[1] == '-') {
											$ydtime = gmmktime(0,0,0,12,31,$this_year);
											$yearnum = $byyearday_arr[2] - 1;
											$next_date_time = strtotime('-'.$yearnum.' year', $ydtime) - strtotimeOffset();
										} else {
											$ydtime = gmmktime(0,0,0,1,1,$this_year);
											$yearnum = $byyearday_arr[2] - 1;
								
											$next_date_time = strtotime('+1 year', $next_date_time) - strtotimeOffset();
										}
										$recur_data[] = $next_date_time;
						...
						$next_range_time = strtotime('+'.$interval.' '.$freq_type, $next_range_time) - strtotimeOffset();
					} else {
					...
					if (isset($recur_data) && is_array($recur_data)) {
						...
						foreach($recur_data as $recur_data_time) {
							$recur_data_year = gmdate('Y', $recur_data_time);
							$recur_data_month = gmdate('m', $recur_data_time);
							$recur_data_day = gmdate('d', $recur_data_time);
							...
							if (($recur_data_time > $start_date_time) && ($recur_data_time <= $end_date_time) && ($count_to != $count) && !in_array($recur_data_date, $except_dates)) {
								if (isset($alldayStart) && $alldayStart != '') {
									...
									$end_time2 = strtotime('+'.$diff_allday_days.' days', $recur_data_time) - strtotimeOffset();
									while ($start_time2 < $end_time2) {
										$start_date2 = gmdate('Ymd', $start_time2);
										...
										$start_time2 = $start_time2 + 86400;
									}
								} else {
									$start_unixtime_tmp = gmmktime($recur_data_hour,$recur_data_minute,0,$recur_data_month,$recur_data_day,$recur_data_year);
									$end_unixtime_tmp = $start_unixtime_tmp + $length;
									if (($bleed_check == '-1')) { //($end_time >= $bleed_time) && 
										$start_tmp = strtotime(gmdate('Ymd',$start_unixtime_tmp)) - strtotimeOffset();
		...
		if (is_array($except_dates)) {
			foreach ($except_dates as $key => $value) {
				...
				unset($master_array[gmdate('Ymd',$value)][$time][$uid]);
				if (count($master_array[gmdate('Ymd',$value)][$time]) < 1) {
					unset($master_array[gmdate('Ymd',$value)][$time]);
					if (count($master_array[gmdate('Ymd',$value)]) < 1) {
						unset($master_array[gmdate('Ymd',$value)]);	
					}
				}
			}
		}
