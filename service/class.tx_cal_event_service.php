...
class tx_cal_event_service extends tx_cal_base_service {
	...
	function findAllWithin($start_date, $end_date, $pidList) {
		...
		if($this->endtime > $this->starttime){
			$this->endtime -= strtotimeOffset($this->endtime) + 86400;
		}
		$this->starttime -= strtotimeOffset($this->starttime);
		
		$service = &$this->getCalendarService();
		$calendarSearchString = $service->getCalendarSearchString($pidList, true, $this->conf['view.']['calendar']?$this->conf['view.']['calendar']:'');
		
		$service = &$this->getCategoryService();
		$categorySearchString = $service->getCategorySearchString($pidList, true);

		// putting everything together
		$additionalWhere = $calendarSearchString.' AND tx_cal_event.pid IN ('.$pidList.') '.$this->cObj->enableFields('tx_cal_event').' AND ((tx_cal_event.start_date>='.$this->starttime.' AND tx_cal_event.start_date<'.$this->endtime.') OR (tx_cal_event.end_date<='.$this->endtime.' AND tx_cal_event.end_date>'.$this->starttime.') OR (tx_cal_event.end_date>='.$this->endtime.' AND tx_cal_event.start_date<='.$this->starttime.') OR tx_cal_event.freq IN ("day","week","month","year"))';
...
//debug($categories);
	...
	function saveEvent($pid){
		...
		$insertFields['start_time'] = $insertFields['start_date'];
		$insertFields['start_time'] += $insertFields['start_time'] + strtotimeOffset($insertFields['start_date']);
	...
	function search($pidList='', $starttime, $endtime, $searchword, $locationIds){
		$service = &$this->getCategoryService();
		$categoryIds = implode(',',$service->getCategoryArray($pidList));
		$events=array();
		$additionalSearch = '';
		if($searchword!=''){
		...
		if($endtime > $starttime){
			$endtime -= strtotimeOffset($endtime) + 86400;
		}
		$starttime -= strtotimeOffset($starttime);
		
		$timeSearchString = ' AND tx_cal_event.pid IN ('.$pidList.') '.$this->cObj->enableFields('tx_cal_event').' AND ((tx_cal_event.start_date>='.$starttime.' AND tx_cal_event.start_date<'.$endtime.') OR (tx_cal_event.end_date<='.$endtime.' AND tx_cal_event.end_date>'.$starttime.') OR (tx_cal_event.end_date>='.$endtime.' AND tx_cal_event.start_date<='.$starttime.') OR tx_cal_event.freq IN ("day","week","month","year"))';
