...
class tx_cal_event_service extends tx_cal_base_service {
	...
	function getEventsFromTable(&$categories, $includeRecurring=false, $additionalWhere='', $serviceKey='', $categoryWhere='', $onlyMeetingsWithoutStatus=false){
		...
		if($categoryWhere!=''){
			...
			$where .= ' AND tx_cal_event_category_mm.uid_local = tx_cal_event.uid'.$categoryWhere;
...
//	t3lib_div::debug('SELECT '.$select.' FROM '.$table.' WHERE '.$where.' GROUP BY '.$groupby);
	 ...
	 */
	function find($uid, $pidList, $showHiddenEvents=false, $showDeletedEvents=false, $getAllInstances=false, $disableCalendarSearchString=false, $disableCategorySearchString=false) {
		$uid = intval($uid);
		if($getAllInstances){
		...
		$events = $this->getEventsFromTable($categories[0][0], $getAllInstances, $additionalWhere, $this->getServiceKey(), $categorySearchString);
	...
	function _saveEvent(&$eventData, $object){
		...
		if($this->rightsObj->isAllowedTo('create','event','attendee') && $object->getEventType()==3){
			$attendeeUids = Array();
			...
			$this->insertIdsIntoTableWithMMRelation('tx_cal_event_attendee_mm',array_unique($attendeeUids),$uid,'tx_cal_attendee');
	...
	function updateEvent($uid){
		...
		$event_old = $this->find($uid, $this->conf['pidList'], true, true);
	...
	function _updateEvent($uid, $eventData, $object){
		...
		if($this->rightsObj->isAllowedTo('edit','event','attendee') && $object->getEventType()==3){
	...
	function removeEvent($uid){
		...
		if ($event->isUserAllowedToDelete()) {
			...
			$extConf = unserialize($GLOBALS['TYPO3_CONF_VARS']['EXT']['extConf']['cal']);
			if($extConf['useNewRecurringModel']){
				tx_cal_recurrence_generator::cleanIndexTableOfUid($uid);
			}
			...
			case 'date':
				foreach($event->getRdateValues() as $rdateValue){
					...
					if ($end->after($this->starttime) && $start->before($this->endtime)) {
						if($this->extConf['useNewRecurringModel']) {
							$table = 'tx_cal_index';
							$eventData = Array('start_datetime' => $start->format('%Y%m%d').$start->format('%H%M%S'), 'end_datetime' => $end->format('%Y%m%d').$end->format('%H%M%S'), 'event_uid' => $event->getUid());
							$result = $GLOBALS['TYPO3_DB']->exec_INSERTquery($table,$eventData);
						} else {
							if($new_event->isAllday()){
	...
	function checkByDay(&$event){
		...
		if(count($allowedValues)==0){
			if($event->getFreq()=='week'){
				$eventStart = $event->getStart();
				$allowedValues = array(substr($eventStart->format('%a'),0,2));
			}else{
			...
			if(!$nextOccuranceTime->equals($event->getStart())){
				if(($totalCount % $event->getInterval()) == 0){
					...
					if($this->starttime->before($nextOccuranceEndTime) || $this->starttime->equals($nextOccuranceTime)){
						if($this->extConf['useNewRecurringModel']) {
							$table = 'tx_cal_index';
							$eventData = Array('start_datetime' => $nextOccuranceTime->format('%Y%m%d').$nextOccuranceTime->format('%H%M%S'), 'end_datetime' => $nextOccuranceEndTime->format('%Y%m%d').$nextOccuranceEndTime->format('%H%M%S'), 'event_uid' => $event->getUid());
							$result = $GLOBALS['TYPO3_DB']->exec_INSERTquery($table,$eventData);
						} else {
							$new_event = $event->cloneEvent();
	...
	function findAllWithAdditionalWhere($where=''){
		...
		// creating events
		return $this->getEventsFromTable($categories[0][0], false, $additionalWhere, $this->getServiceKey(), $categorySearchString);
	}
}
