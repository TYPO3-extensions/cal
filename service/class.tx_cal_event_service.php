...
class tx_cal_event_service extends tx_cal_base_service {
	...
	function findAllWithin($start_date, $end_date, $pidList) {	
		...
		$linkIds = array();
		if($this->cObj->conf['calendar']!=""){
			$linkIds = split(",",$this->cObj->conf['calendar']);
		}
		...
		$calendarSearchString = $service->getCalendarSearchString($pidList, $includePublic, $linkIds);
		
		$service = &$this->getCategoryService();
		$categorySearchString = $service->getCategorySearchString($pidList, $includePublic);

		// putting everything together
		$additionalWhere = $categorySearchString.$calendarSearchString." AND tx_cal_event.pid IN (".$pidList.") AND tx_cal_event.hidden = 0 AND tx_cal_event.deleted = 0 AND (tx_cal_event.starttime>=".$start_date." OR tx_cal_event.endtime<".$end_date." OR tx_cal_event.freq!='none' OR tx_cal_event.freq!='')";
		
		// creating the arrays the user is allowed to see

		$categories = $service->getCategoryArray();	
		$this->descriptionFixForPhp4 = $service->callegenddescription;
		// creating events
		return $this->getEventsFromTable($categories, true, $additionalWhere, $this->getServiceKey());
	}
	 ...
	 */
	function getEventsFromTable(&$categories, $includeRecurring=false, $additionalWhere="", $serviceKey=''){
		...
		$select = "tx_cal_calendar.uid AS calendar_uid, " .
				...
				"tx_cal_event.*, " .
				"tx_cal_category.uid AS category_uid " ;
		$table = "tx_cal_event, tx_cal_category, tx_cal_event_category_mm, tx_cal_calendar";
		$where = "(tx_cal_event_category_mm.uid_local=tx_cal_event.uid AND tx_cal_event_category_mm.uid_foreign = tx_cal_category.uid AND tx_cal_calendar.uid = tx_cal_event.calendar_id)".$additionalWhere;
		$groupBy = "";
		$orderBy = " tx_cal_event.start_date ASC, tx_cal_event.start_time ASC, tx_cal_event_category_mm.sorting";
		$limit = "";
...
//t3lib_div::debug("SELECT ".$select." FROM ".$table." WHERE ".$where." GROUP BY ".$groupBy);
		...
		while ($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($result)) {
			if($row['uid']==$lastUid){
				$event->addCategory($categories[$row['category_uid']]);
				continue;	
			}
			$lastUid = $row['uid'];
...
//debug($events);
	...
	function findAll($pidList='') {		
		...
		$idArray = $calendarService->getIdsFromTable($this->cObj->conf['calendar'],$pidList, $includePublic, false, true);
		...
		$linkIds = split(",",$this->cObj->conf['category']);
		...
		// putting everything together
		$additionalWhere = $categorySearchString." AND ".$calendarSearchString." tx_cal_event.hidden = 0 AND tx_cal_event.deleted = 0";
		if($pidList===""){
		...
		// creating the arrays the user is allowed to see
		$categories = $categoryService->getCategoryArray();
	...
	function createEvent($row, $isException){
		...
		$event = &new $tx_cal_phpicalendar_model($this->cObj,$this->rightsObj, $row, $isException, $this->getServiceKey());
...
//debug($categories);
		...
		$calendarIds = $calendarService->getIdsFromTable($this->cObj->conf['calendar'],$pidList, true);
		if(empty($calendarIds)){
			$result = $GLOBALS['TYPO3_DB']->exec_SELECTquery("uid_local", "tx_cal_calendar_user_group_mm", "");
			while($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($result)) {
				$ids[] = $row['uid_local'];
			...
			$idString = $this->arrayToCommaseparatedString($ids);
			if($idString!=""){
				$calendarSearchString = " AND tx_cal_calendar.uid NOT IN (".$idString.") AND ";
			}
		}else{
			$calendarIds = $this->arrayToCommaseparatedString($calendarIds);
			$calendarSearchString = "AND tx_cal_calendar.uid IN (".$calendarIds.") AND ";
		}
...
//		}
		// putting everything together
		$additionalWhere = $categorySearchString.$calendarSearchString." tx_cal_event.uid=".$uid." AND tx_cal_event.hidden = 0 AND tx_cal_event.deleted = 0";
		$events = $this->getEventsFromTable($categories, true, $additionalWhere);

		foreach($events as $date=>$time){
			foreach($time as $eventArray){
				foreach($eventArray as $event){
					if($event->getStartDate()<=strtotime($this->cObj->conf['getdate']) && $event->getEndDate()>=strtotime($this->cObj->conf['getdate']) && $event->getUid==$uid){
						return $event;
					}
				}
			}
		...
		if(empty($events))
			return;
		if($events[$this->cObj->conf['getdate']]){
			$event = array_pop(array_pop($events[$this->cObj->conf['getdate']]));
			return $event;
		}else{
...
//		}
		...
		if($this->rightsObj->isAllowedToCreateEventException()){
			if($this->controller->piVars['single_exception_ids']!=''){
				$this->insertIdsIntoTableWithMMRelation("tx_cal_exception_event_mm",split(',', $this->controller->piVars['single_exception_ids']),$uid,"tx_cal_exception_event");
			}
			if($this->controller->piVars['group_exception_ids']!=''){
				$this->insertIdsIntoTableWithMMRelation("tx_cal_exception_event_group_mm",split(',', $this->controller->piVars['group_exception_ids']),$uid,"tx_cal_exception_event_group");
			}
		}
		if($this->rightsObj->isAllowedToCreateEventCategory()){
			$this->insertIdsIntoTableWithMMRelation("tx_cal_event_category_mm",split(",",$this->controller->piVars['category_ids']),$uid,"");
		}else{
			$this->insertIdsIntoTableWithMMRelation("tx_cal_event_category_mm",array($this->cObj->conf['rights.']['create.']['event.']['fields']['allowedToCreateCategory.']['uidDefault']),$uid,"");
		}
		
		$this->_notify($uid,$insertFields);
		$this->unsetPiVars();
...
//		}
		if($this->rightsObj->isAllowedToEditEventCategory()){
			...
			$this->insertIdsIntoTableWithMMRelation($table,split(',', $this->controller->piVars['category_ids']),$uid,"");
		}
...
//			}
//			else{
//				$this->insertIdsIntoTableWithMMRelation("tx_cal_fe_user_category_mm",array($this->cObj->conf['anonymousUserUid']),$uid,"fe_users");
//			}
//		}
	...
	function removeEvent($uid){
		if($this->rightsObj->isAllowedToDeleteEvents()){
			...
			//TODO: $this->_notify($uid,$insertFields);
			$this->unsetPiVars();
			require_once(t3lib_extMgm::extPath('cal').'controller/class.tx_cal_functions.php');
			clearCache();
		}
...
//		}
		if($this->rightsObj->isAllowedToEditEventCalendar() || $this->rightsObj->isAllowedToCreateEventCalendar()){
			if($this->controller->piVars['calendar_id']!=""){
				$insertFields['calendar_id'] = $this->controller->piVars['calendar_id'];
			}else{
	...
	function search($pidList=''){
		$service = &$this->getCategoryService();
		$categoryIds = $this->arrayToCommaseparatedString($service->getCategoryArray());
		$sw = $this->controller->piVars['query'];
		$start_day = $this->controller->piVars['start_day'];
		$end_day = $this->controller->piVars['end_day'];
		...
		if($this->controller->piVars['location_ids']){
			$locationSearchString = " AND location_id in (".$this->controller->piVars['location_ids'].")";
		}

		// putting everything together
		$additionalWhere = $categorySearchString.$calendarSearchString.$startDaySearchString.$endDaySearchString.$locationSearchString." AND tx_cal_event.pid IN (".$pidList.") AND tx_cal_event.hidden = 0 AND tx_cal_event.deleted = 0 ".$additionalSearch;
		return $this->getEventsFromTable($service->getCategoryArray(), true, $additionalWhere);
	}
	...
	function recurringEvent($event){
		...
		$current_view = $this->cObj->conf['view'];
		...
		if (isset($start_time) && $start_time != '') {
			...
			$drawKey = drawEventTimes($start_time, $end_time, $this->cObj->conf['view.']['day.']['gridLength']);
			$time3 = array(); 
...
//						$minute = "";
		...
		if (isset($rrule_array) && is_array($rrule_array)) {
			...
			if ($current_view == 'year'){// || ($save_parsed_cals == 'yes' && !$is_webcal)) {
				...
				$start_range_time = strtotime($this->cObj->conf["view."]["list."]['starttime']);
				$end_range_time = strtotime($this->cObj->conf["view."]["list."]['endtime']);
			} else {
			...
			if ($end_range_time_tmp >= $start_date_time && $start_range_time_tmp <= $end_date_time) {
				...
				while (($next_range_time >= $start_range_time_tmp) && ($next_range_time <= $end_range_time_tmp) && ($count_to != $count)) {
					...
					$diff = tx_cal_calendar::$func($next_range_time, $start_date, $this->cObj->conf["view."]['weekStartDay']);
					if ($diff < $count) {
						if ($diff % $number == 0) {
							$interval = $number;
								...
								case 'week':
									...
									if (is_array($byday)) {
										foreach($byday as $day) {
											...
											if ($next_date_time > strtotime($this->cObj->conf["view."]['weekStartDay'], $next_range_time) && $interval > 1) {
	...
	function saveExceptionEvent($pid){
		...
		if($this->controller->piVars['exception_title']!=''){
			$insertFields['title'] = $this->controller->piVars['exception_title'];
		}

		// Creating DB records
		$insertFields['cruser_id'] = $this->rightsObj->getUserId();
		if($insertFields['title']==""){
			$insertFields['title'] = $this->controller->piVars['exception_start_day']." exception";
		}
		...
		$this->insertIdsIntoTableWithMMRelation("tx_cal_exception_event_mm",array($uid),$this->controller->piVars['event_uid'],"tx_cal_exception_event");
		$this->unsetPiVars();
		require_once(t3lib_extMgm::extPath('cal').'controller/class.tx_cal_functions.php');
		clearCache();
	}
