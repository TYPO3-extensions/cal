...
class tx_cal_location_address_service extends tx_cal_base_service {
	...
	function find($uid, $pidList){
		if($pidList==""){
			$result = $GLOBALS['TYPO3_DB']->exec_SELECTquery("*", "tt_address", " hidden = 0 AND deleted = 0 AND tx_cal_controller_islocation=1 AND uid=".$uid);
		}else{
			$result = $GLOBALS['TYPO3_DB']->exec_SELECTquery("*", "tt_address", " pid IN (".$pidList.") AND hidden = 0 AND deleted = 0 AND tx_cal_controller_islocation=1 AND uid=".$uid);
		}
		while($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($result)){
			$tx_cal_location_address = t3lib_div::makeInstanceClassName('tx_cal_location_address');
			return new $tx_cal_location_address($this->cObj, $this->rightsObj, $row, $pidList);
		}
	}
	...
	function findAll($pidList){
		$locations = array();
		if($pidList==""){
			$result = $GLOBALS['TYPO3_DB']->exec_SELECTquery("*", "tt_address", " hidden = 0 AND deleted = 0 AND tx_cal_controller_islocation=1 ");
		}else{
			$result = $GLOBALS['TYPO3_DB']->exec_SELECTquery("*", "tt_address", " pid IN (".$pidList.") AND hidden = 0 AND deleted = 0 AND tx_cal_controller_islocation=1 ");
		}
		while($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($result)){
	...
	function search($pidList=''){
		...
		if($sw!=""){
			$result = $GLOBALS['TYPO3_DB']->exec_SELECTquery("*", "tt_address", " pid IN (".$pidList.") AND hidden = 0 AND deleted = 0 AND tx_cal_controller_islocation = 1 ".$this->searchWhere($sw));
			while($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($result)){
