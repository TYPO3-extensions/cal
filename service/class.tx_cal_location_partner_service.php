...
class tx_cal_location_partner_service extends tx_cal_base_service {
 	...
 	function setCObj(&$cObj){
		...
		$useLocationStructure = ($this->cObj->confArr['useLocationStructure']?$this->cObj->confArr['useLocationStructure']:'tx_cal_organizer');
	...
	function find($uid, $pidList){
		...
		while($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($result)){
			...
			return new $tx_cal_location_partner($this->cObj, $this->rightsObj, $row['uid'], $pidList);
		}	
	}
	...
	function findAll($pidList){
		...
		while($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($result)){
			...
			$locations[] = new $tx_cal_location($this->cObj, $this->rightsObj, $row['uid'], $pidList);
		}
		return $locations;
	}
	...
	function search($pidList=''){
		...
		if($sw!=""){
			...
			while($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($result)){
				$locationArray[] = new $location($this->cObj, $this->rightsObj, $row['uid'], $pidList);
			}
		}
	...
	function searchWhere($sw) {
		$where = $this->cObj->searchWhere($sw, $this->cObj->conf["view."]["search."]["searchLocationFieldList"], 'tx_partner_main');
		return $where;
	}
