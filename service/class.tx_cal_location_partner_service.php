 ...
 * *************************************************************
 */
require_once (\TYPO3\CMS\Core\Utility\ExtensionManagementUtility::extPath ('cal') . 'model/class.tx_cal_location_partner.php');
...
class tx_cal_location_partner_service extends tx_cal_base_service {
	...
	function tx_cal_location_partner_service() {
	...
	function getLocationFromTable($pidList = '', $additionalWhere = '') {
		$locations = array ();
		$orderBy = tx_cal_functions::getOrderBy ($this->keyId);
		if ($pidList != '') {
		...
		$hookObjectsArr = tx_cal_functions::getHookObjectsArray ('tx_cal_location_partner_service', 'locationServiceClass', 'service');
