...
class tx_cal_location_partner_service extends tx_cal_base_service {
	...
	function find($uid, $pidList){
		...
		while($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($result)){
			...
			return new $tx_cal_location_partner($this->controller, $row['uid'], $pidList);
		}	
	}
	...
	function findAll($pidList){
		...
		while($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($result)){
			...
			$locations[] = new $tx_cal_location($this->controller, $row['uid'], $pidList);
		}
		return $locations;
	}
	...
	function search($pidList=''){
		...
		if($sw!=''){
			...
			while($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($result)){
				$locationArray[] = new $location($this->controller, $row['uid'], $pidList);
			}
		}
