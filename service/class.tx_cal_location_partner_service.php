...
class tx_cal_location_partner_service extends tx_cal_base_service {
	...
	function find($uid, $pidList){
		...
		if($result) {
			...
			return tx_cal_functions::makeInstance('tx_cal_location_partner',$row['uid'], $pidList);
		}
		return;
	}
	...
	function search($pidList=''){
		...
		if($sw!=''){
			...
			if($result) {
				while($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($result)){
					$locationArray[] = &tx_cal_functions::makeInstance('tx_cal_location',$row['uid'], $pidList);
				}
				$GLOBALS['TYPO3_DB']->sql_free_result($result);
			}
		}
