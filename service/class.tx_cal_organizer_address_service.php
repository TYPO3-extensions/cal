...
class tx_cal_organizer_address_service extends tx_cal_base_service {
	...
	function find($uid, $pidList){
		...
		if($pidList==''){
			$result = $GLOBALS['TYPO3_DB']->exec_SELECTquery('*', 'tt_address', ' tx_cal_controller_isorganizer = 1 AND uid='.$uid.' '.$this->cObj->enableFields('tt_address'));
		}else{
			$result = $GLOBALS['TYPO3_DB']->exec_SELECTquery('*', 'tt_address', ' pid IN ('.$pidList.') AND tx_cal_controller_isorganizer = 1 AND uid='.$uid.' '.$this->cObj->enableFields('tt_address'));
		}
		while($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($result)){
			return tx_cal_functions::makeInstance('tx_cal_organizer_address',$row, $pidList);
		}
	...
	function findAll($pidList){
		...
		if($pidList==''){
			$result = $GLOBALS['TYPO3_DB']->exec_SELECTquery('*', 'tt_address', ' tx_cal_controller_isorganizer = 1 '.$this->cObj->enableFields('tt_address'), '', $orderBy);
		}else{
			$result = $GLOBALS['TYPO3_DB']->exec_SELECTquery('*', 'tt_address', ' pid IN ('.$pidList.') AND tx_cal_controller_isorganizer = 1 '.$this->cObj->enableFields('tt_address'), '', $orderBy);
		}
		while($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($result)){
			$organizer[] = &tx_cal_functions::makeInstance('tx_cal_organizer_address',$row, $pidList);
		}
		$GLOBALS['TYPO3_DB']->sql_free_result($result);
	...
	function search($pidList=''){
		...
		if($sw!=''){
			$result = $GLOBALS['TYPO3_DB']->exec_SELECTquery('*', 'tt_address', ' pid IN ('.$pidList.') AND tx_cal_controller_isorganizer = 1 '.$this->cObj->enableFields('tt_address').$this->searchWhere($sw));
			while($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($result)){
				$organizerArray[] = &tx_cal_functions::makeInstance('tx_cal_organizer_address',$row, $pidList);
			}
			$GLOBALS['TYPO3_DB']->sql_free_result($result);
		}
