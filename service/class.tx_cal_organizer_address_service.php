...
class tx_cal_organizer_address_service extends tx_cal_base_service {
	...
	function find($uid, $pidList){
		...
		if($pidList==''){
			$result = $GLOBALS['TYPO3_DB']->exec_SELECTquery('*', 'tt_address', ' AND tx_cal_controller_isorganizer = 1 AND uid='.$uid.' '.$this->cObj->enableFields('tt_address'));
		}else{
	...
	function findAll($pidList){
		...
		$organizer = array();
		if($pidList==''){
			$result = $GLOBALS['TYPO3_DB']->exec_SELECTquery('*', 'tt_address', ' tx_cal_controller_isorganizer = 1 '.$this->cObj->enableFields('tt_address'));
		}else{
			$result = $GLOBALS['TYPO3_DB']->exec_SELECTquery('*', 'tt_address', ' pid IN ('.$pidList.') AND tx_cal_controller_isorganizer = 1 '.$this->cObj->enableFields('tt_address'));
		}
		while($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($result)){
			$tx_cal_organizer_address = t3lib_div::makeInstanceClassName('tx_cal_organizer_address');
