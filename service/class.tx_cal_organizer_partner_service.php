...
class tx_cal_organizer_partner_service extends tx_cal_base_service {
	...
	function find($uid, $pidList){
		...
		while($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($result)){
			...
			return new $tx_cal_organizer_partner($this->controller, $row['uid'], $pidList);
		}
	}
	...
	function findAll($pidList){
		...
		$organizer = array();
		if($pidList==''){
			$result = $GLOBALS['TYPO3_DB']->exec_SELECTquery('uid', 'tx_partner_main', ' 0=0 '.$this->cObj->enableFields('tx_partner_main'));
		}else{
			$result = $GLOBALS['TYPO3_DB']->exec_SELECTquery('uid', 'tx_partner_main', ' pid IN ('.$pidList.') '.$this->cObj->enableFields('tx_partner_main'));
		}
		while($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($result)){
			$tx_cal_organizer = t3lib_div::makeInstanceClassName('tx_cal_organizer_partner');
			$organizer[] = new $tx_cal_organizer($this->controller, $row['uid'], $pidList);
		}
		return $organizer;
	}
	...
	function search($pidList=''){
		...
		if($sw!=''){
			...
			while($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($result)){
				$organizerArray[] = new $organizer($this->controller, $row['uid'], $pidList);
			}
		}
