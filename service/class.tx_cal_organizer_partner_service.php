...
class tx_cal_organizer_partner_service extends tx_cal_base_service {
 	...
 	function tx_cal_organizer_partner_service($controller){
	...
	function find($uid, $pidList){
		...
		while($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($result)){
			...
			return new $tx_cal_organizer_partner('', $row['uid'], $pidList);
		}
	}
	...
	function findAll($pidList){
		...
		while($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($result)){
			...
			$organizer[] = new $tx_cal_organizer('', $row['uid'], $pidList);
		}
		return $organizer;
	}
	...
	function search($pidList=''){
		...
		if($sw!=''){
			...
			while($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($result)){
				$organizerArray[] = new $organizer('', $row['uid'], $pidList);
			}
		}
