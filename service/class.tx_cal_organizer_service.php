...
class tx_cal_organizer_service extends tx_cal_base_service {
	...
	function find($uid, $pidList){
		if($pidList==""){
			$result = $GLOBALS['TYPO3_DB']->exec_SELECTquery("*", "tx_cal_organizer", " hidden = 0 AND deleted = 0 AND uid=".$uid);
		}else{
			$result = $GLOBALS['TYPO3_DB']->exec_SELECTquery("*", "tx_cal_organizer", " pid IN (".$pidList.") AND hidden = 0 AND deleted = 0 AND uid=".$uid);
		}
		while($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($result)){
			$tx_cal_organizer = t3lib_div::makeInstanceClassName('tx_cal_organizer');
			return new $tx_cal_organizer($this->cObj, $this->rightsObj, $row, $pidList);
		}
	}
	...
	function findAll($pidList){
		$organizer = array();
		if($pidList==""){
			$result = $GLOBALS['TYPO3_DB']->exec_SELECTquery("*", "tx_cal_organizer", " hidden = 0 AND deleted = 0 ");
		}else{
			$result = $GLOBALS['TYPO3_DB']->exec_SELECTquery("*", "tx_cal_organizer", " pid IN (".$pidList.") AND hidden = 0 AND deleted = 0 ");
		}
		while($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($result)){
			$tx_cal_organizer = t3lib_div::makeInstanceClassName('tx_cal_organizer');
			$organizer[] = new $tx_cal_organizer($this->cObj, $this->rightsObj, $row, $pidList);
		}
		return $organizer;
	}
	...
	function search($pidList=''){
		...
		$organizer = t3lib_div::makeInstanceClassName('tx_cal_organizer');
		if($sw!=""){
			$result = $GLOBALS['TYPO3_DB']->exec_SELECTquery("*", "tx_cal_organizer", " pid IN (".$pidList.") AND hidden = 0 AND deleted = 0 ".$this->searchWhere($sw));
			while($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($result)){
				$organizerArray[] = new $organizer($this->cObj, $row, $pidList);
			}
		}
	...
	function searchWhere($sw) {
		$where = $this->cObj->searchWhere($sw, $this->cObj->conf["view."]["search."]["searchOrganizerFieldList"], 'tx_cal_organizer');
		return $where;
	}
