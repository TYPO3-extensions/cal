...
class tx_cal_organizer_service extends tx_cal_base_service {
	...
	function find($uid, $pidList){
		...
		if($pidList==''){
			$result = $GLOBALS['TYPO3_DB']->exec_SELECTquery('*', 'tx_cal_organizer', ' uid='.$uid.' '.$this->cObj->enableFields('tx_cal_organizer'));
		}else{
			$result = $GLOBALS['TYPO3_DB']->exec_SELECTquery('*', 'tx_cal_organizer', ' pid IN ('.$pidList.') AND uid='.$uid.' '.$this->cObj->enableFields('tx_cal_organizer'));
		}
		while($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($result)){
			$tx_cal_organizer = t3lib_div::makeInstanceClassName('tx_cal_organizer');
			return new $tx_cal_organizer('', $row, $pidList);
		}
	}
	...
	function findAll($pidList){
		...
		if($pidList==''){
			$result = $GLOBALS['TYPO3_DB']->exec_SELECTquery('*', 'tx_cal_organizer', ' 0=0 '.$this->cObj->enableFields('tx_cal_organizer'), '', $orderBy);
		}else{
			$result = $GLOBALS['TYPO3_DB']->exec_SELECTquery('*', 'tx_cal_organizer', ' pid IN ('.$pidList.') '.$this->cObj->enableFields('tx_cal_organizer'), '', $orderBy);
		}
		while($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($result)){
			$tx_cal_organizer = t3lib_div::makeInstanceClassName('tx_cal_organizer');
			$organizer[] = new $tx_cal_organizer('', $row, $pidList);
		}
		return $organizer;
	}
	...
	function search($pidList=''){
		...
		if($sw!=''){
			$result = $GLOBALS['TYPO3_DB']->exec_SELECTquery('*', 'tx_cal_organizer', ' pid IN ('.$pidList.') '.$this->cObj->enableFields('tx_cal_organizer').$this->searchWhere($sw));
			while($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($result)){
	...
	function retrievePostData(&$insertFields){
		...
		if($this->rightsObj->isAllowedTo('edit','organizer','image') || $this->rightsObj->isAllowedTo('create','organizer','image')){
