...
class tx_cal_organizer_service extends tx_cal_base_service {
	...
	function find($uid, $pidList){
		...
		while($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($result)){
			...
			return new $tx_cal_organizer($this->controller, $row, $pidList);
		}
	}
	...
	function findAll($pidList){
		...
		$organizer = array();
		if($pidList==''){
			$result = $GLOBALS['TYPO3_DB']->exec_SELECTquery('*', 'tx_cal_organizer', ' 0=0 '.$this->cObj->enableFields('tx_cal_organizer'));
		}else{
			$result = $GLOBALS['TYPO3_DB']->exec_SELECTquery('*', 'tx_cal_organizer', ' pid IN ('.$pidList.') '.$this->cObj->enableFields('tx_cal_organizer'));
		}
		while($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($result)){
			$tx_cal_organizer = t3lib_div::makeInstanceClassName('tx_cal_organizer');
			$organizer[] = new $tx_cal_organizer($this->controller, $row, $pidList);
		}
		return $organizer;
	}
