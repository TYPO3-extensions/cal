...
/* If we haven't already defined a userfunc for an empty wrapper, then do it */
if(!class_exists('tx_cal_tcauserfunc')) {
	class tx_cal_tcauserfunc {
		function emptyUserFunc($PA, $fobj) {
			return '';
		}
	}
}
...
$TCA['tx_cal_event'] = Array (
	...
	'columns' => Array (
		...
		'startwrapper' => Array (
			'label' => 'LLL:EXT:cal/locallang_db.php:tx_cal_event.start',
			'config' => Array (
				'type' => 'user',
				'userFunc' => 'tx_cal_tcauserfunc->emptyUserFunc',
			),
		),
		'start_date' => Array (		
			'exclude' => 1,	
		...
		'endwrapper' => Array (
			'label' => 'LLL:EXT:cal/locallang_db.php:tx_cal_event.end',
			'config' => Array (
				'type' => 'user',
				'userFunc' => 'tx_cal_tcauserfunc->emptyUserFunc',
			),
	...
	'types' => Array (
		'0' => Array('showitem' => 'type, title;;1;;,startwrapper;;5,endwrapper;;6,calendar_id,category_id,'.($useTeaser ? 'teaser,' : '').'richtext:rte_transform[flag=rte_enabled|mode=ts_css],description;;5;richtext:rte_transform[flag=rte_enabled|mode=ts_css],--div--;Recurrence, freq;;2;;, byday, bymonthday, bymonth,--div--;Location,'.($hideLocationTextfield?'location_id,location_pid':'location,location_id,location_pid').',--div--;Organizer,'.($hideOrganizerTextfield?'organizer_id,organizer_pid':'organizer,organizer_id,organizer_pid').',--div--;Files,image;;4;;;,imagecaption,attachment,--div--;Other, monitor_cnt, ex_freq, ex_byday, ex_bymonthday, ex_bymonth, ex_until,exception_cnt, shared_user_cnt'),
		'1' => Array('showitem' => 'type, title;;1;;,startwrapper;;5,endwrapper;;6,calendar_id,category_id,'.($hideLocationTextfield?'location_id,location_pid':'location,location_id,location_pid').','.($hideOrganizerTextfield?'organizer_id,organizer_pid':'organizer,organizer_id,organizer_pid').', page,--div--;Recurrence, freq;;2;;, byday, bymonthday, bymonth, --div--;Monitor, monitor_cnt, ex_freq, ex_byday, ex_bymonthday, ex_bymonth, ex_until,exception_cnt shared_user_cnt'),
		'2' => Array('showitem' => 'type, title;;1;;,startwrapper;;5,endwrapper;;6,calendar_id,category_id,'.($hideLocationTextfield?'location_id,location_pid':'location,location_id,location_pid').','.($hideOrganizerTextfield?'organizer_id,organizer_pid':'organizer,organizer_id,organizer_pid').', ext_url,--div--;Recurrence, freq;;2;;, byday, bymonthday, bymonth, --div--;Monitor, monitor_cnt, ex_freq, ex_byday, ex_bymonthday, ex_bymonth, ex_until,exception_cnt, shared_user_cnt'),
	),
	'palettes' => Array (
...
$TCA['tx_cal_category'] = Array (
	...
	'interface' => Array (
		'showRecordFieldList' => 'hidden,title,headerstyle,bodystyle,shared_user_allowed'
	),
	'columns' => Array (
	...
	'types' => Array (
		'0' => Array('showitem' => 'type,title;;1;;,calendar_id,shared_user_allowed'),
	),
	'palettes' => Array (
...
if($confArr['nestedCategories']) {
	...
	$TCA['tx_cal_category']['types'][0] = Array('showitem' => 'type,title;;1;;,calendar_id,parent_category,shared_user_allowed');
}
