...
$TCA['tx_cal_event'] = Array (
	...
	'interface' => Array (
		'showRecordFieldList' => 'hidden,category_id,title,start_date,start_time,end_date,end_time,organizer,location,description,image,imagecaption,attachment,freq,byday,bymonthday,bymonth,until,count,end,intrval,ex_freq, ex_byday, ex_bymonthday, ex_bymonth, ex_until,exception_cnt, shared_user_cnt'
	),
...
//				),
	...
	'types' => Array (
		'0' => Array('showitem' => 'type, title;;1;;,calendar_id,category_id,'.($hideLocationTextfield?'location_id':'location;;2;;').','.($hideOrganizerTextfield?'organizer_id':'organizer;;3;;').',start_date, start_time,end_date,end_time, description;;5;richtext[paste|bold|italic|underline|formatblock|class|left|center|right|orderedlist|unorderedlist|outdent|indent|link|table|image]:rte_transform[flag=rte_enabled|mode=ts],image,imagecaption,attachment;;4;;;--div--;Recurrence, freq, byday, bymonthday, bymonth, until, cnt, intrval, monitor_cnt, ex_freq, ex_byday, ex_bymonthday, ex_bymonth, ex_until,exception_cnt, shared_user_cnt'),
...
if(is_array($GLOBALS['HTTP_POST_VARS']['data']['tx_cal_event'])){
	$TCA['tx_cal_event']['columns']['category_id']['config']['foreign_table_where'] = 'AND tx_cal_category.calendar_id IN (###REC_FIELD_calendar_id###,0)';
}else{
