...
class tx_cal_adminview extends tx_cal_base_view {
	...
	function drawAdminPage() {
		...
		$params = array ( 'view' => 'edit_event', 'type' => 'tx_cal_phpicalendar');
		foreach($params as $key => $value){
			$editEventParams .= '<input type="hidden" value="'.$value.'" name="'.$this->prefixId.'['.$key.']"/>';
		}
		
		$params = array ( 'view' => 'delete_event', 'type' => 'tx_cal_phpicalendar');
		foreach($params as $key => $value){
	...
	public function getCalendarSubscriptionUrlMarker($page, &$sims, &$rems, &$wrapped){
		$sims['###CALENDAR_SUBSCRIPTION_URL###'] = $this->controller->pi_linkTP_keepPIvars_url(array ( 'view' => 'admin'), 0);
	}
	...
	private function checkAction(){
		switch($this->controller->piVars['adminAction']){
			case 'editCalendarSubscription':
				...
				$fields = Array('tx_cal_calendar_subscription'=>'');
