...
class tx_cal_base_view extends tx_cal_base_service {
	...
	function getUserLoginMarker(&$page, &$sims, &$rems, &$wrapped){
		...
		if ($this->conf['view.']['other.']['showLogin'] == 1) {
			...
			$rems['###USER_LOGIN###'] = substituteMarkerArrayNotCached($this->cObj->getSubpart($page,'###USER_LOGIN###'), $local_sims, $local_rems, array());
		}
	}
	...
	function getSearchMarker(&$page, &$sims, &$rems, &$wrapped){
		...
		if ($this->conf['view.']['other.']['showSearch'] == 1) {
			...
			$rems['###SEARCH###'] = substituteMarkerArrayNotCached($this->cObj->getSubpart($page,'###SEARCH###'), $local_sims, array(), array());
		}
	}
	...
	function getJumpsMarker(&$page, &$sims, &$rems, &$wrapped){
		...
		if ($this->conf['view.']['other.']['showJumps'] == 1) {
			...
			$rems['###JUMPS###'] = substituteMarkerArrayNotCached($this->cObj->getSubpart($page, '###JUMPS###'), $temp_sims, array(), array());
		}
	}
	...
	function getCalendarSelectorMarker(&$page, &$sims, &$rems, &$wrapped){
		...
		if ($this->conf['view.']['other.']['showCalendarSelector']) {
			...
			$rems['###CALENDAR_SELECTOR###'] = substituteMarkerArrayNotCached($this->cObj->getSubpart($page, '###CALENDAR_SELECTOR###'), $temp_sims, array(), array());
		}
	}
	...
	function getBackLinkMarker(&$page, &$sims, &$rems, &$wrapped){
		...
		if($this->rightsObj->isViewEnabled($viewParams['view']) && $this->conf['view'] != $viewParams['view']){
			$this->initLocalCObject();
	...
	function getListMarker(&$page, &$sims, &$rems, &$wrapped){
		...
		if ($this->conf['view']=='month' && $this->conf['view.']['month.']['showListInMonthView']) {
			...
			$endtime = tx_cal_calendar::calculateEndMonthTime($starttime+235900);
			...
			$endtime = tx_cal_calendar::calculateEndDayTime($starttime+235900);
			$rems['###LIST###'] = $tx_cal_listview->drawList($this->master_array,$listSubpart,$starttime,$endtime);
		}else if($this->conf['view']=='week'){
			$starttime = tx_cal_calendar::calculateStartWeekTime($starttime);
			$endtime = tx_cal_calendar::calculateEndWeekTime($starttime+235900);
			$rems['###LIST###'] = $tx_cal_listview->drawList($this->master_array,$listSubpart,$starttime,$endtime);
		}else if($this->conf['view']=='year'){
			$starttime = tx_cal_calendar::calculateStartYearTime($starttime);
			$endtime = tx_cal_calendar::calculateEndYearTime($starttime+235900);
			$rems['###LIST###'] = $tx_cal_listview->drawList($this->master_array,$listSubpart,$starttime,$endtime);
		}
...
//debug($cal_time_obj->format('%Y%m%d %H%M').' after'.$now->format('%Y%m%d %H%M'),'hier:'.$createOffset);
	...
	function getCategoryurlMarker(&$page, &$sims, &$rems, $view){
		$sims['###CATEGORYURL###'] = $this->controller->pi_linkTP_keepPIvars_url(array('view'=>$this->conf['view'],'offset'=>null));
	}
	...
	function finish(&$page, &$rems){
		...
		$page = substituteMarkerArrayNotCached($page, $sims, $rems, $wrapped);
		$sims = array();
		$rems = array();
		$this->getImgPathMarker($page, $sims, $rems, $this->conf['view']);
		return substituteMarkerArrayNotCached($page, $sims, $rems, $wrapped);
	}
	...
	function checkForMonthMarker($page) {
		...
		$languageArray = array (
			...
			'prev_year' => $prev_year,
/*
* these are automatically fetched now
...
*/
	...
	function addSubCategory(&$treeConf, &$parentCategoryArray, &$parentCategory, $level, $renderAsForm){
		...
		$sims['###LEVEL###'] = $level;
		$treeHtml = substituteMarkerArrayNotCached($treeHtml, $sims, $rems, $wrapper);
		...
		if(is_array($categoryArray)){
			...
			$sims['###LEVEL###'] = $level;
			$treeHtml .= substituteMarkerArrayNotCached($tempHtml, $sims, $rems, $wrapper);
	...
	function tomorrows_events($template) {
		...
		if (is_array($this->master_array[$next_day]) && sizeof($this->master_array[$next_day]) > 0) {
			...
			return substituteMarkerArrayNotCached($template, array (), $rems, array ());
		}
	...
	function _draw_month($page, $offset = '+0', $type) {
		...
		if($monthTemplate!=''){
			...
			do {
				...
				$middle .= substituteMarkerArrayNotCached($temp, $switch, array(), $wraped);
			...
			$rems['###LOOPWEEK_NUMS###'] = '';
			$monthTemplate = substituteMarkerArrayNotCached($monthTemplate, array (), $rems, array ());
			$page = substituteMarkerArrayNotCached($page, array(), array ('###MONTH_TEMPLATE###'=>$monthTemplate), array ());
		}
		...
		$return = substituteMarkerArrayNotCached($page, array (), $rems, array ());
