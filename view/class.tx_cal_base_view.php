...
class tx_cal_base_view extends tx_cal_base_service {
	...
	function getListMarker(&$page, &$sims, &$rems, &$wrapped){
		...
		if ($this->conf['view']=='month' && $this->conf['view.']['month.']['showListInMonthView']) {
			...
			$starttime = tx_cal_calendar::calculateStartWeekTime($starttime, $this->conf['view.']['weekStartDay']);
			$endtime = tx_cal_calendar::calculateEndWeekTime($starttime, $this->conf['view.']['weekStartDay']);
			$rems['###LIST###'] = $tx_cal_listview->drawList($this->master_array,$listSubpart,$starttime,$endtime);
...
//debug($cal_time_obj->format('%Y%m%d %H%M').' after'.$now->format('%Y%m%d %H%M'),'hier:'.$createOffset);
	...
	function checkForMonthMarker($page) {
		...
		$start_month_day = tx_cal_calendar::calculateStartWeekTime($start_month_day,$this->conf['view.']['weekStartDay']);
	...
	function _draw_month($page, $offset = '+0', $type) {
		...
		if($monthTemplate!=''){
			...
			for ($i = 0; $i < 7; $i ++) {
				...
				$loop_tmp = str_replace('###ADDITIONAL_CLASSES###', ' cal_day'.$start_day->format('%w'), $loop_tmp);
			...
			do {
				...
				$style .= 'cal_day'.$start_day->format('%w').' '; 
