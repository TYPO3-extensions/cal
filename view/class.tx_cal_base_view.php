...
//			$rems["###SHOW_USER_LOGIN###"] =  $this->cObj->getSubpart($page, "###SHOW_USER_LOGIN###");
	...
	function checkForMonthMarker($page) {
		...
		$thisMonthsEvents = $this->cObj->conf["view."]["month."]["thisMonthsEvents"];
		if ($thisMonthsEvents == 1) {
			require_once ('class.tx_cal_listview.php');
			$tx_cal_listview = t3lib_div :: makeInstanceClassName('tx_cal_listview');
			$tx_cal_listview = new $tx_cal_listview($this->cObj);
						...
						if ($val->getStartHour() == '') {
							...
							if ($type == 'large') {
								...
								if($this->cObj->conf['view.']['freeAndBusy.']['enable'] && !$val->isEventOwner($this->rightsObj->getUserId(),$this->rightsObj->getUserGroups()) && $this->cObj->conf['option']=='freeandbusy' && $this->cObj->conf['calendar']){
									...
									$switch['EVENT'] .= $this->getLinkToEvent($val, $val->renderEventForMonth(),$this->cObj->conf['view'], $daylink).'<br />';
									$switch['EVENT'] .= '</span></div>';
								}
							}else if ($type == 'small'){// && !$isAllowedToCreateEvents){
								$switch['LINK'] = '<span class="bold_link">'.$switch['LINK'].'</span>';
								...
								$switch['EVENT'] .= $this->getLinkToEvent($val, $eventtext,$this->cObj->conf['view'], $daylink).'<br />';
							}
						}
					}
				}
