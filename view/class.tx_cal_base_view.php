...
//			$rems['###TOMORROWS_EVENTS###'] =  $this->cObj->getSubpart($page, '###TOMORROWS_EVENTS###');
	...
	function showList(&$page, &$rems, &$sims){
		...
		if ($this->conf['view.']['month.']['thisMonthsEvents']) {
			...
			$rems['###SHOWBOTTOMEVENTS###'] = $tx_cal_listview->drawList($this->master_array,'',$starttime,$endtime);
		...
		foreach($allMarkers as $marker) {
			switch($marker) {
				...
				case 'IMG_PATH':
					...
					break;
				case 'CATEGORYURL':
					$sims['###CATEGORYURL###'] = $this->controller->pi_linkTP_keepPIvars_url();
					break;
			}
		}
	...
	function list_legend(&$return) {
		...
		$this->conf['view.']['category.']['tree.']['calendar'] = $this->conf['calendar'];
...
//		$first_of_year = $minical_year.'0101';
		...
		do {
			...
			if ($daylink>=gmdate('Ymd') && $isAllowedToCreateEvents && $start_day+86400>(time()+$createOffset)) {
				$GLOBALS['TSFE']->ATagParams = 'title="'.$this->controller->pi_getLL('l_create_event').'"';
				$switch['###LINK###'] = $this->controller->pi_linkTP_keepPIvars($this->conf['view.']['week.']['addIcon'], array ('getdate' => $daylink, 'lastview' => $this->controller->extendLastView(), 'view' => 'create_event'), $this->conf['cache'], $this->conf['clear_anyway'], $this->conf['view.']['event.']['createEventViewPid']);
			} else {
				$switch['###LINK###'] = '';
			}
			$GLOBALS['TSFE']->ATagParams = 'title="'.$this->controller->pi_getLL('l_'.$this->conf['view.']['dayLinkTarget'].'_view').'"';
			...
			if(($this->rightsObj->isViewEnabled($this->conf['view.']['dayLinkTarget']) || $this->conf['view.'][$this->conf['view.']['dayLinkTarget'].'.'][$this->conf['view.']['dayLinkTarget'].'ViewPid']) && ($this->viewarray[$daylink] || $isAllowedToCreateEvents)){
				$switch['###LINK###'] .= $this->controller->pi_linkTP_keepPIvars(gmdate('j', $start_day), array ('getdate' => $daylink, 'view' => $this->conf['view.']['dayLinkTarget'], 'lastview' => $this->controller->extendLastView()), $this->conf['cache'], $this->conf['clear_anyway'], $this->conf['view.']['day.']['dayViewPid']);
			}else{
