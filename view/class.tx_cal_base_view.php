...
class tx_cal_base_view extends tx_cal_base_service {
	...
	function getIcsLinkMarker(&$page, &$sims, &$rems, $view){
		...
		if ($this->conf['view.']['ics.']['showIcsLinks'] == 1) {
			...
			$this->controller->getParametersForTyposcriptLink($this->local_cObj->data, array ('view' => 'icslist',  $this->pointerName => NULL), $this->conf['cache'], 1);
...
*/
	...
	function getCalendarSelectorMarker(&$page, &$sims, &$rems, &$wrapped){
		...
		if ($this->conf['view.']['other.']['showCalendarSelector']) {
			...
			$calendarArray = $calendarService->findAll($this->conf['pidList']);
			if(is_array($calendarArray)){
				...
				foreach($calendarArray as $calendar){
					if($this->conf['calendar']==$calendar->row['uid']){
						...
						$calendarOptions .= '<option value="'.$calendarRow->row['uid'].'">'.$calendar->getTitle().'</option>';
					}
				}
			}
...
//debug($cal_time_obj->format('%Y%m%d %H%M').' after'.$now->format('%Y%m%d %H%M'),'hier:'.$createOffset);
		if (($cal_time_obj->after($now) || $this->rightsObj->isAllowedToCreateEventInPast()) && $isAllowedToCreateEvent) {
			...
			if($this->conf['view.']['enableAjax']){
				...
				$this->local_cObj->data['link_no_cache'] = 0;
	...
	function getCategoryurlMarker(&$page, &$sims, &$rems, $view){
		$sims['###CATEGORYURL###'] = htmlspecialchars($this->controller->pi_linkTP_keepPIvars_url(array('view'=>$this->conf['view'], $this->pointerName => NULL),$this->conf['cache'],$this->conf['clear_anyway']));
	}
	...
	function checkForMonthMarker($page) {
		...
		$this->controller->getParametersForTyposcriptLink($this->local_cObj->data, array ('getdate' => $next_month, 'view' => 'month', $this->pointerName => NULL), $this->conf['cache'], $this->conf['clear_anyway'], $monthViewPid);
		...
		$this->controller->getParametersForTyposcriptLink($this->local_cObj->data, array ('getdate' => $prev_month, 'view' => 'month', $this->pointerName => NULL), $this->conf['cache'], $this->conf['clear_anyway'], $monthViewPid);
		...
		$this->controller->getParametersForTyposcriptLink($this->local_cObj->data, array ('getdate' => $next_year, 'view' => 'year', $this->pointerName => NULL), $this->conf['cache'], $this->conf['clear_anyway'], $yearViewPid);
		...
		$this->controller->getParametersForTyposcriptLink($this->local_cObj->data, array ('getdate' => $prev_year, 'view' => 'year', $this->pointerName => NULL), $this->conf['cache'], $this->conf['clear_anyway'], $yearViewPid);
