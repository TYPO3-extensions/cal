	...
	function drawConfirmEvent() {
		...
		if($this->rightsObj->isAllowedToEditEventCategory() || $this->rightsObj->isAllowedToCreateEventCategory()){
			if ($this->controller->piVars["category_ids_selected"]) {
				...
				$first = true;
				$idname = array();
				foreach (strip_tags($this->controller->piVars["category_ids_selected"]) as $value) {
...
//				}
		...
		if($this->rightsObj->isAllowedToEditEventDateTime() || $this->rightsObj->isAllowedToCreateEventDateTime()){
			...
			$dateFormatArray = split('-',strtr(strip_tags($this->controller->piVars["event_start_day"]),$this->conf['view.']['event.']['dateSplitSymbol'],'-'));
			$languageArray['event_start_day_value'] = $dateFormatArray[$this->conf['view.']['event.']['dateYearPosition']].$dateFormatArray[$this->conf['view.']['event.']['dateMonthPosition']].$dateFormatArray[$this->conf['view.']['event.']['dateDayPosition']];
			
			$dateFormatArray = split('-',strtr(strip_tags($this->controller->piVars["event_end_day"]),$this->conf['view.']['event.']['dateSplitSymbol'],'-'));
			$languageArray['event_end_day_value'] = $dateFormatArray[$this->conf['view.']['event.']['dateYearPosition']].$dateFormatArray[$this->conf['view.']['event.']['dateMonthPosition']].$dateFormatArray[$this->conf['view.']['event.']['dateDayPosition']];
			
			$languageArray['event_start_day'] = strip_tags($this->controller->piVars["event_start_day"]);
			$languageArray['event_end_day'] = strip_tags($this->controller->piVars["event_end_day"]);
			$languageArray['event_start_time'] = strip_tags($this->controller->piVars["event_start_hour"]).':'.strip_tags($this->controller->piVars["event_start_minutes"]);
			$languageArray['event_end_time'] = strip_tags($this->controller->piVars["event_end_hour"]).':'.strip_tags($this->controller->piVars["event_end_minutes"]);
...
//			$end_day = parseDate($this->controller->piVars["event_end_day"], $dateFormat);
			$start_time = strip_tags($this->controller->piVars["event_start_hour"]).strip_tags($this->controller->piVars["event_start_minutes"]);
			$end_time = strip_tags($this->controller->piVars["event_end_hour"]).strip_tags($this->controller->piVars["event_end_minutes"]);
...
//		}
		...
		if($this->rightsObj->isAllowedToEditEventOrganizer() || $this->rightsObj->isAllowedToCreateEventOrganizer()){
			...
			if ($this->controller->piVars["organizer_id"]) {
				...
				$organizer = $this->modelObj->findOrganizer(intval($this->controller->piVars["organizer_id"]),$useOrganizerStructure);
			...
			$languageArray['organizer'] = strip_tags($this->controller->piVars["organizer"]);
		...
		if($this->rightsObj->isAllowedToEditEventLocation() || $this->rightsObj->isAllowedToCreateEventLocation()){
			...
			if ($this->controller->piVars["location_id"]) {
				...
				$location = $this->modelObj->findLocation(intval($this->controller->piVars["location_id"]),$useLocationStructure);
			...
			$languageArray['location'] = strip_tags($this->controller->piVars["location"]);
		}
		if($this->rightsObj->isAllowedToEditEventDescription() || $this->rightsObj->isAllowedToCreateEventDescription()){
			...
			$languageArray['description'] = $this->cObj->removeBadHTML($this->controller->piVars["description"], $this->conf);	
		}	
		if($this->rightsObj->isAllowedToEditEventRecurring() || $this->rightsObj->isAllowedToCreateEventRecurring()){
			...
			$languageArray['frequency_id'] = intval($this->controller->piVars["frequency_id"]);
			$languageArray['l_by_day'] = $this->controller->pi_getLL('l_event_edit_byday');
			$languageArray['by_day'] = strip_tags($this->controller->piVars["by_day"]);
			$languageArray['l_by_monthday'] = $this->controller->pi_getLL('l_event_edit_bymonthday');
			$languageArray['by_monthday'] = strip_tags($this->controller->piVars["by_monthday"]);
			$languageArray['l_by_month'] = $this->controller->pi_getLL('l_event_edit_bymonth');
			$languageArray['by_month'] = strip_tags($this->controller->piVars["by_month"]);
			$languageArray['l_until'] = $this->controller->pi_getLL('l_event_edit_until');
			$languageArray['until'] = strip_tags(($this->controller->piVars["until"]));
			$dateFormatArray = split('-',strtr(strip_tags($this->controller->piVars["until"]),$this->conf['view.']['event.']['dateSplitSymbol'],'-'));
			$languageArray['until_value'] = $dateFormatArray[$this->conf['view.']['event.']['dateYearPosition']].$dateFormatArray[$this->conf['view.']['event.']['dateMonthPosition']].$dateFormatArray[$this->conf['view.']['event.']['dateDayPosition']];
			$languageArray['l_count'] = $this->controller->pi_getLL('l_event_count');
			$languageArray['count'] = intval($this->controller->piVars["count"]);
			$languageArray['l_interval'] = $this->controller->pi_getLL('l_event_interval');
			$languageArray['interval'] = intval($this->controller->piVars["interval"]);
		}
		if($this->rightsObj->isAllowedToEditEventNotify() || $this->rightsObj->isAllowedToCreateEventNotify()){
			if (is_array($this->controller->piVars["notify_ids_selected"])) {
				...
				$notifydisplaylist = "";
				$notifyids = "";
				foreach (strip_tags($this->controller->piVars["notify_ids_selected"]) as $value) {
					preg_match('/(^[a-z])_([0-9]+)_(.*)/', $value, $idname);
					$notifyids .= ",".$idname[2];
