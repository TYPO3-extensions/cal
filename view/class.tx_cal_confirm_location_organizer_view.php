...
class tx_cal_confirm_location_organizer_view extends t3lib_svbase {
	 ...
	 */
	function drawConfirmLocationOrOrganizer($isLocation=true, $cObj, $rightsObj){
		
		$this->cObj = $cObj;
		$page = $this->cObj->fileResource($cObj->conf["view."]["location."]["confirmLocationTemplate"]);
		if ($page=="") {
			return "<h3>calendar: no confirm event template file found:</h3>".$cObj->conf["view."]["location."]["confirmLocationTemplate"];
		}

		$tx_cal_shared = t3lib_div::makeInstanceClassName('tx_cal_shared');
		$this->shared = new $tx_cal_shared($this->cObj);
		if($isLocation){
			$url = $this->shared->pi_getPageLink($GLOBALS['TSFE']->id, $GLOBALS['TSFE']->sPre, array("no_cache"=>1,"tx_cal_controller[view]"=>"save_location","tx_cal_controller[lastview]"=>$cObj->conf['lastview'],"tx_cal_controller[getdate]"=>$cObj->conf['getdate']));
		}else{
			$url = $this->shared->pi_getPageLink($GLOBALS['TSFE']->id, $GLOBALS['TSFE']->sPre, array("no_cache"=>1,"tx_cal_controller[view]"=>"save_organizer","tx_cal_controller[lastview]"=>$cObj->conf['lastview'],"tx_cal_controller[getdate]"=>$cObj->conf['getdate']));
		}		
		...
		$languageArray = array(
			'type'					=> $GLOBALS['HTTP_POST_VARS']["type"],
			...
			'description'			=> $GLOBALS['HTTP_POST_VARS']["description"],
			'l_street'				=> $this->shared->lang('l_location_street'),
			'street'				=> $GLOBALS['HTTP_POST_VARS']["street"],
			'l_zip'					=> $this->shared->lang('l_location_zip'),
			'zip'					=> $GLOBALS['HTTP_POST_VARS']["zip"],
			'l_city'				=> $this->shared->lang('l_location_city'),
			'city'					=> $GLOBALS['HTTP_POST_VARS']["city"],
			'l_phone'				=> $this->shared->lang('l_location_phone'),
			'phone'					=> $GLOBALS['HTTP_POST_VARS']["phone"],
			'l_email'				=> $this->shared->lang('l_location_email'),
			'email'					=> $GLOBALS['HTTP_POST_VARS']["email"],
			'l_image'				=> $this->shared->lang('l_location_image'),
			'image_url'				=> $GLOBALS['HTTP_POST_VARS']["image_url"],
			'l_link'				=> $this->shared->lang('l_location_link'),
			'link'					=> $GLOBALS['HTTP_POST_VARS']["link"],
