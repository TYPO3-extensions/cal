...
class tx_cal_confirm_location_organizer_view extends tx_cal_base_view {
	...
	function drawConfirmLocationOrOrganizer($isLocation=true){
		...
		if($isLocation){
			$url = $this->controller->pi_linkTP_keepPIvars_url(array("view"=>"save_location","lastview"=>$this->conf['lastview'],"getdate"=>$this->conf['getdate']));
		}else{
			$url = $this->controller->pi_linkTP_keepPIvars_url(array("view"=>"save_organizer","lastview"=>$this->conf['lastview'],"getdate"=>$this->conf['getdate']));
		}		
