...
class tx_cal_confirm_location_organizer_view extends tx_cal_base_view {
	...
	function drawConfirmLocationOrOrganizer($isLocation=true){
		...
		if($isLocation){
			$url = $this->shared->pi_getPageLink($GLOBALS['TSFE']->id, $GLOBALS['TSFE']->sPre, array("no_cache"=>1,"tx_cal_controller[view]"=>"save_location","tx_cal_controller[lastview]"=>$this->conf['lastview'],"tx_cal_controller[getdate]"=>$this->conf['getdate']));
		}else{
			$url = $this->shared->pi_getPageLink($GLOBALS['TSFE']->id, $GLOBALS['TSFE']->sPre, array("no_cache"=>1,"tx_cal_controller[view]"=>"save_organizer","tx_cal_controller[lastview]"=>$this->conf['lastview'],"tx_cal_controller[getdate]"=>$this->conf['getdate']));
		}		
