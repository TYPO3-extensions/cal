...
class tx_cal_create_calendar_view extends tx_cal_fe_editing_base_view {
	...
	function drawCreateCalendar($pidList, $object=''){	
		...
		$sims['###ACTION_URL###'] = $this->controller->pi_linkTP_keepPIvars_url(array('formCheck'=>'1'));	
        $page = substituteMarkerArrayNotCached($page, array(), $rems, $wrapped);
		$page = substituteMarkerArrayNotCached($page, $sims, array(), array ());
		return substituteMarkerArrayNotCached($page, $requiredFieldsSims, array(), array ());
	}
	...
	function getFormStartMarker(& $template, & $sims, & $rems, & $wrapped){
		...
		$rems['###FORM_START###'] = substituteMarkerArrayNotCached($temp, $temp_sims, array(), array ());
