 ...
 */
class tx_cal_create_category_view extends t3lib_svbase {
	...
	function drawCreateCategory($pidList, $object=''){	
		...
		$page = $this->cObj->fileResource($this->cObj->conf["view."]["category."]["createCategoryTemplate"]);
		if ($page=="") {
			return "<h3>category: no create category template file found:</h3>".$this->cObj->conf["view."]["category."]["createCategoryTemplate"];
		}
		...
		if(!$object){
			$languageArray ['l_create_category'] = $this->shared->lang('l_create_category');
			$languageArray['uid'] = "";
			...
			$languageArray ['l_create_category'] = $this->shared->lang('l_edit_category');
