...
class tx_cal_create_category_view extends tx_cal_base_view {
	...
	function drawCreateCategory($pidList, $object=''){	
		...
		$where = 'pid in ('.$this->conf['pages'].')';
		$result = $GLOBALS['TYPO3_DB']->exec_SELECTquery($select,$table,$where);
		while ($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($result)) {
			
			if($object && $object['calendar_id']==$row['uid']){
				$calendarIds .= '<option value="'.$row['uid'].'" selected="selected">'.$row['title'].'</option>';
			}else{
				$calendarIds .= '<option value="'.$row['uid'].'" >'.$row['title'].'</option>';
			}
		}
		...
		if(!$object){
			...
			if($this->rightsObj->isAllowedToCreateCategoryCalendar()){
				...
				$languageArray['calendar_ids'] = $calendarIds;
			}
			...
			if($this->rightsObj->isAllowedToEditCategoryCalendar()){
				$form 	.= $this->cObj->getSubpart($page, '###FORM_CALENDAR###');
				$languageArray['l_calendar'] = $this->controller->pi_getLL('l_calendar');
				$languageArray['calendar_ids'] = $calendarIds;
			}
