...
//			}
			if($this->rightsObj->isAllowedToEditEventDateTime()){
				...
				$start_hour = getHourFromTime($this->controller->piVars['gettime']);
				$start_minute = getMinutesFromTime($this->controller->piVars['gettime']);
...
//			}
		...
		// no event has been passed on -> create event form
			...
			if($this->rightsObj->isAllowedToCreateEventCategory()){
				...
				$allowedUids = split(',',$this->conf["rights."]["create."]["event."]["fields."]["allowedToCreateCategory."]["allowedUids"]);
				if($this->conf['switch_calendar']){
