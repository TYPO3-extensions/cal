...
//debug($this->controller->piVars);		
		...
		if(is_object($event)){
			...
			if($this->rightsObj->isAllowedToEditEventCategory()){
				...
				$validation .= "if(form.elements[i].name=='tx_cal_controller[category_ids_selected][]'){ selectAll(form.elements[i]);ok = atLeastOne(form.elements[i]); if(!ok){error = true; alert('".$this->shared->lang('l_edit_event_atleastone_category')."');}}";
