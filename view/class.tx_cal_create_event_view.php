...
class tx_cal_create_event_view extends tx_cal_fe_editing_base_view {
	...
	function getStartdateMarker(& $template, & $sims, & $rems){
		...
		if($this->isAllowed('startdate')){
			...
			$startDateValue = $eventStart->format(tx_cal_functions::getFormatStringFromConf($this->conf));
	...
	function getEnddateMarker(& $template, & $sims, & $rems) {
		...
		if($this->isAllowed('enddate')) {
			...
			$endDateValue = $eventEnd->format(tx_cal_functions::getFormatStringFromConf($this->conf));
	...
	function getCalOrganizerMarker(& $template, & $sims, & $rems){
		...
		if($this->isAllowed('cal_organizer')){
			$uidList = array(explode(',',$this->conf['rights.'][$this->isEditMode?'edit.':'create.']['event.']['fields.']['cal_organizer.']['allowedUids']));
	...
	function getCalLocationMarker(& $template, & $sims, & $rems){
		...
		if($this->isAllowed('cal_location')){		
			$uidList = array(explode(',',$this->conf['rights.'][$this->isEditMode?'edit.':'create.']['event.']['fields.']['cal_location.']['allowedUids']));
	...
	function getUntilMarker(& $template, & $sims, & $rems){
		...
		if($this->isAllowed('recurring')) {
			...
			if(is_object($until) && $until->getYear()!=0) {
				$untilValue = $until->format(tx_cal_functions::getFormatStringFromConf($this->conf));
	...
	function getRdateTypeMarker(& $template, & $sims, & $rems){
		$sims['###RDATE_TYPE###'] = '';
		$rdateType_values = array('none', 'date_time', 'date', 'period');
		$rdateType = '';
			...
			while ($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($result)) {
				...
				if($this->conf['view.'][$this->conf['view'].'.']['freeAndBusyViewPid'] && $row['tx_cal_calendar'] && $this->rightsObj->isLoggedIn()){
					...
					if($freeAndBusyIsEnabled && !empty($calendarOwner)){
						...
						$this->controller->pi_linkTP('test',array('tx_cal_controller[calendar]'=>$row['tx_cal_calendar'],'tx_cal_controller[getdate]'=>$start->format('%Y%m%d')), $this->conf['clear_anyway'],$this->conf['view.'][$this->conf['view'].'.']['freeAndBusyViewPid']);
