...
//debug($event);		
		...
		$languageArray['L_WRONG_SPLIT_SYMBOL_MSG'] = str_replace('###DATE_SPLIT_SYMBOL###',$this->conf['view.']['event.']['dateSplitSymbol'],$this->controller->pi_getLL('l_wrong_split_symbol_msg'));
		$languageArray['L_WRONG_DATE_MSG'] = $this->controller->pi_getLL('l_wrong_date');
		$languageArray['L_WRONG_TIME_MSG'] = $this->controller->pi_getLL('l_wrong_time');
		$languageArray['L_IS_IN_PAST_MSG'] = $this->controller->pi_getLL('l_is_in_past');
		...
		if(is_object($event) && $event->isUserAllowedToEdit($this->rightsObj->getUserId())){
			$languageArray ['l_edit_event'] = $this->controller->pi_getLL('l_edit_event');
			...
			$languageArray['calendar_id'] = $this->conf['calendar'];
			...
			if($this->rightsObj->isAllowedToEditEventCategory()){
				...
				$this->conf['calendar'] = $event->getCalendarUid();
				$categoryIds = $this->categoryService->getCategoryArray($pidList, true);
				$allowedUids = split(',',$this->conf["rights."]["edit."]["event."]["fields."]["allowedToEditCategory."]["allowedUids"]);
//debug($allowedUids);
				...
				$validation .= "if(form.elements[i].name=='tx_cal_controller[category_ids_selected][]'){ selectAll(form.elements[i]);}"; //ok = atLeastOne(form.elements[i]); if(!ok){error = true; alert('".$this->controller->pi_getLL('l_edit_event_atleastone_category')."');
				$languageArray['l_category'] = $this->controller->pi_getLL('l_event_category');
				$languageArray['l_category_selected'] = $this->controller->pi_getLL('l_event_category_selected');
			}
//			if($this->rightsObj->isAllowedToEditEventCategory()){
//				$form 	.= $this->cObj->getSubpart($page, "###FORM_CATEGORY###");
//				
//				// creating options for category
//				$category = "";//'<option value="">'.$this->controller->pi_getLL('l_select').'</option>';
...
//			}
			if($this->rightsObj->isAllowedToEditEventDateTime()){
				...
				$eventEndDay = array();
				$dateFormatArray = array();
				$dateFormatArray[$this->conf['view.']['event.']['dateDayPosition']] = 'dd';
				$dateFormatArray[$this->conf['view.']['event.']['dateMonthPosition']] = 'mm';
				$dateFormatArray[$this->conf['view.']['event.']['dateYearPosition']] = 'yyyy';
				$languageArray['l_event_start_day'] = $this->controller->pi_getLL('l_event_edit_startdate').' ('.$dateFormatArray[0].$this->conf['view.']['event.']['dateSplitSymbol'].$dateFormatArray[1].$this->conf['view.']['event.']['dateSplitSymbol'].$dateFormatArray[2].')';
				...
				$languageArray['event_start_day'] = $startDayArray[0].$this->conf['view.']['event.']['dateSplitSymbol'].$startDayArray[1].$this->conf['view.']['event.']['dateSplitSymbol'].$startDayArray[2];
				...
				$languageArray['gettime'] = intval($this->controller->piVars['gettime']);
				$languageArray['start_minutes'] = $start_minutes;
				$languageArray['end_minutes'] = $end_minutes;
				
				$languageArray['l_event_end_day'] = $this->controller->pi_getLL('l_event_edit_enddate').' ('.$dateFormatArray[0].$this->conf['view.']['event.']['dateSplitSymbol'].$dateFormatArray[1].$this->conf['view.']['event.']['dateSplitSymbol'].$dateFormatArray[2].')';
				ereg ("([0-9]{4})([0-9]{2})([0-9]{2})", $event_end_day, $eventEndDay);
				$endDayArray = array();
				$endDayArray[$this->conf['view.']['event.']['dateDayPosition']] = $eventStartDay[3];
				$endDayArray[$this->conf['view.']['event.']['dateMonthPosition']] = $eventStartDay[2];
				$endDayArray[$this->conf['view.']['event.']['dateYearPosition']] = $eventStartDay[1];
				$languageArray['event_end_day'] = $endDayArray[0].$this->conf['view.']['event.']['dateSplitSymbol'].$endDayArray[1].$this->conf['view.']['event.']['dateSplitSymbol'].$endDayArray[2];
...
//				}
				...
				if($this->rightsObj->isAllowedToEditEventException()){
					...
					$validation .= "if(form.elements[i].name=='tx_cal_controller[exception_ids_selected][]'){ selectAll(form.elements[i]);}";
					...
					$languageArray['l_exception'] = $this->controller->pi_getLL('l_event_exception');
					$languageArray['exception_ids'] = $exception;
					$languageArray['l_exception_selected'] = $this->controller->pi_getLL('l_event_exception_selected');
				...
				if($this->rightsObj->isAllowedToEditEventNotify()){
					...
					if($this->conf["rights."]['allowedUsers']!=""){
						$validation .= "if(form.elements[i].name=='tx_cal_controller[notify_ids_selected][]'){ selectAll(form.elements[i]);}";								
						// creating options for exceptions and monitoring - users
						$result = $GLOBALS['TYPO3_DB']->exec_SELECTquery("*","fe_users","uid in (".$this->conf["rights."]['allowedUsers'].")");
						while ($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($result)) {
...
//							}
					...
					if($this->conf["rights."]['allowedGroups']!=""){
						// creating options for exception groups
						$result = $GLOBALS['TYPO3_DB']->exec_SELECTquery("*","fe_groups","uid in (".$this->conf["rights."]['allowedGroups'].")");
//						while ($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($result)) {
...
//						}
					...
					$languageArray['l_notify_on_change'] = $this->controller->pi_getLL('l_event_monitor');
					$languageArray['notify_ids'] = $cal_notify_user;
					$languageArray['l_notify_on_change_selected'] = $this->controller->pi_getLL('l_event_monitor_selected');
...
//				$languageArray['l_edit_event_atleastone'] = $this->controller->pi_getLL('l_edit_event_atleastone');
...
//			}
			...
			if($this->rightsObj->isAllowedToEditEventDescription()){
				...
				if($this->RTEObj->isAvailable() && $this->conf['rights.']['create.']['event.']['enableRTE']) {
					$this->RTEcounter++;
					...
					$languageArray['description'] = '<textarea name="tx_cal_controller[description]" cols="30" rows="10">'.$event->getDescription().'</textarea>';
				}
			...
			if($this->rightsObj->isAllowedToEditEventRecurring()){
				...
				foreach ($freqency_values as $freq) {
					$frequency .= '<option value="'.$freq.'"'.($freq==$event->getFreq()?" selected":"").'>'.$this->controller->pi_getLL('l_'.$freq).'</option>';
				}

				$until = $event->getUntil();
				if($until>0){
					$untilArray = array();
					ereg ("([0-9]{4})([0-9]{2})([0-9]{2})", $until, $untilArray);
					$startDayArray = array();
...
//					$until = date("Ymd", $until);
				...
				foreach ($by_day as $day) {
					...
					$languageArray['L_DAYSOFWEEKSHORT_LANG_'.$day] = $this->controller->pi_getLL('l_daysofweekshort_lang_'.strtolower($day));
				}
				...
				$languageArray['until_selector'] = $useDateSelector ? tx_rlmpdateselectlib::getInputButton ('event_until',$dateSelectorConf) : '';
		...
		// no event has been passed on -> create event form
			...
			$languageArray ['l_edit_event'] = $this->controller->pi_getLL('l_create_event');
			...
			if($this->rightsObj->isAllowedToCreateEventCalendar()){
				...
				if(!$this->conf['switch_calendar'] && count($calendarIds)>1){
			...
			if($this->rightsObj->isAllowedToCreateEventCategory()){
				$form 	.= $this->cObj->getSubpart($page, "###FORM_CATEGORY###");
				// JH: check vs conf list and conf default value(s) of Categories
				$uidList = $this->conf["rights."]["create."]["event."]["fields."]["allowedToCreateCategory."]["uidList"];
				$uidDefault = $this->conf["rights."]["create."]["event."]["fields."]["allowedToCreateCategory."]["uidDefault"];
				$allowedUids = $this->conf["rights."]["create."]["event."]["fields."]["allowedToCreateCategory."]["allowedUids"]?split(',',$this->conf["rights."]["create."]["event."]["fields."]["allowedToCreateCategory."]["allowedUids"]):array();
				if($this->conf['switch_calendar']){
					$this->conf['calendar'] = $this->conf['switch_calendar'];
				}else{
					$this->conf['calendar'] = '';
				}
				$categoryIds = $this->categoryService->getCategoryArray($pidList);
				$rememberUid = array();
				foreach($categoryIds as $temp){
					foreach($temp as $categoryRow){

						$isInAllowedUidList = !empty($allowedUids)&&$allowedUids[0]>0?in_array($categoryRow['uid'], $allowedUids):true;
						if(!in_array($categoryRow['uid'], $rememberUid) && $categoryRow['calendar_id']==$this->conf['calendar'] && $isInAllowedUidList){
...
//				$languageArray['event_start_time'] = $this->conf['gettime'];
				...
				$dateFormatArray[$this->conf['view.']['event.']['dateYearPosition']] = 'yyyy';
				$languageArray['l_event_start_day'] = $this->controller->pi_getLL('l_event_edit_startdate').' ('.$dateFormatArray[0].$this->conf['view.']['event.']['dateSplitSymbol'].$dateFormatArray[1].$this->conf['view.']['event.']['dateSplitSymbol'].$dateFormatArray[2].')';
				ereg ("([0-9]{4})([0-9]{2})([0-9]{2})", $getdate, $eventStartDay);
				$startDayArray = array();
				$startDayArray[$this->conf['view.']['event.']['dateDayPosition']] = $eventStartDay[3];
				$startDayArray[$this->conf['view.']['event.']['dateMonthPosition']] = $eventStartDay[2];
				$startDayArray[$this->conf['view.']['event.']['dateYearPosition']] = $eventStartDay[1];
				...
				$languageArray['l_event_end_day'] = $this->controller->pi_getLL('l_event_edit_enddate').' ('.$dateFormatArray[0].$this->conf['view.']['event.']['dateSplitSymbol'].$dateFormatArray[1].$this->conf['view.']['event.']['dateSplitSymbol'].$dateFormatArray[2].')';
				ereg ("([0-9]{4})([0-9]{2})([0-9]{2})", $getdate, $eventEndDay);
				$endDayArray = array();
				$endDayArray[$this->conf['view.']['event.']['dateDayPosition']] = $eventStartDay[3];
				$endDayArray[$this->conf['view.']['event.']['dateMonthPosition']] = $eventStartDay[2];
				$endDayArray[$this->conf['view.']['event.']['dateYearPosition']] = $eventStartDay[1];
...
//				$languageArray['event_end_time'	] = "";
				$languageArray['start_day_selector'] = $useDateSelector ? tx_rlmpdateselectlib::getInputButton ('event_start_day',$dateSelectorConf) : '';
				$languageArray['end_day_selector'] = $useDateSelector ? tx_rlmpdateselectlib::getInputButton ('event_end_day',$dateSelectorConf) : '';
				$languageArray['DATE_SPLIT_SYMBOL'] = $this->conf['view.']['event.']['dateSplitSymbol'];
				$languageArray['DATE_DAY_POSITION'] = $this->conf['view.']['event.']['dateDayPosition'];
				$languageArray['DATE_MONTH_POSITION'] = $this->conf['view.']['event.']['dateMonthPosition'];
				$languageArray['DATE_YEAR_POSITION'] = $this->conf['view.']['event.']['dateYearPosition'];
			}
			if($this->rightsObj->isAllowedToCreateEventTitle()){
				$form 	.= $this->cObj->getSubpart($page, "###FORM_TITLE###");
				$languageArray['l_title'] = $this->controller->pi_getLL('l_event_title');
				if(!empty($this->conf["rights."]["create."]["event."]["fields."]["allowedToCreateTitle."]["default"])) {
					$languageArray['title'] = $this->conf["rights."]["create."]["event."]["fields."]["allowedToCreateTitle."]["default"];
				}
				else {
					$languageArray['title'] = "";
				}	
...
//			if(($this->conf["rights."]['allowedUsers']!="" || $this->conf["rights."]['allowedGroups']!="")){

//				$form 	.= $this->cObj->getSubpart($page, "###FORM_CREATOR###");
				// selecting uids of available creator & -groups
//				$validation .= "case 'user_ids_selected[]':	if(!atLeastOne(form.elements[i])){error = true;	break;}	selectAll(form.elements[i]);break;";
//				$cal_user = "";
//				$uidList = $this->conf["rights."]["create."]["event."]["allowedToCreateCreator."]["uidList"];
//				$uidDefault = $this->conf["rights."]["create."]["event."]["allowedToCreateCreator."]["uidDefault"];
//				if(!empty($uidDefault)) {
...
//				}
//				else {
//					if(!empty($this->conf["rights."]['allowedUsers'])) {
//						$result = $GLOBALS['TYPO3_DB']->exec_SELECTquery("*","fe_users","uid IN (".$this->conf["rights."]['allowedUsers'].") ".$notUidDefaultSQL." AND deleted = 0 AND disable = 0");
//						while ($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($result)) {
//							$cal_user .= '<option value="u_'.$row['uid'].'_'.$row['username'].'" onclick="addOption(\'tx_cal_controller_user_ids_selected\',\''.$row['username'].' (U)\',\'u_'.$row['uid'].'_'.$row['username'].'\',\'tx_cal_controller_user_ids\');removeOption(\'tx_cal_controller_user_ids\',this.index);">'.$row['username'].' (U)</option>';
//						}
//					}
//					$only_user = $cal_user;
//					if(!empty($this->conf["rights."]['allowedGroups'])) {
//						$result = $GLOBALS['TYPO3_DB']->exec_SELECTquery("*","fe_groups","uid IN (".$this->conf["rights."]['allowedGroups'].") AND deleted = 0 AND hidden = 0");
//						while ($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($result)) {
//							$cal_user .= '<option value="g_'.$row['uid'].'_'.$row['title'].'" onclick="addOption(\'tx_cal_controller_user_ids_selected\',\''.$row['title'].' (G)\',\'g_'.$row['uid'].'_'.$row['title'].'\',\'tx_cal_controller_user_ids\');removeOption(\'tx_cal_controller_user_ids\',this.index);">'.$row['title'].' (G)</option>';
//						}
//					}
...
//			
			if($this->rightsObj->isAllowedToCreateEventOrganizer()){
				...
				$uidList = array($this->conf["rights."]["create."]["event."]["fields."]["allowedToCreateOrganizer."]["uidList"]);
			...
			if($this->rightsObj->isAllowedToCreateEventLocation()){
				...
				$uidList = array($this->conf["rights."]["create."]["event."]["fields."]["allowedToCreateLocation."]["uidList"]);
			...
			if($this->rightsObj->isAllowedToCreateEventDescription()){
				...
				if($this->RTEObj->isAvailable() && $this->conf['rights.']['create.']['event.']['enableRTE']) {
					...
					$languageArray['ADDITIONALJS_POST'] = '';
					$languageArray['ADDITIONALJS_SUBMIT'] = '';
					$languageArray['description'] = '<textarea name="tx_cal_controller[description]" cols="30" rows="10"></textarea>';
				}
				/* End setting the RTE markers */
			}else{
				$languageArray['ADDITIONALJS_SUBMIT'] = "";
			...
			if($this->rightsObj->isAllowedToCreateEventRecurring()){
				...
				$languageArray['l_until'] = $this->controller->pi_getLL('l_event_edit_until');
				$languageArray['until'] = "";
				$languageArray['until_selector'] = $useDateSelector ? tx_rlmpdateselectlib::getInputButton ('until',$dateSelectorConf) : '';
				$languageArray['l_count'] = $this->controller->pi_getLL('l_event_count');
				$languageArray['count'] = "";
				$languageArray['l_interval'] = $this->controller->pi_getLL('l_event_interval');
			...
			if($this->rightsObj->isAllowedToCreateEventNotify() && ($this->conf["rights."]['allowedUsers']!="" || $this->conf["rights."]['allowedGroups']!="")){
				$form 	.= $this->cObj->getSubpart($page, "###FORM_NOTIFY###");
				if($this->conf["rights."]['allowedUsers']!=""){
					$result = $GLOBALS['TYPO3_DB']->exec_SELECTquery("*","fe_users","uid in (".$this->conf["rights."]['allowedUsers'].")");
					while ($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($result)) {
