...
//					$until = date('Ymd', $until);
			...
			$languageArray['lastview'] = $this->controller->extendLastView();
			...
			$languageArray['back_link'] = $this->controller->pi_linkTP_keepPIvars_url( $this->controller->shortenLastViewAndGetTargetViewParameters());
			...
			if($this->rightsObj->isAllowedToCreateEventCategory()){
				...
				foreach($categoryIds as $temp){
					foreach($temp as $categoryRow){
						...
						$isInAllowedUidList = !empty($allowedUids)&&$allowedUids[0]>0?in_array($categoryRow['uid'], $allowedUids):true;
						if(!in_array($categoryRow['uid'], $rememberUid) && ($categoryRow['calendar_id']==$this->conf['calendar'] || $categoryRow['calendar_id']==0) && $isInAllowedUidList){
