...
//debug($view_array);
		...
		for ($i = 0; $i < 7; $i ++) {
			...
			if($view_array[$daylink] || $isAllowedToCreateEvents){
				$GLOBALS['TSFE']->ATagParams = 'title="'.$this->controller->pi_getLL('l_'.$this->conf['view.']['dayLinkTarget'].'_view').'"';
				if (!empty ($this->conf['view.']['day.']['dayViewPid'])) {
					$link = $this->controller->pi_linkTP_keepPIvars($weekday, array ('getdate' => $daylink, 'view' => $this->conf['view.']['dayLinkTarget']), 0, $this->conf['clear_anyway'], $this->conf['view.']['day.']['dayViewPid']);
				} else {
					$link = $this->controller->pi_linkTP_keepPIvars($weekday, array ('getdate' => $daylink, 'view' => $this->conf['view.']['dayLinkTarget']), 0, $this->conf['clear_anyway']);
				}
			}else{
				$link = $weekday;
			}
					...
					for($l = 0; $l < $nbrGridCols; $l++){
						if(!$val[$l]){
							...
							if ($isAllowedToCreateEvents && $time>(time()+$createOffset)) {
								...
								$link .= $this->controller->pi_linkTP_keepPIvars($this->conf['view.']['day.']['addIcon'], array ('gettime' => $cal_time, 'lastview' => $this->controller->extendLastView(), 'view' => 'create_event'), 0, $this->conf['clear_anyway'],$this->conf['view.']['event.']['create_eventViewPid']);
							}
