...
class tx_cal_dayview extends tx_cal_base_view {
	...
	function drawDay(&$master_array, $getdate) {
		...
		$next_day_obj = new tx_cal_date();
		$next_day_obj->copy($unix_time);
		$next_day_obj->addSeconds(86400);
		$next_day = $next_day_obj->format('%Y%m%d');
		$prev_day_obj = new tx_cal_date();
		$prev_day_obj->copy($unix_time);
		$prev_day_obj->subtractSeconds(86401);
		...
		$start_day_time = new tx_cal_date($getdate,'Ymd');
		$start_day_time->setTZbyId('UTC');
		$end_day_time = tx_cal_calendar::calculateEndDayTime($start_day_time);

		$GLOBALS['TSFE']->register['cal_day_starttime'] = $start_day_time->format($this->conf['view.']['day.']['strftimeTitleStartFormat']);
		$GLOBALS['TSFE']->register['cal_day_endtime'] = $end_day_time->format($this->conf['view.']['day.']['strftimeTitleEndFormat']);
		...
		if (!empty($this->master_array)) {
			foreach ($this->master_array as $ovlKey => $ovlValue) {
				...
				$d_start = new tx_cal_date($dDate[1].$dDate[2].$dDate[3].' '.$dTimeStart[1].':'.$dTimeStart[2].':00','Ymd H:i:s');
				$d_start->setTZbyId('UTC');
				$d_end = new tx_cal_date($dDate[1].$dDate[2].$dDate[3].' '.$dTimeEnd[1].':'.$dTimeEnd[2].':00','Ymd H:i:s');
				$d_end->setTZbyId('UTC');
				...
				foreach ($ovlValue as $ovl_time_key => $ovl_time_Value) {
					foreach ($ovl_time_Value as $event) {
						...
						if ($ovl_time_key == '-1') {
							...
							for($j->copy($starttime); $j->before($endtime) && $end_day_time->after($j) ; $j->addSeconds(86400)){
...
//debug($view_array);
		...
		for ($i = 0; $i < 7; $i ++) {
			...
			if($view_array[$daylink] || $isAllowedToCreateEvent){
				$GLOBALS['TSFE']->ATagParams = 'title="'.$this->controller->pi_getLL('l_'.$this->conf['view.']['dayLinkTarget'].'_view').'"';
		...
		$d_start = new tx_cal_date($dDate[1].$dDate[2].$dDate[3].' '.$dTimeStart[1].':'.sprintf("%02d", $this->$dTimeStart[2]).':00','Ymd H:i:s');
		$d_start->setTZbyId('UTC');
		$d_end = new tx_cal_date($dDate[1].$dDate[2].$dDate[3].' '.$dTimeEnd[1].':'.sprintf("%02d", $this->$dTimeEnd[2]).':00','Ymd H:i:s');
		$d_end->setTZbyId('UTC');
...
//debug($t_array);		
		...
		$cal_time_obj = new tx_cal_date($getdate);
					...
					if(!empty($val[$i])){
						...
						switch ($keys[0]) {
							case 'begin' :
								...
								$colSpan = floor(($event->getLengthInSeconds()+$rest*60) / ($gridLength * 60));
