				...
				foreach ($ovlValue as $ovl_time_key => $ovl_time_Value) {
					foreach ($ovl_time_Value as $event) {
						...
						if ($ovl_time_key == '-1') {
							...
							$j->copy($starttime);
							while(!$j->after($endtime) && $j->before($end_week_time)){
								if($j->after($endOfDay)){
...
//debug($cal_time_obj->format('%Y%m%d %H%M'));
		...
		$dayTemplate = substituteMarkerArrayNotCached($dayTemplate, $sims, array (), array ());
		$rems['###DAYEVENTS###'] = $daydisplay;
		$page = substituteMarkerArrayNotCached($page, array (), array ('###DAY_TEMPLATE###'=>$dayTemplate), array ());
		return $this->finish($page, $rems);
	}
}
