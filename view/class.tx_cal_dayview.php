...
class tx_cal_dayview extends tx_cal_base_view {
	...
	function drawDay(&$master_array, $getdate) {
		...
		$display_date = gmstrftime($this->conf['view.']['day.']['dateFormatWeekList'], $unix_time);
						...
						switch ($keys[0]) {
							case 'begin' :
								...
								if ($event->isUserAllowedToEdit()) {
									...
									$sims['###EDITLINK###'] = $this->controller->pi_linkTP_keepPIvars($this->conf['view.']['day.']['editIcon'], array ('lastview' => $this->controller->extendLastView(), 'view' => 'edit_event', 'type' => $event->getType(), 'uid' => $event->getUid()), $this->conf['cache'], $this->conf['clear_anyway'], $this->conf['view.']['event.']['editEventViewPid']);
								}
								if($event->isUserAllowedToDelete()){
									...
									$sims['###EDITLINK###'] .= $this->controller->pi_linkTP_keepPIvars($this->conf['view.']['day.']['deleteIcon'], array('lastview' => $this->controller->extendLastView(), 'view' => 'delete_event', 'type' => $event->getType(), 'uid' => $event->getUid()), $this->conf['cache'], $this->conf['clear_anyway'], $this->conf['view.']['event.']['deleteEventViewPid']);
								}
								...
								$e_start = gmstrftime($this->conf['view.']['day.']['timeFormatDay'], $event->getStarttime());
								$sims['###EVENT_START###'] = $confirmed.$this->cObj->substituteMarkerArrayCached($e_start, $event_temp, array (), array ());
								$e_end = gmstrftime($this->conf['view.']['day.']['timeFormatDay'], $event->getEndtime());
