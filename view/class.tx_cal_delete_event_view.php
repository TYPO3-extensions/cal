...
class tx_cal_delete_event_view extends t3lib_svbase {
	...
	function drawDeleteEvent(&$event, $pidList ) {
		...
		$page = $this->cObj->fileResource($this->cObj->conf["view."]["event."]["deleteEventTemplate"]);
		if ($page=="") {
			return "<h3>calendar: no delete event template file found:</h3>".$this->cObj->conf["view."]["event."]["deleteEventTemplate"];
		}
...
//		}
		...
		if($event->isClone()){
			...
			$languageArray['l_create_exception'] = $this->shared->lang('l_create_exception');
