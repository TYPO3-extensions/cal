 ...
 */
class tx_cal_delete_event_view extends t3lib_svbase {
	 ...
	 */
	function drawDeleteEvent(&$event, &$cObj, &$rightsObj, $pidList ) {

		$this->cObj = $cObj;
		
		$page = $this->cObj->fileResource($cObj->conf["view."]["event."]["deleteEventTemplate"]);
		if ($page=="") {
			return "<h3>calendar: no delete event template file found:</h3>".$cObj->conf["view."]["event."]["deleteEventTemplate"];
		}
		...
		switch ($event->getFreq()){
			case 'none':
				$frequency = $this->shared->lang('l_none');
				break;
			case 'year':
				$frequency = $this->shared->lang('l_format_recur_lang_yearly_single');
				break;
			case 'month':
				$frequency = $this->shared->lang('l_format_recur_lang_monthly_single');
				break;
			case 'week':
				$frequency = $this->shared->lang('l_format_recur_lang_weekly_single');
				break;
			case 'day':
				$frequency = $this->shared->lang('l_format_recur_lang_daily_single');
				break;
		}
		
		// creating options for category
		$category = "";
		$result = $GLOBALS['TYPO3_DB']->exec_SELECTquery("*","tx_cal_category","pid in (".$pidList.")");
		...
		// url parameter
		$parameter = array("no_cache"=>1,"tx_cal_controller[view]"=>"remove_event","tx_cal_controller[lastview]"=>$cObj->conf['lastview'],"tx_cal_controller[getdate]"=>$cObj->conf['getdate']);
		
		$hidden = "";		
		$languageArray = array(
			'uid'					=> $uid,
			'type'					=> "tx_cal_phpicalendar",
			'l_delete_event'		=> $this->shared->lang('l_delete_event'),
			'l_hidden'				=> $this->shared->lang('l_hidden'),
			'hidden'				=> $hidden,
			'l_category'			=> $this->shared->lang('l_event_category'),
			'category_ids'			=> $category,
			'l_event_start_day'		=> $this->shared->lang('l_event_edit_startdate'),
			'event_start_day'		=> $event_start_day,
			'l_event_start_time'	=> $this->shared->lang('l_event_edit_starttime'),
			'event_start_time'		=> $event_start_time,
			'l_event_end_day'		=> $this->shared->lang('l_event_edit_enddate'),
			'event_end_day'			=> $event_end_day,
			'l_event_end_time'		=> $this->shared->lang('l_event_edit_endtime'),
			'event_end_time'		=> $event_end_time,
			'l_title'				=> $this->shared->lang('l_event_title'),
			'title'					=> $title,
			'l_user'				=> $this->shared->lang('l_event_user'),
			'user_ids' 				=> $cal_user,
			'l_organizer'			=> $this->shared->lang('l_event_organizer'),
			'organizer'				=> $organizer,
			'l_cal_organizer'		=> $this->shared->lang('l_event_cal_organizer'),
			'organizer_ids'			=> $cal_organizer,
			'l_location'			=> $this->shared->lang('l_event_location'),
			'location_ids'			=> $cal_location,
			'l_cal_location'		=> $this->shared->lang('l_event_cal_location'),
			'location'				=> $location,
			'l_description'			=> $this->shared->lang('l_event_description'),
			'description'			=> $description,
			'l_frequency'			=> $this->shared->lang('l_event_frequency'),
			'frequency_ids' 		=> $frequency,
			'l_by_day'				=> $this->shared->lang('l_event_edit_byday'),
			'by_day'				=> $by_day,
			'l_by_monthday'			=> $this->shared->lang('l_event_edit_bymonthday'),
			'by_monthday'			=> $by_monthday,
			'l_by_month'			=> $this->shared->lang('l_event_edit_bymonth'),
			'by_month'				=> $by_month,
			'l_until'				=> $this->shared->lang('l_event_edit_until'),
			'until'					=> $until,
			'l_count'				=> $this->shared->lang('l_event_count'),
			'count'					=> $count,
			'l_interval'			=> $this->shared->lang('l_event_interval'),
			'interval'				=> $interval,
			'l_notify_on_change'	=> $this->shared->lang('l_event_monitor'),
			'notify_ids'			=> $cal_notify_user,
			'l_exception'			=> $this->shared->lang('l_event_exception'),
			'exception_ids'			=> $exception,
			'l_delete'				=> $this->shared->lang('l_delete'),
			'l_cancel'				=> $this->shared->lang('l_cancel'),
			'action_url'			=> $this->shared->pi_getPageLink($GLOBALS['TSFE']->id, $GLOBALS['TSFE']->sPre, $parameter),
		);
