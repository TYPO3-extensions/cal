...
class tx_cal_delete_location_organizer_view extends t3lib_svbase {
	 ...
	 */
	function drawConfirmLocationOrOrganizer($isLocation=true, &$object, &$cObj, &$rightsObj){
		
		$this->cObj = $cObj;
		$page = $this->cObj->fileResource($cObj->conf["view."]["location."]["delete_locationTemplate"]);
		if ($page=="") {
			return "<h3>calendar: no delete event template file found:</h3>".$cObj->conf["view."]["location."]["delete_locationTemplate"];
		}
