...
class tx_cal_delete_location_organizer_view extends tx_cal_base_view {
	...
	function drawDeleteLocationOrOrganizer($isLocation=true, &$object){
		...
		if($isLocation){
			$url = $this->controller->pi_linkTP_keepPIvars_url(array("view"=>"remove_location","lastview"=>$this->conf['lastview'],"getdate"=>$this->conf['getdate']));
		}else{
			$url = $this->controller->pi_linkTP_keepPIvars_url(array("view"=>"remove_organizer","lastview"=>$this->conf['lastview'],"getdate"=>$this->conf['getdate']));
		}		
