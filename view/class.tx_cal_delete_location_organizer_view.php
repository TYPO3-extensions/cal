...
class tx_cal_delete_location_organizer_view extends tx_cal_fe_editing_base_view {
	...
	function drawDeleteLocationOrOrganizer($isLocation=true, &$object){
		...
		if ($page=='') {
			return '<h3>calendar: no delete event template file found:</h3>'.$this->conf['view.']['location.']['deleteLocationTemplate'];
		}
		...
		$zip = $object->getPostalCode();
		$city = $object->getCity();
		$countryzone = $object->getCountryZone();
		...
		$languageArray = array(
			...
			'city'					=> $city,
			'l_countryzone'			=> $this->controller->pi_getLL('l_location_countryzone'),
			'countryzone'			=> $countryzone,
			'l_country'				=> $this->controller->pi_getLL('l_location_country'),
			'country'				=> $country,
