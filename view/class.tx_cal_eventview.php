...
class tx_cal_eventview extends tx_cal_base_view {
	...
	function drawEvent(&$event, $getdate) {
		...
		$page = $this->cObj->fileResource($this->cObj->conf["view."]["event."]["eventTemplate"]);
		if ($page == "") {
			return "<h3>calendar: no template file found:</h3>".$this->cObj->conf["view."]["event."]["eventTemplate"];
		}
		if($this->cObj->conf['preview']){
