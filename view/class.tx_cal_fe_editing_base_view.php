...
class tx_cal_fe_editing_base_view extends tx_cal_base_view {
	...
	function getFormEndMarker(& $template, & $sims, & $rems, & $wrapped){
		...
		$rems['###FORM_END###'] = tx_cal_functions::substituteMarkerArrayNotCached($temp, $temp_sims, array(), array ());
	}
		...
		if($this->isConfirm){
			...
			if($this->isEditMode){
				if($marker=='image'){
					...
					$files = Array($this->object->getIcsFile());
				}else{
		...
		if($this->isEditMode && $this->rightsObj->isViewEnabled('translation') && $this->rightsObj->isAllowedTo('create', 'translation')){
			...
			if(!empty($langIds)){
				...
				foreach($langIds as $key => $langId){
					if($langId>0){
						$overrulePIvars = array ('view'=>'translation','overlay'=>$langId,'servicename'=>$this->serviceName,'subtype'=>$this->objectString,'uid'=>$this->object->getUid(),'type'=>$this->object->getType());
						$piVars = $this->piVars;
						unset($piVars['DATA']);
...
//					}
	...
	function getFeUserDisplayName(&$row,$tsName='defaultFeUserDisplayName'){
	...
	function getFeGroupDisplayName(&$row,$tsName='defaultFeGroupDisplayName'){
