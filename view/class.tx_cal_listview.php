...
class tx_cal_listview extends tx_cal_base_view {
				...
				foreach ($event_times as $a_key => $a) {
					if (is_array($a)) {
						foreach ($a as $uid => $val) {
                            ...
                            $cal_month=intval(substr($cal_time,4,2));
							...
							$count ++;
							if ($count == $this->conf['view.']['list.']['maxEvents'] && !$this->conf['view.']['list.']['pageBrowser']) {
  ...
  //t3lib_div::debug("$min $max von $pagesTotal ::: $pstart - $pend",'PB'); 
        ...
        if($this->conf['view.']['list.']['pageBrowser'] && $finished!='') return $pb.$finished;
