								...
								if($this->conf['view.']['list.']['showLongEventsInEachWrapper']){
									if($this->conf['view.']['list.']['enableDayWrapper'] && $eventStart->format('%Y%m%d')!=$eventEnd->format('%Y%m%d')){
										...
										while($tempEventStart->format('%Y%m%d')!=$eventEnd->format('%Y%m%d')){
											...
											$eventsInList[$tempEventStart->format('%Y%m%d')][] = &$event;
										}
									}
									...
									if($this->conf['view.']['list.']['enableMonthWrapper'] && $eventStart->format('%Y%m')!=$eventEnd->format('%Y%m')){
										...
										while($tempEventStart->format('%Y%m')!=$eventEnd->format('%Y%m')){
											...
											$eventsInList[$tempEventStart->format('%Y%m01')][] = &$event;
										}
									}
									if($this->conf['view.']['list.']['enableYearWrapper'] && $eventStart->format('%Y')!=$eventEnd->format('%Y')){
										...
										while($tempEventStart->format('%Y')!=$eventEnd->format('%Y')){
											...
											$eventsInList[$tempEventStart->format('%Y0101')][] = &$event;
										}
									}
								}
			...
			if(count($eventsInList) && $count > 0) {
				...
				foreach($times as $cal_time) {
					...
					foreach($e_keys as $e_key) {
						...
						//yearwrapper
						if($this->conf['view.']['list.']['enableYearWrapper'] && ($this->hasPeriodChanged($lastEventYear->getYear(),$cal_year,$reverse) || $firstTime)){
						...
						//monthwrapper
						if($this->conf['view.']['list.']['enableMonthWrapper'] && ($this->hasPeriodChanged($lastEventMonth->format('%Y%m'),$calTimeObject->format('%Y%m'),$reverse) || $firstTime || $this->hasPeriodChanged($lastEventMonth->getYear(),$cal_year,$reverse))){
							$this->initLocalCObject();
							if($this->conf['view.']['list.']['enableSectionMenu']){
						...
						//weekwrapper
						if($this->conf['view.']['list.']['enableWeekWrapper'] && ($this->hasPeriodChanged($lastEventWeek->format('%U'),$cal_week,$reverse) || $firstTime || $this->hasPeriodChanged($lastEventWeek->getYear(),$cal_year,$reverse))){
							$this->initLocalCObject();
							if($this->conf['view.']['list.']['enableSectionMenu']){
						...
						//daywrapper
						if($this->conf['view.']['list.']['enableDayWrapper'] && (($reverse ? $lastEventDay->after($calTimeObject) : $lastEventDay->before($calTimeObject)) || $firstTime || $this->hasPeriodChanged($lastEventDay->getYear(),$cal_year,$reverse))){
							$this->initLocalCObject();
							if($this->conf['view.']['list.']['enableSectionMenu']){
						...
						$eventText = '';
						if($this->conf['view']=='location' || $this->conf['view']=='organizer'){
							$eventText = $event->renderEventFor(strtoupper($this->conf['view']),$layoutSuffix);
						} else {
			...
			if(count($hookObjectsArr)) {
				foreach ($hookObjectsArr as $hookObj) {
					if (method_exists($hookObj, 'renderPageBrowser')) {
						$hookObj->renderPageBrowser($this, $pb, $recordsTotal, $recordsPerPage, $template);
					}
				}
				if($pb != '') {
					return $pb;
