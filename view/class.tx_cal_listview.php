				...
				foreach ($event_times as $a_key => $a) {
					if (is_array($a)) {
						foreach ($a as $uid => $event) {
							...
							if(!$firstEventDate) {
								$firstEventDate = $eventStart;
								$lastEventTime = $eventStart;
							}
							...
							//monthwrapper
							if($this->conf['view.']['list.']['enableMonthWrapper'] && $lastEventMonth->getMonth() < $cal_month){
							...
							//weekwrapper
							if($this->conf['view.']['list.']['enableWeekWrapper'] && $lastEventWeek->format('%U') < $cal_week){
							...
							//daywrapper
							if($this->conf['view.']['list.']['enableDayWrapper'] && $lastEventTime->before($calTimeObject)){
