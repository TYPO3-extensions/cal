...
class tx_cal_monthview extends tx_cal_base_view {
	...
	function drawMonth(&$master_array, $getdate) {
		...
		$page = $this->cObj->fileResource($this->cObj->conf["view."]["month."]["monthTemplate"]);
		if ($page == "") {
			return "<h3>calendar: no template file found:</h3>".$this->cObj->conf["view."]["month."]["monthTemplate"]."<br />Please check your template record and add both cal items at 'include static (from extension)'";
		}
		$rems = array();
