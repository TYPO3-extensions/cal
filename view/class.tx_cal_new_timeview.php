 ...
 * @subpackage cal
 */
abstract class tx_cal_new_timeview {
	 ...
	 */
	public function tx_cal_new_timeview(){
	}
	...
	public function render(&$template){
		...
		return $this->finish(tx_cal_functions::substituteMarkerArrayNotCached($subpart, $sims, $rems, $wrapped));
	}
			...
			switch ($marker) {
				...
				default :
					...
					if(method_exists($this,$funcFromMarker)) {
						...
						$cObj = &tx_cal_registry::Registry('basic','local_cobj');
	...
	*/
	...
	public function getCreateEventLink($view, $wrap, $date, $createOffset, $isAllowedToCreateEvent, $remember, $class, $time){
		...
		if (($date->after($now) || $rightsObj->isAllowedToCreateEventInPast()) && $isAllowedToCreateEvent) {
			...
			if($conf['view.']['enableAjax']){
				...
				$controller->getParametersForTyposcriptLink($local_cObj->data, array ('gettime' => $time, 'getdate'=>$date->format('%Y%m%d'),  'view' => 'create_event'), 0, $conf['clear_anyway'],$conf['view.']['event.']['createEventViewPid']);
	...
	public function getTimetableHeightMarker(& $template, & $sims, & $rems, & $wrapped, $view){
		...
		$d_end = new tx_cal_date('01012000'.$dayEnd);
	...
	public function getTimeCellsMarker(& $template, & $sims, & $rems, & $wrapped, $view){
		...
		if($conf['view']=='day'){
			...
			$d_start = new tx_cal_date($this->weekStart.$dayStart);
			$d_end = new tx_cal_date($this->weekStart.$dayEnd);
		}
		...
		while($d_end->after($d_start)){
			...
			$times .= str_replace(array('###TIME###','###VALUE###','###CREATE_EVENT_LINK###'), array($d_start->format($conf['view.'][$conf['view'].'.']['timeFormatDay']), $d_start->getHour(), $createLink), $timesSubpart);
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/cal/view/class.tx_cal_new_timeview.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/cal/view/class.tx_cal_new_timeview.php']);
}
?>