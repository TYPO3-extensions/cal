...
class tx_cal_notification_view extends tx_cal_base_service {
	...
	function notifyOfChanges($oldEventDataArray, $newEventDataArray){
		...
		$event_old = $this->modelObj->findEvent($oldEventDataArray['uid'],'tx_cal_phpicalendar', $this->conf['pidList'], true, true, false, true);
		$event_old->updateWithPiVars($oldEventDataArray); 
		$event_new = $this->modelObj->findEvent($oldEventDataArray['uid'],'tx_cal_phpicalendar', $this->conf['pidList'], true, true, false, true);
	...
	function sendNotificationOfChanges(&$event_old, &$event_new, $email, $templatePath, $titleText, $unsubscribeLink, $acceptLink='', $declineLink=''){
		...
		$htmlTemplate = substituteMarkerArrayNotCached($htmlTemplate, $switch, array('###OLD_EVENT###' => $oldEventHTMLSubpart,'###NEW_EVENT###' => $newEventHTMLSubpart), array());
		$plainTemplate = substituteMarkerArrayNotCached($plainTemplate, $switch, array('###OLD_EVENT###' => $oldEventPlainSubpart,'###NEW_EVENT###' => $newEventPlainSubpart), array());

		$switch = array();
		$rems = array();
		$wrapped = array();
		$event_new->getMarker($titleText,$switch,$rems, $wrapped, 'title');
		$this->mailer->subject = substituteMarkerArrayNotCached($titleText, $switch, $rems, $wrapped);
	...
	function fillTemplate(&$event, &$eventHTMLSubpart,&$eventPlainSubpart){
		...
		$eventHTMLSubpart = substituteMarkerArrayNotCached($eventHTMLSubpart, $switch, $rems, $wrapped);
		
		$switch = array();
		$rems = array();
		$wrapped = array();
		$event->getMarker($eventPlainSubpart,$rems,$switch, $wrapped, 'notification');
		$eventPlainSubpart = substituteMarkerArrayNotCached($eventPlainSubpart, $switch, $rems, $wrapped);
	}

	
	function notify(&$newEventDataArray){
		$event = $this->modelObj->findEvent($newEventDataArray['uid'],'tx_cal_phpicalendar', $this->conf['pidList'], true, true);
	...
	function sendNotification(&$event, $email, $templatePath, $titleText, $unsubscribeLink, $acceptLink='', $declineLink='', $ics=''){
		...
		$switch['###ACCEPT_LINK###'] = $acceptLink;
		$switch['###DECLINE_LINK###'] = $declineLink;
		$htmlTemplate = substituteMarkerArrayNotCached($htmlTemplate, $switch, $rems, $wrapped);
		...
		$switch['###ACCEPT_LINK###'] = $acceptLink;
		$switch['###DECLINE_LINK###'] = $declineLink;
		$plainTemplate = substituteMarkerArrayNotCached($plainTemplate, $switch, $rems, $wrapped);
	...
	function invite($oldEventDataArray, $newEventDataArray){
		...
		$event_old = $this->modelObj->findEvent($oldEventDataArray['uid'],'tx_cal_phpicalendar', $this->conf['pidList']);
		$event_new = $this->modelObj->findEvent($oldEventDataArray['uid'],'tx_cal_phpicalendar', $this->conf['pidList']);
		$event_new->updateWithPiVars(array_merge($oldEventDataArray,$newEventDataArray));
