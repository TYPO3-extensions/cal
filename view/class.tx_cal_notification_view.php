...
class tx_cal_notification_view extends tx_cal_base_service {
	...
	function sendNotificationOfChanges(&$event_old, &$event_new, $email, $templatePath, $titleText, $unsubscribeLink, $acceptLink = '', $declineLink = '') {
		...
		$htmlTemplate = tx_cal_functions::substituteMarkerArrayNotCached ($htmlTemplate, $switch, array (
				'###OLD_EVENT###' => $oldEventHTMLSubpart,
				'###NEW_EVENT###' => $newEventHTMLSubpart 
		), array ());
		$plainTemplate = tx_cal_functions::substituteMarkerArrayNotCached ($plainTemplate, $switch, array (
