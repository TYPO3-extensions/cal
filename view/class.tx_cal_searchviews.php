		...
		if($isOnlyResultType){
			...
			if(count($this->conf["view."]['defaultView'])==1){
				...
				if (!empty ($this->conf['page_id'])) {
					$sims["###BACKLINK###"] = $this->controller->pi_linkTP_keepPIvars($this->controller->pi_getLL('l_back'), array ("view" => $this->conf['lastview'],"query" => null, "lastview" => null, "start_day" => null, "end_day" => null, "category" => null, "location_ids" => null), $this->conf['cache'], $this->conf['clear_anyway'], $this->conf['page_id']);
				} else if($this->conf['lastview']){
					$sims["###BACKLINK###"] = $this->controller->pi_linkTP_keepPIvars($this->controller->pi_getLL('l_back'), array ("view" => $this->conf['lastview'],"query" => null, "lastview" => null, "start_day" => null, "end_day" => null, "category" => null, "location_ids" => null), $this->conf['cache'], $this->conf['clear_anyway']);
				} else {
					$sims["###BACKLINK###"] = "";
				}
