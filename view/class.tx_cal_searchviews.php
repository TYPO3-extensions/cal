...
class tx_cal_searchviews extends tx_cal_listview {
	...
	function drawSearchEventResult($master_array, $starttime, $endtime, $searchword, $locationIds, $isOnlyResultType=true) {
		...
		/* Set the date format */
		$format = $this->conf['view.']['search.']['rlmp_dateselectorlib_config.']['inputFieldDateTimeFormat'];
		if(!$format) {
			$format = "%Y%m%d";
		}
		
		if(!$this->controller->piVars['submit']){
			$languageArray['event_start_day'] = gmstrftime($format, $this->controller->getListViewTime($this->conf['view.']['search.']['defaultValues.']['start_day']));
			$languageArray['event_end_day'] = gmstrftime($format, $this->controller->getListViewTime($this->conf['view.']['search.']['defaultValues.']['end_day']));
		}else{
			if(intval($this->controller->piVars['start_day']) == 0){
				$languageArray['event_start_day'] = gmstrftime($format,$starttime);
			}else{
				$languageArray['event_start_day'] = strip_tags($this->controller->piVars['start_day']);
			}
			
			if(intval($this->controller->piVars['end_day']) == 0){
				$languageArray['event_end_day'] = gmstrftime($format,$endtime);
			}else{
	...
	function drawSearchLocationResult(&$master_array, $searchword, $isOnlyResultType=true) {
			...
			foreach ($master_array as $a => $b) {
				if (is_array($b)) {
					foreach ($b as $id => $location) {
						...
						$wrapped['###LOCATION_LINK###'] = explode('|',$this->controller->getLinkToLocation($location,'|'));
		...
		/* @todo 	Fix the formatting here.  The text ends up inside a table without rows or cells. */
			...
			foreach ($master_array as $a => $b) {
				if (is_array($b)) {
					foreach ($b as $id => $organizer) {
						...
						$wrapped['###ORGANIZER_LINK###'] = explode('|',$this->controller->getLinkToOrganizer($organizer,'|'));
						$subTemplate = $this->cObj->substituteMarkerArrayCached($subTemplate, $tempSims, $tempRems, array());
						$middle .= $this->cObj->substituteMarkerArrayCached($subTemplate, array(), array(), $wrapped);
						$i = ($i == 1) ? 0 : 1;
					}
