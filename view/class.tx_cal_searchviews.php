...
class tx_cal_searchviews extends tx_cal_listview {
	...
	function drawSearchAllResult(&$master_array) {
		...
		$parameter = array ("view" => "search_all", "lastview" => $this->conf['view'], "getdate" => $this->conf['getdate']);
			...
			foreach ($master_array as $a => $b) {
				if (is_array($b)) {
					foreach ($b as $id => $location) {
						$switch['NAME'] = $this->controller->pi_linkTP_keepPIvars( $location->getName(), array("lastview" => $this->conf['view'], "view" => "location","uid" => $location->getUid(),"type" => $location->getType()), $this->conf['cache'], $this->conf['clear_anyway']);
			...
			foreach ($master_array as $a => $b) {
				if (is_array($b)) {
					foreach ($b as $id => $organizer) {
						$switch['NAME'] = $this->controller->pi_linkTP_keepPIvars( $organizer->getName(), array("lastview" => $this->conf['view'], "view" => "organizer","uid" => $organizer->getUid(),"type" => $organizer->getType()), $this->conf['cache'], $this->conf['clear_anyway']);
		...
		if($isOnlyResultType){
			...
			if(count($this->conf["view."]['defaultView'])==1){
				...
				if (!empty ($this->conf['page_id'])) {
					$sims["###BACKLINK###"] = $this->controller->pi_linkTP_keepPIvars($this->controller->pi_getLL('l_back'), array ("view" => $this->controller->piVars['lastview'],"query" => null, "lastview" => null, "start_day" => null, "end_day" => null, "category" => null, "location_ids" => null), $this->conf['cache'], $this->conf['clear_anyway'], $this->conf['page_id']);
				} else if($this->controller->piVars['lastview']){
					$sims["###BACKLINK###"] = $this->controller->pi_linkTP_keepPIvars($this->controller->pi_getLL('l_back'), array ("view" => $this->controller->piVars['lastview'],"query" => null, "lastview" => null, "start_day" => null, "end_day" => null, "category" => null, "location_ids" => null), $this->conf['cache'], $this->conf['clear_anyway']);
				} else {
					$sims["###BACKLINK###"] = "";
				}
				$GLOBALS['TSFE']->ATagParams = 'title="'.$this->controller->pi_getLL('l_search').'"';
				if (!empty ($this->conf['page_id'])) {
					$sims["###OTHERSEARCH###"] = $this->controller->pi_linkTP_keepPIvars($this->controller->pi_getLL('l_search'), array ("view" => "search_all","lastview" => $this->conf['view']), $this->conf['cache'], $this->conf['clear_anyway'], $this->conf['page_id']);
				} else {
					$sims["###OTHERSEARCH###"] = $this->controller->pi_linkTP_keepPIvars($this->controller->pi_getLL('l_search'), array ("view" => "search_all","lastview" => $this->conf['view']), $this->conf['cache'], $this->conf['clear_anyway']);
				}
			}
