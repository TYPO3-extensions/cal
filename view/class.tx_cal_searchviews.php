...
class tx_cal_searchviews extends tx_cal_listview {
	...
	function drawSearchAllResult(&$master_array, $starttime, $endtime, $searchword, $locationIds='', $organizerIds='') {
		...
		$sims['search_action_url'] = htmlspecialchars($this->controller->pi_linkTP_keepPIvars_url($parameter));
		$page = $this->controller->replace_tags($sims, $page);
	...
	function drawSearchEventResult(&$master_array, $starttime, $endtime, $searchword, $locationIds='', $organizerIds='', $isOnlyResultType=true) {
		...
		$inputFormat = tx_cal_functions::getFormatStringFromConf($this->conf);
		...
		if(!$this->controller->piVars['submit']){
			...
			if(intval($this->controller->piVars['start_day']) == 0){
				...
				$languageArray['event_start_day'] = htmlspecialchars(strip_tags($this->controller->piVars['start_day']));
			}
			
			if(intval($this->controller->piVars['end_day']) == 0){
				$languageArray['event_end_day'] = $endtime->format($outputFormat);
			}else{
				$languageArray['event_end_day'] = htmlspecialchars(strip_tags($this->controller->piVars['end_day']));
			}
		}
		...
		if($isOnlyResultType){				
			$parameter = array ('view' => 'search_event',  'getdate' => $this->conf['getdate']);
			$urlsims['###SEARCH_ACTION_URL###'] = htmlspecialchars($this->controller->pi_linkTP_keepPIvars_url($parameter,$this->conf['cache'],true));
			$page = tx_cal_functions::substituteMarkerArrayNotCached($page, $urlsims, array(), array ());
			$rems = array();
			return $this->finish($page,$rems);
		}
	...
	function drawSearchLocationResult(&$master_array, $searchword, $isOnlyResultType=true) {
			...
			foreach ($master_array as $a => $b) {
				if (is_array($b)) {
					foreach ($b as $id => $location) {
						...
						$middle .= tx_cal_functions::substituteMarkerArrayNotCached($subTemplate, $tempSims, $tempRems, $wrapped);
						$i = ($i == 1) ? 0 : 1;
					}
				}
			}
		...
		if($isOnlyResultType){
			$parameter = array ('view' => 'search_location',  'getdate' => $this->conf['getdate']);
			$urlsims['###SEARCH_ACTION_URL###'] = htmlspecialchars($this->controller->pi_linkTP_keepPIvars_url($parameter,$this->conf['cache'],true));
			$rems['###SEARCHLOCATIONFORM###'] = tx_cal_functions::substituteMarkerArrayNotCached($rems['###SEARCHLOCATIONFORM###'], $urlsims, array(), array ());
			$page = tx_cal_functions::substituteMarkerArrayNotCached($page, $sims, $rems, array ());
			$rems = array();
			return $this->finish($page,$rems);
		}
	...
	function drawSearchOrganizerResult(&$master_array, $searchword, $isOnlyResultType=true) {
		...
		if($isOnlyResultType){				
			$parameter = array ('view' => 'search_organizer',  'getdate' => $this->conf['getdate']);
			$urlsims['###SEARCH_ACTION_URL###'] = htmlspecialchars($this->controller->pi_linkTP_keepPIvars_url($parameter,$this->conf['cache'],true));
			$rems['###SEARCHORGANIZERFORM###'] = tx_cal_functions::substituteMarkerArrayNotCached($rems['###SEARCHORGANIZERFORM###'], $urlsims, array(), array ());
			$page = tx_cal_functions::substituteMarkerArrayNotCached($page, $sims, $rems, array ());
			$rems = array();
			return $this->finish($page,$rems);
		}
	...
	function _finishSearch(&$page, &$sims ,&$rems ,$isOnlyResultType, $searchtype='all'){
		...
		if($isOnlyResultType){
			...
			if(count($this->conf['view.']['allowedViews']) > 1) {
				...
				$this->controller->getParametersForTyposcriptLink($this->local_cObj->data, array ('view' => 'search_all'), $this->conf['cache'], $this->conf['clear_anyway'], $this->conf['page_id']);
			...
			$rems['###LINKS###'] = tx_cal_functions::substituteMarkerArrayNotCached($rems['###LINKS###'], $sims, array (), array ());
		}else{
			$rems['###LINKS###'] = '';
		}
		$sims['###QUERY###'] = htmlspecialchars(strip_tags($this->controller->piVars['query']));
		$sims['###GETDATE###'] = htmlspecialchars($this->conf['getdate']);
	}
}
