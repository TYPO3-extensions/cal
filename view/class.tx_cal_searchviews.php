...
class tx_cal_searchviews extends tx_cal_listview {
	...
	function drawSearchEventResult(&$master_array, $starttime, $endtime, $searchword, $locationIds='', $organizerIds='', $isOnlyResultType=true) {
		...
		$page = substituteMarkerArrayNotCached($page, $sims, $rems, array ());
		...
		if($isOnlyResultType){				
			...
			$page = substituteMarkerArrayNotCached($page, $urlsims, array(), array ());
			$rems = array();
			return $this->finish($page,$rems);
		}
	...
	function drawSearchLocationResult(&$master_array, $searchword, $isOnlyResultType=true) {
			...
			foreach ($master_array as $a => $b) {
				if (is_array($b)) {
					foreach ($b as $id => $location) {
						...
						$middle .= substituteMarkerArrayNotCached($subTemplate, $tempSims, $tempRems, $wrapped);
						$i = ($i == 1) ? 0 : 1;
					}
				}
			}
		...
		if($isOnlyResultType){
			...
			$urlsims['###SEARCH_ACTION_URL###'] = $this->controller->pi_linkTP_keepPIvars_url($parameter);
			$rems['###SEARCHLOCATIONFORM###'] = substituteMarkerArrayNotCached($rems['###SEARCHLOCATIONFORM###'], $urlsims, array(), array ());
			$page = substituteMarkerArrayNotCached($page, $sims, $rems, array ());
			$rems = array();
			return $this->finish($page,$rems);
		}
		return substituteMarkerArrayNotCached($page, $sims, $rems, array ());
	}
	...
	function drawSearchOrganizerResult(&$master_array, $searchword, $isOnlyResultType=true) {
		...
		if($isOnlyResultType){				
			...
			$urlsims['###SEARCH_ACTION_URL###'] = $this->controller->pi_linkTP_keepPIvars_url($parameter);
			$rems['###SEARCHORGANIZERFORM###'] = substituteMarkerArrayNotCached($rems['###SEARCHORGANIZERFORM###'], $urlsims, array(), array ());
			$page = substituteMarkerArrayNotCached($page, $sims, $rems, array ());
			$rems = array();
			return $this->finish($page,$rems);
		}
	...
	function _finishSearch(&$page, &$sims ,&$rems ,$isOnlyResultType, $searchtype='all'){
		...
		if($isOnlyResultType){
			...
			$rems['###LINKS###'] = substituteMarkerArrayNotCached($rems['###LINKS###'], $sims, array (), array ());
		}else{
			$rems['###LINKS###'] = '';
		}
