...
class tx_cal_subscription_manager_view extends tx_cal_base_view {
	...
	function drawSubscriptionManager(){
		...
		if($eventUID > 0 && $email && $subscriptionHash){
			...
			//$event->getEventMarker($status, $local_rems, $local_sims, $local_wrapped);
			//$rems['###STATUS###'] = $this->cObj->substituteMarkerArrayCached($status, $local_sims, $local_rems, $local_wrapped);
		} else {
			...
			if($this->conf['subscribeFeUser'] && $this->rightsObj->isLoggedIn()){
				...
				while ($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($result)) {
					...
					$event = &new $tx_cal_phpicalendar_model('',$row, false, 'tx_cal_phpicalendar');
					$this->conf['uid'] = $event->getUid();
					$this->conf['type'] = $event->getType();
					$view = 'list';
					$event->getEventMarker($subscriptionContainer, $local_rems, $local_sims, $view);
					$eventList[] = '<li>'.$this->cObj->substituteMarkerArrayCached($subscriptionContainer, $local_sims, $local_rems, array()).'</li>';
				}
	...
	function insertMMRow($mmTable, $uid_local, $uid_foreign, $table, $sorting) {
		...
		$result = $GLOBALS['TYPO3_DB']->exec_SELECTquery($uid_local, $table, $where);
