	...
	function drawSubscriptionManager(){
		...
		if($eventUID > 0 && $email && $subscriptionHash){
			...
			if (is_object($event)) {
				...
				$noeventmessage = $this->conf['monitor']=='stop' ?
                               'l_monitor_event_unsubscribe_noevent' :
                               'l_monitor_event_subscribe_noevent';
			...
			if($this->conf['subscribeFeUser'] && $this->rightsObj->isLoggedIn()){
				...
				while ($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($result)) {
					...
					$eventList[] = '<li>'.tx_cal_functions::substituteMarkerArrayNotCached($subscriptionContainer, $local_sims, $local_rems, $local_wrapped).'</li>';
				}
