	...
	class tx_cal_weekview extends tx_cal_base_view {
		...
		*/
		function drawWeek(&$master_array, $getdate) {
			...
			if (!empty ($this->conf['view.'][$weekLinkViewTarget.'.'][$weekLinkViewTarget.'ViewPid'])) {
				$next_week_link_url = $this->controller->pi_linkTP_keepPIvars_url(array ('getdate' => $next_week, 'view' => $weekLinkViewTarget), $this->conf['cache'], $this->conf['clear_anyway'], $this->conf['view.']['week.']['weekViewPid']);
				$prev_week_link_url = $this->controller->pi_linkTP_keepPIvars_url(array ('getdate' => $prev_week, 'view' => $weekLinkViewTarget), $this->conf['cache'], $this->conf['clear_anyway'], $this->conf['view.']['week.']['weekViewPid']);
			} else {
				$next_week_link_url = $this->controller->pi_linkTP_keepPIvars_url(array ('getdate' => $next_week, 'view' => $weekLinkViewTarget), $this->conf['cache'], $this->conf['clear_anyway']);
				$prev_week_link_url = $this->controller->pi_linkTP_keepPIvars_url(array ('getdate' => $prev_week, 'view' => $weekLinkViewTarget), $this->conf['cache'], $this->conf['clear_anyway']);
			}

			$this->local_cObj->setCurrentVal($this->conf['view.']['week.']['nextWeekSymbol']);
			$this->local_cObj->data['link'] = $next_week_link_url;
			$next_week_link = $this->local_cObj->cObjGetSingle($this->conf['view.']['week.']['nextWeekLink'],$this->conf['view.']['week.']['nextWeekLink.']);
			
			$this->local_cObj->setCurrentVal($this->conf['view.']['week.']['previousWeekSymbol']);
			$this->local_cObj->data['link'] = $prev_week_link_url;
			$prev_week_link = $this->local_cObj->cObjGetSingle($this->conf['view.']['week.']['prevWeekLink'],$this->conf['view.']['week.']['prevWeekLink.']);

			$dayLinkViewTarget = &$this->conf['view.']['dayLinkTarget'];
			$this->local_cObj->data['view'] = $dayLinkViewTarget;
			if (!empty ($this->conf['view.']['day.'][$dayLinkViewTarget.'ViewPid'])) {
				$next_day_link_url = $this->controller->pi_linkTP_keepPIvars_url(array ('getdate' => $next_day, 'view' => $this->conf['view.']['dayLinkTarget']), $this->conf['cache'], $this->conf['clear_anyway'], $this->conf['view.'][$dayLinkViewTarget.'.'][$dayLinkViewTarget.'ViewPid']);
				$prev_day_link_url = $this->controller->pi_linkTP_keepPIvars_url(array ('getdate' => $prev_day, 'view' => $this->conf['view.']['dayLinkTarget']), $this->conf['cache'], $this->conf['clear_anyway'], $this->conf['view.'][$dayLinkViewTarget.'.'][$dayLinkViewTarget.'ViewPid']);
				$legend_prev_day_link_url = $this->controller->pi_linkTP_keepPIvars_url(array ('getdate' => $next_week, 'view' => $this->conf['view.']['dayLinkTarget']), $this->conf['cache'], $this->conf['clear_anyway'], $this->conf['view.'][$dayLinkViewTarget.'.'][$dayLinkViewTarget.'ViewPid']);
				$legend_next_day_link_url = $this->controller->pi_linkTP_keepPIvars_url(array ('getdate' => $prev_week, 'view' => $this->conf['view.']['dayLinkTarget']), $this->conf['cache'], $this->conf['clear_anyway'], $this->conf['view.'][$dayLinkViewTarget.'.'][$dayLinkViewTarget.'ViewPid']);
			} else {
				...
				$legend_prev_day_link_url = $this->controller->pi_linkTP_keepPIvars_url(array ('getdate' => $next_week, 'view' => $this->conf['view.']['dayLinkTarget']), $this->conf['cache'], $this->conf['clear_anyway']);
				$legend_next_day_link_url = $this->controller->pi_linkTP_keepPIvars_url(array ('getdate' => $prev_week, 'view' => $this->conf['view.']['dayLinkTarget']), $this->conf['cache'], $this->conf['clear_anyway']);
			}

			$this->local_cObj->setCurrentVal($this->conf['view.']['week.']['nextDaySymbol']);
			$this->local_cObj->data['link'] = $next_day_link_url;
			$next_day_link = $this->local_cObj->cObjGetSingle($this->conf['view.']['week.']['nextDayLink'],$this->conf['view.']['week.']['nextDayLink.']);
			
			$this->local_cObj->setCurrentVal($this->conf['view.']['week.']['previousDaySymbol']);
			$this->local_cObj->data['link'] = $prev_day_link_url;
			$prev_day_link = $this->local_cObj->cObjGetSingle($this->conf['view.']['week.']['prevDayLink'],$this->conf['view.']['week.']['prevDayLink.']);
			...
			if($this->conf['view.']['week.']['dynamic']==1){
				...
				$firstStart = true;
				$firstEnd = true;
				$dynamicEnd = intval($start_week_time->format('%Y%m%d'))+7;
				...
				for ($i->copy($d_start); !$i->after($d_end); $i->addSeconds($gridLength * 60)) {
					...
					if (is_array($view_array[$week_key][$timeKey]) && count($view_array[$week_key][$timeKey]) > 0) {
						foreach (array_keys($view_array[$week_key][$timeKey]) as $eventKey) {
							...
							$eventStart = $event->getStart();
							if (is_array($pos_array[$week_key]) && array_key_exists($event->getType().$event->getUid().'_'.$eventStart->format('%Y%m%d%H%M'), ($pos_array[$week_key]))) {
								$nd = new tx_cal_date();
								$nd->copy($event->getEnd());
								$nd->addSeconds(($gridLength-(($nd->getMinute())%$gridLength))*60);
								if ($nd->before($i)) {
									$t_array[$week_key][$timeKey][$pos_array[$week_key][$event->getType().$event->getUid().'_'.$eventStart->format('%Y%m%d%H%M')]] = array ('ended' => $week_key.'_'.$event->getType().'_'.$event->getUid().'_'.$eventStart->format('%Y%m%d%H%M'));
								} else {
									$t_array[$week_key][$timeKey][$pos_array[$week_key][$event->getType().$event->getUid().'_'.$eventStart->format('%Y%m%d%H%M')]] = array ('started' => $week_key.'_'.$event->getType().'_'.$event->getUid().'_'.$eventStart->format('%Y%m%d%H%M'));
								}
							} else {
								for ($j = 0; $j < $nbrGridCols[$week_key]?$nbrGridCols[$week_key]:1; $j ++) {
									if (count($t_array[$week_key][$timeKey][$j]) == 0 || !isset ($t_array[$week_key][$timeKey][$j])) {
										$pos_array[$week_key][$event->getType().$event->getUid().'_'.$eventStart->format('%Y%m%d%H%M')] = $j;
			...
			for ($i = 0; $i < 7; $i ++) {
				...
				if(($this->rightsObj->isViewEnabled($dayLinkViewTarget) || $this->conf['view.'][$dayLinkViewTarget.'.'][$dayLinkViewTarget.'ViewPid']) && ($view_array[$daylink] || $isAllowedToCreateEvent)){
					if (!empty ($this->conf['view.'][$dayLinkViewTarget.'.'][$dayLinkViewTarget.'ViewPid'])) {
						$link_url = $this->controller->pi_linkTP_keepPIvars_url(array ('getdate' => $daylink, 'view' => $this->conf['view.']['dayLinkTarget']), $this->conf['cache'], $this->conf['clear_anyway'], $this->conf['view.'][$dayLinkViewTarget.'.'][$dayLinkViewTarget.'ViewPid']);
					} else {
						$link_url = $this->controller->pi_linkTP_keepPIvars_url(array ('getdate' => $daylink, 'view' => $this->conf['view.']['dayLinkTarget']), $this->conf['cache'], $this->conf['clear_anyway']);
					}
					$this->initLocalCObject();
					$this->local_cObj->setCurrentVal($weekday);
					$this->local_cObj->data['view'] = $dayLinkViewTarget;
					$this->local_cObj->data['link'] = $link_url;
			...
			$weekTemplate = substituteMarkerArrayNotCached($weekTemplate, $sims, array (), array ());
			$rems['###LOOPEVENTS###'] = $weekdisplay;
			$page = substituteMarkerArrayNotCached($page, array(), array ('###WEEK_TEMPLATE###'=>$weekTemplate), array ());
			return $this->finish($page, $rems);
		}
	}
