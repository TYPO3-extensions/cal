		...
		foreach ($weekarray as $get_date) {
			...
			if (is_array($view_array[$get_date]['-1'])) {
				foreach ($view_array[$get_date]['-1'] as $id => $allday) {
					$sims["###ALLDAY###"] = '<div class="'.$allday->getHeaderStyle().'_allday">'.$this->cObj->substituteMarkerArrayCached($this->getLinkToEvent($allday, $allday->renderEventForAllDay(),"day", $this->cObj->conf['getdate']), $loop_ad, array (), array ())."</div>";
						...
						if(!empty($something[$k])){
							...
							switch ($keys[0]) {
								case 'begin' :
									...
									$sims["###EVENT###"] = $this->cObj->substituteMarkerArrayCached($this->getLinkToEvent($event, $event->renderEventForWeek(),"week", $this->cObj->conf['getdate']), $event_temp, array (), array ());
