							...
							for($l = 0; $l < ($nbrGridCols[$day]?$nbrGridCols[$day]:1); $l++) {
								if (!$something[$l]) {
									...
									if ($time > time()) {
										...
										$link = $this->controller->pi_linkTP_keepPIvars($this->conf['view.']['week.']['addIcon'], array ('getdate' => $getdate, 'gettime' => date('Hi', $time), 'lastview' => 'week', 'view' => 'create_event'), $this->conf['cache'], $this->conf['clear_anyway'], $this->conf['view.']['event.']['createEventViewPid']);
									}
