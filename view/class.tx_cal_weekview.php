	...
	class tx_cal_weekview extends tx_cal_base_view {
		...
		*/
		function drawWeek(&$master_array, $getdate) {
			...
			$start_week_time = strtotime($dateOfWeek) + strtotimeOffset();
			...
			$start_day = strtotime($dateOfWeek) + strtotimeOffset();
			...
			for ($i = 0; $i < 7; $i ++) {
				...
				if($this->rightsObj->isViewEnabled($this->conf['view.']['dayLinkTarget']) && ($view_array[$daylink] || $isAllowedToCreateEvents)){
					$GLOBALS['TSFE']->ATagParams = 'title="'.$this->controller->pi_getLL('l_'.$this->conf['view.']['dayLinkTarget'].'_view').'"';
					if (!empty ($this->conf['view.']['day.']['dayViewPid'])) {
						...
						for ($k = 0; $k < count($something); $k ++) {
							if (!empty($something[$k])) {
								...
								switch ($keys[0]) {
									...
									else if (is_array($event->getCalRecu()) && count($event->getCalRecu())>0) {
									...
									if ($event->isUserAllowedToEdit()) {
										...
										$sims['###EDITLINK###'] = $this->controller->pi_linkTP_keepPIvars($this->conf['view.']['week.']['editIcon'], array ('lastview' => $this->controller->extendLastView(), 'view' => 'edit_event', 'type' => $event->getType(), 'uid' => $event->getUid()), $this->conf['cache'], $this->conf['clear_anyway'], $this->conf['view.']['event.']['editEventViewPid']);
									}
									if ($event->isUserAllowedToDelete()) {
										...
										$sims['###EDITLINK###'] .= $this->controller->pi_linkTP_keepPIvars($this->conf['view.']['week.']['deleteIcon'], array('lastview' => $this->controller->extendLastView(), 'view' => 'delete_event', 'type' => $event->getType(), 'uid' => $event->getUid()), $this->conf['cache'], $this->conf['clear_anyway'], $this->conf['view.']['event.']['deleteEventViewPid']);
									}
									...
									$e_start = gmstrftime($this->conf['view.']['week.']['eventStartTimeFormatWeek'], $event->getStarttime());
									$sims['###EVENT_START###'] = $confirmed.$this->cObj->substituteMarkerArrayCached($e_start, $event_temp, array (), array ());
									$e_end = gmstrftime($this->conf['view.']['week.']['eventEndTimeFormatWeek'], $event->getEndtime());
									...
									if ($this->conf['view.']['freeAndBusy.']['enable'] && (!$event->isEventOwner($this->rightsObj->getUserId(), $this->rightsObj->getUserGroups()) && !$event->isSharedUser($this->rightsObj->getUserId())) && $this->conf['option'] == 'freeandbusy' && $this->conf['calendar']) {
										...
										$sims['###EVENT###'] = $this->getLinkToEvent($event, $event->renderEventForWeek(), 'week', $getdate);
							...
							for($l = 0; $l < ($nbrGridCols[$day]?$nbrGridCols[$day]:1); $l++) {
								if (!$something[$l]) {
									...
									if ($time > (time()+$createOffset) && $isAllowedToCreateEvents) {
										...
										$link = $this->controller->pi_linkTP_keepPIvars($this->conf['view.']['week.']['addIcon'], array ('getdate' => $day, 'gettime' => gmdate('Hi', $time), 'lastview' => $this->controller->extendLastView(), 'view' => 'create_event'), $this->conf['cache'], $this->conf['clear_anyway'], $this->conf['view.']['event.']['createEventViewPid']);
									}
