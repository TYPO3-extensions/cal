	...
	class tx_cal_weekview extends tx_cal_base_view {
		...
		*/
		function drawWeek(&$master_array, $getdate) {
			...
			$weekViewPid = $this->conf['view.'][$weekLinkViewTarget.'.'][$weekLinkViewTarget.'ViewPid'] ? $this->conf['view.'][$weekLinkViewTarget.'.'][$weekLinkViewTarget.'ViewPid'] : false;
			
			// next week
			$this->controller->getParametersForTyposcriptLink($this->local_cObj->data, array ('getdate' => $next_week, 'view' => $weekLinkViewTarget, $this->pointerName => NULL), $this->conf['cache'], $this->conf['clear_anyway'], $weekViewPid);
			...
			// prev week
			$this->controller->getParametersForTyposcriptLink($this->local_cObj->data, array ('getdate' => $prev_week, 'view' => $weekLinkViewTarget, $this->pointerName => NULL), $this->conf['cache'], $this->conf['clear_anyway'], $weekViewPid);
			...
			$dayViewPid = $this->conf['view.']['day.'][$dayLinkViewTarget.'ViewPid'] ? $this->conf['view.']['day.'][$dayLinkViewTarget.'ViewPid'] : false;
						...
						foreach ($ovlDayKeys as $ovl2Key) {
							...
							if ($ovl_time_key == '-1') {
								...
								while($j->before($endtime) && $j->before($end_week_time)){
									$counter++;
									$view_array[$j->format('%Y%m%d')][$j->format('%H%M')][] = $ovlKey.'_'.$event->getType().'_'.$event->getUid().'_'.$eventStart->format('%Y%m%d%H%M');
									if($j->after($endOfDay)){
