...
require_once (t3lib_extMgm :: extPath('cal').'view/class.tx_cal_monthview.php');
...
class tx_cal_yearview extends tx_cal_monthview {
	...
	function drawYear(&$master_array, $getdate) {
		...
		$page = $this->cObj->fileResource($this->cObj->conf["view."]["year."]["yearTemplate"]);
		if ($page == "") {
			return "<h3>calendar: no template file found:</h3>".$this->cObj->conf["view."]["year."]["yearTemplate"]."<br />Please check your template record and add both cal items at 'include static (from extension)'";
		}
		$array = array();
